  ...
  void printTimer(std::map<std::string, double> r) {
    for (auto &p : r) {
      ...
      std::cout << std::setw(10) << std::right << std::fixed
                << std::setprecision(3) << p.second << std::endl;
    }
  }
...
class TimeTestBFVBGV : public TimeTest {
 ...
 public:
  TimeTestBFVBGV(bool bgv, size_t polyModulusDegree,
                 uint64_t plainModulusBitSize, vector<int> qs,
                 int dataBound = 1 << 6) {
    KernelProvider::initialize();
    slotCount = polyModulusDegree / 2;
    this->dataBound = dataBound;
    ...
    parms.setPlainModulus(
        PlainModulus::Batching(polyModulusDegree, plainModulusBitSize));
