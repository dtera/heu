...
int getDecValue(char c) { return c - '0'; }
...
Plaintext &Plaintext::operator=(const string &hex_poly) {
  ...
  int last_power = safe_cast<int>(
      min(data_.maxSize(), safe_cast<size_t>(numeric_limits<int>::max())));
  ...
  while (pos < length) {
    ...
    int coeff_bit_count =
        getHexStringBitCount(hex_poly_ptr + pos, coeff_length);
...
//         sizeof(parms_id_type)); uint64_t coeff_count64 =
//         static_cast<uint64_t>(coeff_count_);
//         stream.write(reinterpret_cast<const char *>(&coeff_count64),
//         sizeof(uint64_t)); stream.write(reinterpret_cast<const char
//         *>(&scale_), sizeof(double)); data_.save(stream,
...
// void Plaintext::load_members(const SEALContext &context, istream &stream,
...
//         stream.read(reinterpret_cast<char *>(&parms_id),
//         sizeof(parms_id_type));

//         uint64_t coeff_count64 = 0;
//         stream.read(reinterpret_cast<char *>(&coeff_count64),
//         sizeof(uint64_t));
...
//         // further means that functions reading in Plaintext objects must
//         check
