...
#define KERNEL_CALL(funcname, n)                                \
  size_t block_count = seal_gpu::kernel_util::ceilDiv_(n, 256); \
...
void CiphertextCuda::save(std::ostream &stream) const {
  ...
  if (seed_ != 0) {
    ...
    stream.write(reinterpret_cast<char *>(r.get()),
                 sizeof(CiphertextCuda::ct_coeff_type) * r.size());
  } else {
    auto r = data_.toHost();
    size_t dataSize = r.size();
    savet(stream, &dataSize);
    stream.write(reinterpret_cast<char *>(r.begin()),
                 sizeof(CiphertextCuda::ct_coeff_type) * r.size());
  }
}

void CiphertextCuda::saveTerms(std::ostream &stream, EvaluatorCuda &evaluator,
  ...
  savet(stream, &dataSize);
  stream.write(reinterpret_cast<char *>(r.begin() + offset),
               sizeof(CiphertextCuda::ct_coeff_type) * dataSize);
}
...
void CiphertextCuda::load(std::istream &stream) {
  ...
  if (terms)
    throw std::invalid_argument(
        "Trying to load a termed ciphertext, but indices is not specified");
  if (seed == 0) {
    size_t dataSize;
    loadt(stream, &dataSize);
    util::HostArray<ct_coeff_type> host(dataSize);
    stream.read(reinterpret_cast<char *>(host.get()),
                dataSize * sizeof(ct_coeff_type));
...
void CiphertextCuda::loadTerms(std::istream &stream, EvaluatorCuda &evaluator,
  ...
  if (!terms)
    throw std::invalid_argument(
        "Trying to load a normal ciphertext, but term indices is specified");

  util::HostArray<ct_coeff_type> host(coeff_modulus_size_ *
                                      poly_modulus_degree_ * size_);
  // load degree 0 terms
  for (size_t id : termIds) {
    for (size_t j = 0; j < coeff_modulus_size_; j++) {
      ct_coeff_type num;
  ...
  if (seed == 0) {
    ...
    size_t dataSize;
    loadt(stream, &dataSize);
    stream.read(reinterpret_cast<char *>(host.get() + offset),
                dataSize * sizeof(ct_coeff_type));
    data_.ensure(host.size());
...
void CiphertextCuda::load(std::istream &stream,
  ...
  if (seed == 0) {
    ...
    KernelProvider::copy(data_.get(), host.get(), dataSize);
    util::DeviceArray<curandState> curandStates(poly_modulus_degree_ *
                                                coeff_modulus_size_);
    auto &modulus = context.getContextData(parms_id_)->parms().coeffModulus();
    util::sampler::setupCurandStates(curandStates.get(), poly_modulus_degree_,
                                     seed);
    util::sampler::kSamplePolyUniform(curandStates.get(), modulus.size(),
                                      poly_modulus_degree_, modulus, data(1));
    ...
    if (type == SchemeType::bfv) {
      kernel_util::kInverseNttNegacyclicHarvey(data(1), 1, coeff_modulus_size,
