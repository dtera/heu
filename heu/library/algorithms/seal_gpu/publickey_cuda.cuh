...
class PublicKeyCuda {
 ...
 public:
  PublicKeyCuda(const PublicKey &copy) : pk_(copy.data()) {}
  ...
  */
  inline auto &data() noexcept { return pk_; }

  /**
  Returns a const reference to the underlying data.
  */
  inline auto &data() const noexcept { return pk_; }
  ...
  */
  inline auto &parmsID() noexcept { return pk_.parmsID(); }

  /**
  Returns a const reference to parms_id.
  */
  inline auto &parmsID() const noexcept { return pk_.parmsID(); }

  void save(std::ostream &stream) const { pk_.save(stream); }

  void load(std::istream &stream) { pk_.load(stream); }
