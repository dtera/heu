...
template <typename T>
void print(const char *message, T i, T total) {
  std::cout << message << " = " << i << " / " << total << "\n";
}

inline void printArray(const uint64_t *start, size_t count,
...
inline void printArray(const HostArray<uint64_t> &s,
                       bool dont_compress = false) {
...
inline bool isScaleWithinBounds(
  ...
  return !(scale <= 0 ||
           (static_cast<int>(log2(scale)) >= scale_bit_count_bound));
}
...
(3) abs(e1_bal) + abs(e2_bal) is minimal, where e1_bal and e2_bal represent e1
and e2 in (-p/2, p/2].
*/
inline auto balanceCorrectionFactors(uint64_t factor1, uint64_t factor2,
  ...
  while (a != 0) {
    ...
    uint64_t a_mod =
        barrettReduce64(static_cast<uint64_t>(abs(a)), plain_modulus);
    if (a < 0) {
      a_mod = negateUintMod(a_mod, plain_modulus);
    }
    uint64_t b_mod =
        barrettReduce64(static_cast<uint64_t>(abs(b)), plain_modulus);
    if (b < 0) {
      b_mod = negateUintMod(b_mod, plain_modulus);
    }
    if (a_mod != 0 &&
        gcd(a_mod, t) == 1)  // which also implies gcd(b_mod, t) == 1
    {
...
void Evaluator::negateInplace(Ciphertext &encrypted) const {
  ...
  negatePolyCoeffmod(encrypted.data(), encrypted_size, coeff_modulus.size(),
                     parms.polyModulusDegree(), &coeff_modulus[0],
                     encrypted.data());
}

void Evaluator::addInplace(Ciphertext &encrypted1,
                           const Ciphertext &encrypted2) const {
  // Verify parameters.
  ...
  if (encrypted1.correctionFactor() != encrypted2.correctionFactor()) {
    ...
    auto factors =
        balanceCorrectionFactors(encrypted1.correctionFactor(),
                                 encrypted2.correctionFactor(), plain_modulus);
    multiplyPolyScalarCoeffmod(encrypted1.data(), encrypted1.size(),
                               coeff_modulus_size, coeff_count, get<1>(factors),
                               &coeff_modulus[0], encrypted1.data());

    Ciphertext encrypted2_copy = encrypted2;
    multiplyPolyScalarCoeffmod(encrypted2.data(), encrypted2.size(),
                               coeff_modulus_size, coeff_count, get<2>(factors),
    ...
    // Add ciphertexts
    addPolyCoeffmod(encrypted1.data(), encrypted2.data(), min_count,
                    coeff_modulus_size, coeff_count, &coeff_modulus[0],
                    encrypted1.data());

    // Copy the remainding polys of the array with larger count into encrypted1
    if (encrypted1_size < encrypted2_size) {
      setPolyArray(encrypted2.data(min_count),
                   encrypted2_size - encrypted1_size, coeff_count,
                   coeff_modulus_size, encrypted1.data(encrypted1_size));
    }
  }
}

void Evaluator::addMany(const vector<Ciphertext> &encrypteds,
...
void Evaluator::subInplace(Ciphertext &encrypted1,
                           const Ciphertext &encrypted2) const {
  // Verify parameters.
  if (!isMetadataValidFor(encrypted1, context_) || !isBufferValid(encrypted1)) {
    throw invalid_argument("encrypted1 is not valid for encryption parameters");
  ...
  if (encrypted1.correctionFactor() != encrypted2.correctionFactor()) {
    // Balance correction factors and multiply by scalars before subtraction in
    // BGV
    auto factors =
        balanceCorrectionFactors(encrypted1.correctionFactor(),
                                 encrypted2.correctionFactor(), plain_modulus);
    ...
    subPolyCoeffmod(encrypted1.data(), encrypted2.data(), min_count,
                    coeff_modulus_size, coeff_count, coeff_modulus.data(),
                    encrypted1.data());

    // If encrypted2 has larger count, negate remaining entries
    if (encrypted1_size < encrypted2_size) {
      negatePolyCoeffmod(encrypted2.data(min_count),
                         encrypted2_size - min_count, coeff_modulus_size,
...
void Evaluator::multiplyInplace(Ciphertext &encrypted1,
                                const Ciphertext &encrypted2) const {
  // Verify parameters.
  if (!isMetadataValidFor(encrypted1, context_) || !isBufferValid(encrypted1)) {
    throw invalid_argument("encrypted1 is not valid for encryption parameters");
...
void Evaluator::bfvMultiply(Ciphertext &encrypted1,
                            const Ciphertext &encrypted2) const {
  ...
  size_t dest_size =
      sub_safe(add_safe(encrypted1_size, encrypted2_size), size_t(1));
  ...
  // Microsoft SEAL uses BEHZ-style RNS multiplication. This process is somewhat
  // complex and consists of the following steps:
  //
  // (1) Lift encrypted1 and encrypted2 (initially in base q) to an extended
  // base q U Bsk U {m_tilde} (2) Remove extra multiples of q from the results
  // with Montgomery reduction, switching base to q U Bsk (3) Transform the data
  // to NTT form (4) Compute the ciphertext polynomial product using dyadic
  // multiplication (5) Transform the data back from NTT form (6) Multiply the
  // result by t (plain_modulus) (7) Scale the result by q using a
  // divide-and-floor algorithm, switching base to Bsk (8) Use Shenoy-Kumaresan
  // method to convert the result to base q
  ...
  // Allocate space for a base q output of behz_extend_base_convertToNtt for
  // encrypted1
  auto encrypted1_q =
      allocatePolyArray(encrypted1_size, coeff_count, base_q_size);

  // Allocate space for a base Bsk output of behz_extend_base_convertToNtt for
  // encrypted1
  auto encrypted1_Bsk =
      allocatePolyArray(encrypted1_size, coeff_count, base_Bsk_size);

  // Perform BEHZ steps (1)-(3) for encrypted1
  // SEAL_ITERATE(iter(encrypted1, encrypted1_q, encrypted1_Bsk),
  // encrypted1_size, behz_extend_base_convertToNtt);
  for (size_t i = 0; i < encrypted1_size; i++) {
    ...
    //
    // It performs steps (1)-(3) of the BEHZ multiplication (see above) on the
    // given input polynomial (given as an RNSIter or ConstRNSIter) and writes
    // the results in base q and base Bsk to the given output iterators. auto
    // behz_extend_base_convertToNtt = [&](auto I) { Make copy of input
    // polynomial (in base q) and convert to NTT form Lazy reduction
    setPoly(encrypted1.data(i), coeff_count, base_q_size,
            encrypted1_q.get() + i * coeff_count * base_q_size);
    nttNegacyclicHarveyLazy(encrypted1_q.get() + i * coeff_count * base_q_size,
                            base_q_size, base_q_ntt_tables);
    ...
    // (2) Reduce q-overflows in with Montgomery reduction, switching base to
    // Bsk
    rns_tool->smMrq(temp.asPointer(),
                    encrypted1_Bsk.get() + i * coeff_count * base_Bsk_size);

    // Transform to NTT form in base Bsk
    // Lazy reduction
    nttNegacyclicHarveyLazy(
        encrypted1_Bsk.get() + i * coeff_count * base_Bsk_size, base_Bsk_size,
        base_Bsk_ntt_tables);
  }
  ...
  auto encrypted2_q =
      allocatePolyArray(encrypted2_size, coeff_count, base_q_size);
  auto encrypted2_Bsk =
      allocatePolyArray(encrypted2_size, coeff_count, base_Bsk_size);

  for (size_t i = 0; i < encrypted2_size; i++) {
    setPoly(encrypted2.data(i), coeff_count, base_q_size,
            encrypted2_q.get() + i * coeff_count * base_q_size);
    nttNegacyclicHarveyLazy(encrypted2_q.get() + i * coeff_count * base_q_size,
                            base_q_size, base_q_ntt_tables);
    ...
    // Bsk
    rns_tool->smMrq(temp.asPointer(),
                    encrypted2_Bsk.get() + i * coeff_count * base_Bsk_size);

    nttNegacyclicHarveyLazy(
        encrypted2_Bsk.get() + i * coeff_count * base_Bsk_size, base_Bsk_size,
        base_Bsk_ntt_tables);
  }
  ...
  auto temp_dest_Bsk =
      allocateZeroPolyArray(dest_size, coeff_count, base_Bsk_size);
  ...
  for (size_t i = 0; i < dest_size; i++) {
    ...
    // We iterate over relevant components of encrypted1 and encrypted2 in
    // increasing order for encrypted1 and reversed (decreasing) order for
    // encrypted2. The bounds for the indices of the relevant terms are obtained
    // as follows.
    ...
    // Perform the BEHZ ciphertext product both for base q and base Bsk
    {
      ...
      for (size_t j = 0; j < steps; j++) {
        for (size_t k = 0; k < base_q_size; k++) {
          ...
          // std::cout << std::hex << *(shifted_in1_iter + j * d + k *
          // coeff_count) << std::endl;
          dyadicProductCoeffmod(
              shifted_in1_iter + j * d + k * coeff_count,
              shifted_reversed_in2_iter - j * d + k * coeff_count, coeff_count,
              base_q[k], temp.asPointer());
          // std::cout << "first of temp: " << temp[0] << std::endl;
          addPolyCoeffmod(temp.asPointer(), shifted_out_iter + k * coeff_count,
                          coeff_count, base_q[k],
                          shifted_out_iter + k * coeff_count);
          // std::cout << "first of res: " << *(shifted_out_iter + k *
          // coeff_count) << std::endl;
        }
      }
    }
    {
      ...
      // Create a shifted reverse iterator for the second input
      auto shifted_reversed_in2_iter =
          encrypted2_Bsk + curr_encrypted2_first * d;
      ...
      for (size_t j = 0; j < steps; j++) {
        for (size_t k = 0; k < base_Bsk_size; k++) {
          ...
          dyadicProductCoeffmod(
              shifted_in1_iter + j * d + k * coeff_count,
              shifted_reversed_in2_iter - j * d + k * coeff_count, coeff_count,
              base_Bsk[k], temp.asPointer());
          addPolyCoeffmod(temp.asPointer(), shifted_out_iter + k * coeff_count,
                          coeff_count, base_Bsk[k],
                          shifted_out_iter + k * coeff_count);
        }
      }
    }
  ...
  // Lazy reduction here. The following multiplyPolyScalarCoeffmod will correct
  // the value back to [0, p)
  inverseNttNegacyclicHarveyLazy(temp_dest_q.asPointer(), dest_size,
                                 base_q_size, base_q_ntt_tables);
  inverseNttNegacyclicHarveyLazy(temp_dest_Bsk.asPointer(), dest_size,
                                 base_Bsk_size, base_Bsk_ntt_tables);
  ...
  for (size_t i = 0; i < dest_size; i++) {
    // SEAL_ITERATE(iter(temp_dest_q, temp_dest_Bsk, encrypted1), dest_size,
    // [&](auto I) { Bring together the base q and base Bsk components into a
    // single allocation
    auto temp_q_Bsk = allocatePoly(coeff_count, base_q_size + base_Bsk_size);

    // Step (6): multiply base q components by t (plain_modulus)
    multiplyPolyScalarCoeffmod(temp_dest_q + i * coeff_count * base_q_size,
                               base_q_size, coeff_count, plain_modulus, base_q,
                               temp_q_Bsk.asPointer());

    multiplyPolyScalarCoeffmod(temp_dest_Bsk + i * coeff_count * base_Bsk_size,
                               base_Bsk_size, coeff_count, plain_modulus,
                               base_Bsk,
                               temp_q_Bsk + base_q_size * coeff_count);
    ...
    // Step (8): use Shenoy-Kumaresan method to convert the result to base q and
    // write to encrypted1
...
void Evaluator::ckksMultiply(Ciphertext &encrypted1,
                             const Ciphertext &encrypted2) const {
  if (!(encrypted1.isNttForm() && encrypted2.isNttForm())) {
  ...
  for (size_t i = 0; i < dest_size; i++) {
    ...
    // Create a shifted reverse iterator for the second input
    auto shifted_reversed_encrypted2_iter =
        encrypted2.data(curr_encrypted2_first);
    ...
    for (size_t j = 0; j < steps; j++) {
      for (size_t k = 0; k < coeff_modulus_size; k++) {
        ...
        addPolyCoeffmod(prod.asPointer(), temp + i * d + k * coeff_count,
  ...
  setPolyArray(temp.get(), dest_size, coeff_count, coeff_modulus_size,
               encrypted1.data());
...
void Evaluator::bgvMultiply(Ciphertext &encrypted1,
                            const Ciphertext &encrypted2) const {
  if (encrypted1.isNttForm() || encrypted2.isNttForm()) {
  ...
  nttNegacyclicHarvey(encrypted1.data(), encrypted1_size, coeff_modulus_size,
                      ntt_table);
  ...
  if (&encrypted1 == &encrypted2) {
    ...
    nttNegacyclicHarvey(encrypted2_cpy.data(), encrypted2_size,
                        coeff_modulus_size, ntt_table);
  ...
  for (size_t i = 0; i < dest_size; i++) {
    ...
    // Create a shifted reverse iterator for the second input
    auto shifted_reversed_encrypted2_iter =
        encrypted2_iter + curr_encrypted2_first * d;
  ...
  inverseNttNegacyclicHarvey(encrypted1.data(), encrypted1.size(),
                             coeff_modulus_size, ntt_table);

  // Set the correction factor
  encrypted1.correctionFactor() =
      multiplyUintMod(encrypted1.correctionFactor(),
                      encrypted2.correctionFactor(), parms.plainModulus());
}
...
void Evaluator::bfvSquare(Ciphertext &encrypted) const {
  ...
  // Determine destination.size()
  size_t dest_size =
      sub_safe(add_safe(encrypted_size, encrypted_size), size_t(1));
  ...
  // Microsoft SEAL uses BEHZ-style RNS multiplication. For details, see
  // Evaluator::bfv_multiply. This function uses additionally Karatsuba
  // multiplication to reduce the complexity of squaring a size-2 ciphertext,
  // but the steps are otherwise the same as in Evaluator::bfv_multiply.
  ...
  auto encrypted_q =
      allocatePolyArray(encrypted_size, coeff_count, base_q_size);
  // SEAL_ALLOCATE_GET_POLY_ITER(encrypted_q, encrypted_size, coeff_count,
  // base_q_size, pool);

  // Allocate space for a base Bsk output of behz_extend_base_convertToNtt
  auto encrypted_Bsk =
      allocatePolyArray(encrypted_size, coeff_count, base_Bsk_size);
  // SEAL_ALLOCATE_GET_POLY_ITER(encrypted_Bsk, encrypted_size, coeff_count,
  // base_Bsk_size, pool);
  ...
  for (size_t i = 0; i < encrypted_size; i++) {
    ...
    // 3. RNSIter for the output in base Bsk
    //
    // It performs steps (1)-(3) of the BEHZ multiplication on the given input
    // polynomial (given as an RNSIter or ConstRNSIter) and writes the results
    // in base q and base Bsk to the given output iterators.
    // [&](auto I) {
    // Make copy of input polynomial (in base q) and convert to NTT form
    // Lazy reduction
    setPoly(encrypted.data(i), coeff_count, base_q_size,
            encrypted_q.get() + i * coeff_count * base_q_size);
    nttNegacyclicHarveyLazy(encrypted_q.get() + i * coeff_count * base_q_size,
                            base_q_size, base_q_ntt_tables);

    // Allocate temporary space for a polynomial in the Bsk U {m_tilde} base
    // FIXME: allocate related action frequent
    auto temp = allocatePoly(coeff_count, base_Bsk_m_tilde_size);
    // SEAL_ALLOCATE_GET_RNS_ITER(temp, coeff_count, base_Bsk_m_tilde_size,
    // pool);
    ...
    // Bsk
    rns_tool->smMrq(temp.asPointer(),
                    encrypted_Bsk.get() + i * coeff_count * base_Bsk_size);

    // Transform to NTT form in base Bsk
    // Lazy reduction
    nttNegacyclicHarveyLazy(
        encrypted_Bsk.get() + i * coeff_count * base_Bsk_size, base_Bsk_size,
        base_Bsk_ntt_tables);
  }
  ...
  // This lambda function computes the size-2 ciphertext square for BFV
  // multiplication. Since we use the BEHZ approach, the multiplication of
  // individual polynomials is done using a dyadic product where the inputs are
  // already in NTT form. The arguments of the lambda function are expected to
  // be as follows:
  //
  // 1. a ConstPolyIter pointing to the beginning of the input ciphertext (in
  // NTT form)
  ...
  // Compute c0^2
  dyadicProductCoeffmod(encrypted_q + 0 * d_q, encrypted_q + 0 * d_q,
                        base_q_size, coeff_count, base_q,
                        temp_dest_q + 0 * d_q);
  // Compute 2*c0*c1
  dyadicProductCoeffmod(encrypted_q + 0 * d_q, encrypted_q + 1 * d_q,
                        base_q_size, coeff_count, base_q,
                        temp_dest_q + 1 * d_q);
  addPolyCoeffmod(temp_dest_q + 1 * d_q, temp_dest_q + 1 * d_q, base_q_size,
                  coeff_count, base_q, temp_dest_q + 1 * d_q);
  // Compute c1^2
  dyadicProductCoeffmod(encrypted_q + 1 * d_q, encrypted_q + 1 * d_q,
                        base_q_size, coeff_count, base_q,
                        temp_dest_q + 2 * d_q);

  // Compute c0^2
  dyadicProductCoeffmod(encrypted_Bsk + 0 * d_Bsk, encrypted_Bsk + 0 * d_Bsk,
                        base_Bsk_size, coeff_count, base_Bsk,
                        temp_dest_Bsk + 0 * d_Bsk);
  // Compute 2*c0*c1
  dyadicProductCoeffmod(encrypted_Bsk + 0 * d_Bsk, encrypted_Bsk + 1 * d_Bsk,
                        base_Bsk_size, coeff_count, base_Bsk,
                        temp_dest_Bsk + 1 * d_Bsk);
  addPolyCoeffmod(temp_dest_Bsk + 1 * d_Bsk, temp_dest_Bsk + 1 * d_Bsk,
                  base_Bsk_size, coeff_count, base_Bsk,
                  temp_dest_Bsk + 1 * d_Bsk);
  // Compute c1^2
  dyadicProductCoeffmod(encrypted_Bsk + 1 * d_Bsk, encrypted_Bsk + 1 * d_Bsk,
                        base_Bsk_size, coeff_count, base_Bsk,
                        temp_dest_Bsk + 2 * d_Bsk);

  // Perform BEHZ step (5): transform data from NTT form
  inverseNttNegacyclicHarvey(temp_dest_q.asPointer(), dest_size, base_q_size,
                             base_q_ntt_tables);
  inverseNttNegacyclicHarvey(temp_dest_Bsk.asPointer(), dest_size,
                             base_Bsk_size, base_Bsk_ntt_tables);
...
void Evaluator::ckksSquare(Ciphertext &encrypted) const {
  ...
  // Compute c1^2
  dyadicProductCoeffmod(encrypted_iter + 1 * d, encrypted_iter + 1 * d,
                        coeff_modulus_size, coeff_count, coeff_modulus,
                        encrypted_iter + 2 * d);

  // Compute 2*c0*c1
  dyadicProductCoeffmod(encrypted_iter + 0 * d, encrypted_iter + 1 * d,
                        coeff_modulus_size, coeff_count, coeff_modulus,
                        encrypted_iter + 1 * d);
  addPolyCoeffmod(encrypted_iter + 1 * d, encrypted_iter + 1 * d,
                  coeff_modulus_size, coeff_count, coeff_modulus,
                  encrypted_iter + 1 * d);

  // Compute c0^2
  dyadicProductCoeffmod(encrypted_iter + 0 * d, encrypted_iter + 0 * d,
                        coeff_modulus_size, coeff_count, coeff_modulus,
                        encrypted_iter + 0 * d);
...
void Evaluator::bgvSquare(Ciphertext &encrypted) const {
  ...
  // Convert c0 and c1 to ntt
  nttNegacyclicHarvey(encrypted.data(), encrypted_size, coeff_modulus_size,
                      ntt_table);
  ...
  dyadicProductCoeffmod(encrypted_iter + 0 * d, encrypted_iter + 1 * d,
                        coeff_modulus_size, coeff_count, coeff_modulus,
                        temp + 1 * d);
  addPolyCoeffmod(temp + 1 * d, temp + 1 * d, coeff_modulus_size, coeff_count,
                  coeff_modulus, temp + 1 * d);
  ...
  inverseNttNegacyclicHarvey(encrypted.data(), dest_size, coeff_modulus_size,
                             ntt_table);

  // Set the correction factor
  encrypted.correctionFactor() =
      multiplyUintMod(encrypted.correctionFactor(),
                      encrypted.correctionFactor(), parms.plainModulus());
}

void Evaluator::relinearizeInternal(Ciphertext &encrypted,
  ...
  if (destination_size < 2 || destination_size > encrypted_size) {
    throw invalid_argument(
        "destination_size must be at least 2 and less than or equal to current "
        "count");
  }
  ...
  for (size_t i = 0; i < relins_needed; i++) {
    // std::cout << "encrypted_iter diff = " << std::dec << encrypted_iter -
    // encrypted.data() << std::endl;
    this->switchKeyInplace(encrypted, encrypted_iter,
...
void Evaluator::modSwitchScaleToNext(const Ciphertext &encrypted,
                                     Ciphertext &destination) const {
  // Assuming at this point encrypted is already validated.
  auto context_data_ptr = context_.getContextData(encrypted.parmsID());
  if (context_data_ptr->parms().scheme() == SchemeType::bfv &&
      encrypted.isNttForm()) {
    throw invalid_argument("BFV encrypted cannot be in NTT form");
  }
  if (context_data_ptr->parms().scheme() == SchemeType::ckks &&
      !encrypted.isNttForm()) {
    throw invalid_argument("CKKS encrypted must be in NTT form");
  }
  if (context_data_ptr->parms().scheme() == SchemeType::bgv &&
      encrypted.isNttForm()) {
  ...
  switch (next_parms.scheme()) {
    ...
    case SchemeType::ckks:
      for (size_t i = 0; i < encrypted_size; i++)
        rns_tool->divideAndRoundqLastNttInplace(encrypted_copy.data(i),
                                                context_data.smallNTTTables());
      break;
  ...
  for (size_t i = 0; i < encrypted_size; i++) {
    setPoly(encrypted_copy.data(i), coeff_count, next_coeff_modulus_size,
            destination.data(i));
  }
  ...
  if (next_parms.scheme() == SchemeType::ckks) {
    ...
    destination.scale() =
        encrypted.scale() /
        static_cast<double>(context_data.parms().coeffModulus().back().value());
  } else if (next_parms.scheme() == SchemeType::bgv) {
    // Change the correction factor when using BGV
    destination.correctionFactor() =
        multiplyUintMod(encrypted.correctionFactor(), rns_tool->invqLastModt(),
                        next_parms.plainModulus());
  }
}

void Evaluator::modSwitchDropToNext(const Ciphertext &encrypted,
                                    Ciphertext &destination) const {
  // Assuming at this point encrypted is already validated.
  auto context_data_ptr = context_.getContextData(encrypted.parmsID());
  if (context_data_ptr->parms().scheme() == SchemeType::ckks &&
  ...
  // Size check
  if (!productFitsIn(encrypted_size,
                     mul_safe(coeff_count, next_coeff_modulus_size))) {
    throw logic_error("invalid parameters");
  }
  ...
  if (&encrypted == &destination) {
    ...
    auto temp =
        allocatePolyArray(encrypted_size, coeff_count, next_coeff_modulus_size);

    // Copy data over to temp; only copy the RNS components relevant after
    // modulus drop
    for (size_t i = 0; i < encrypted_size; i++) {
      for (size_t j = 0; j < next_coeff_modulus_size; j++) {
        setUint(encrypted.data(i) + j * coeff_count, coeff_count,
                temp.get() + i * coeff_count * next_coeff_modulus_size +
                    j * coeff_count);
      }
    }
    ...
    setPolyArray(temp.get(), encrypted_size, coeff_count,
                 next_coeff_modulus_size, destination.data());
    // TODO: avoid copying and temporary space allocation
  } else {
    // Resize destination before writing
    destination.resize(context_, next_context_data.parmsID(), encrypted_size);

    // Copy data over to destination; only copy the RNS components relevant
    // after modulus drop
    for (size_t i = 0; i < encrypted_size; i++) {
      for (size_t j = 0; j < next_coeff_modulus_size; j++) {
        setUint(encrypted.data(i) + j * coeff_count, coeff_count,
...
void Evaluator::modSwitchToNext(const Ciphertext &encrypted,
                                Ciphertext &destination) const {
  // Verify parameters.
  if (!isMetadataValidFor(encrypted, context_) || !isBufferValid(encrypted)) {
    throw invalid_argument("encrypted is not valid for encryption parameters");
...
void Evaluator::modSwitchToInplace(Ciphertext &encrypted,
...
void Evaluator::rescaleToNext(const Ciphertext &encrypted,
                              Ciphertext &destination) const {
  // Verify parameters.
  if (!isMetadataValidFor(encrypted, context_) || !isBufferValid(encrypted)) {
    throw invalid_argument("encrypted is not valid for encryption parameters");
...
void Evaluator::rescaleToInplace(Ciphertext &encrypted,
                                 ParmsID parms_id) const {
  // Verify parameters.
  if (!isMetadataValidFor(encrypted, context_) || !isBufferValid(encrypted)) {
    throw invalid_argument("encrypted is not valid for encryption parameters");
...
void Evaluator::multiplyMany(const vector<Ciphertext> &encrypteds,
                             const RelinKeys &relin_keys,
                             Ciphertext &destination) const {
  // Verify parameters.
  ...
  // Repeatedly multiply and add to the back of the vector until the end is
  // reached
...
void Evaluator::exponentiateInplace(Ciphertext &encrypted, uint64_t exponent,
...
void Evaluator::addPlainInplace(Ciphertext &encrypted,
  ...
  switch (parms.scheme()) {
    case SchemeType::bfv: {
      multiplyAddPlainWithScalingVariant(plain, context_data,
                                         encrypted.data(0));
      break;
    }
    ...
    case SchemeType::ckks: {
      ...
      addPolyCoeffmod(encrypted_iter, plain_iter, coeff_modulus_size,
                      coeff_count, coeff_modulus.data(), encrypted_iter);
      break;
    }

    case SchemeType::bgv: {
      Plaintext plain_copy = plain;
      multiplyPolyScalarCoeffmod(plain.data(), plain.coeffCount(),
                                 encrypted.correctionFactor(),
                                 parms.plainModulus(), plain_copy.data());
      addPlainWithoutScalingVariant(plain_copy, context_data,
                                    encrypted.data(0));
      break;
    }
...
void Evaluator::subPlainInplace(Ciphertext &encrypted,
                                const Plaintext &plain) const {
  // Verify parameters.
  if (!isMetadataValidFor(encrypted, context_) || !isBufferValid(encrypted)) {
    throw invalid_argument("encrypted is not valid for encryption parameters");
  ...
  switch (parms.scheme()) {
    case SchemeType::bfv: {
      multiplySubPlainWithScalingVariant(plain, context_data,
                                         encrypted.data(0));
      break;
    }
    ...
    case SchemeType::ckks: {
      HostPointer encrypted_iter(encrypted.data());
      ConstHostPointer plain_iter(plain.data());
      subPolyCoeffmod(encrypted_iter, plain_iter, coeff_modulus_size,
                      coeff_count, coeff_modulus.data(), encrypted_iter);
      break;
    }
    ...
    case SchemeType::bgv: {
      ...
      multiplyPolyScalarCoeffmod(plain.data(), plain.coeffCount(),
                                 encrypted.correctionFactor(),
                                 parms.plainModulus(), plain_copy.data());
      subPlainWithoutScalingVariant(plain_copy, context_data,
                                    encrypted.data(0));
      break;
    }
...
void Evaluator::multiplyPlainInplace(Ciphertext &encrypted,
                                     const Plaintext &plain) const {
  // Verify parameters.
  if (!isMetadataValidFor(encrypted, context_) || !isBufferValid(encrypted)) {
    throw invalid_argument("encrypted is not valid for encryption parameters");
...
void Evaluator::multiplyPlainNormal(Ciphertext &encrypted,
                                    const Plaintext &plain) const {
  // Extract encryption parameters.
  auto &context_data = *context_.getContextData(encrypted.parmsID());
  auto &parms = context_data.parms();
  ...
  /*
  Optimizations for constant / monomial multiplication can lead to the presence
  of a timing side-channel in use-cases where the plaintext data should also be
  kept private.
  */
  if (plain_nonzero_coeff_count == 1) {
    ...
    if (plain[mono_exponent] >= plain_upper_half_threshold) {
      if (!context_data.qualifiers().using_fast_plain_lift) {
        ...
        // We need to adjust the monomial modulo each coeff_modulus prime
        // separately when the coeff_modulus primes may be larger than the
        // plain_modulus. We add plain_upper_half_increment (i.e., q-t) to the
        // monomial to ensure it is smaller than coeff_modulus and then do an
        // RNS multiplication. Note that in this case plain_upper_half_increment
        // contains a multi-precision integer, so after the addition we
        // decompose the multi-precision integer into RNS components, and then
        // multiply.
        addUint(plain_upper_half_increment, coeff_modulus_size,
                plain[mono_exponent], temp.get());
        context_data.rnsTool()->baseq()->decompose(temp.get());
        negacyclicMultiplyPolyMonoCoeffmod(
            encrypted.data(), encrypted_size, coeff_modulus_size, coeff_count,
            temp.asPointer(), mono_exponent, coeff_modulus.data(),
            encrypted.data());
      } else {
        // Every coeff_modulus prime is larger than plain_modulus, so there is
        // no need to adjust the monomial. Instead, just do an RNS
        // multiplication.
        negacyclicMultiplyPolyMonoCoeffmod(
            encrypted.data(), encrypted_size, coeff_modulus_size, coeff_count,
            plain[mono_exponent], mono_exponent, coeff_modulus.data(),
            encrypted.data());
      }
    } else {
      // The monomial represents a positive number, so no RNS multiplication is
      // needed.
      negacyclicMultiplyPolyMonoCoeffmod(
          encrypted.data(), encrypted_size, coeff_modulus_size, coeff_count,
          plain[mono_exponent], mono_exponent, coeff_modulus.data(),
  ...
  if (!context_data.qualifiers().using_fast_plain_lift) {
    ...
    for (size_t i = 0; i < plain_coeff_count; i++) {
      // SEAL_ITERATE(iter(plain.data(), temp_iter), plain_coeff_count, [&](auto
      // I) {
      auto plain_value = plain.data()[i];
      if (plain_value >= plain_upper_half_threshold) {
        addUint(plain_upper_half_increment, coeff_modulus_size, plain_value,
                temp.get() + coeff_modulus_size * i);
      } else {
    ...
    // Note that in this case plain_upper_half_increment holds its value in RNS
    // form modulo the coeff_modulus primes.
    for (size_t i = 0; i < coeff_modulus_size; i++) {
      // SEAL_ITERATE(iter(temp_iter, plain_upper_half_increment),
      // coeff_modulus_size, [&](auto I) {
      for (size_t j = 0; j < plain_coeff_count; j++) {
        // SEAL_ITERATE(iter(get<0>(I), plain.data()), plain_coeff_count,
        // [&](auto J) {
        temp[i * coeff_count + j] =
            plain.data()[j] >= plain_upper_half_threshold
                ? plain.data()[j] + plain_upper_half_increment[i]
                : plain.data()[j];
      }
    }
  }
  ...
  // transform to NTT form RNSIter temp_iter(temp.get(), coeff_count);
  nttNegacyclicHarvey(temp.asPointer(), coeff_modulus_size, ntt_tables);

  for (size_t i = 0; i < encrypted_size; i++) {
    // SEAL_ITERATE(iter(encrypted), encrypted_size, [&](auto I) {
    for (size_t j = 0; j < coeff_modulus_size; j++) {
      // SEAL_ITERATE(iter(I, temp_iter, coeff_modulus, ntt_tables),
      // coeff_modulus_size, [&](auto J) { Lazy reduction
      auto target_ptr = encrypted.data(i) + j * coeff_count;
      nttNegacyclicHarveyLazy(target_ptr, ntt_tables[j]);
      dyadicProductCoeffmod(target_ptr, temp + j * coeff_count, coeff_count,
                            coeff_modulus[j], target_ptr);
...
void Evaluator::multiplyPlainNtt(Ciphertext &encrypted_ntt,
                                 const Plaintext &plain_ntt) const {
  // Verify parameters.
  ...
  // Size check
  if (!productFitsIn(encrypted_ntt_size,
                     mul_safe(coeff_count, coeff_modulus_size))) {
    throw logic_error("invalid parameters");
  }
  ...
  for (size_t i = 0; i < encrypted_ntt_size; i++) {
    ...
    dyadicProductCoeffmod(encrypted_ntt.data(i), plain_ntt_iter,
                          coeff_modulus_size, coeff_count, coeff_modulus.data(),
                          encrypted_ntt.data(i));
  }
...
void Evaluator::transformToNttInplace(Plaintext &plain,
                                      ParmsID parms_id) const {
  // Verify parameters.
  ...
  if (!context_data.qualifiers().using_fast_plain_lift) {
    ...
    // SEAL_ALLOCATE_ZERO_GET_RNS_ITER(temp, coeff_modulus_size, coeff_count,
    // pool);
    ...
    // Create a "reversed" helper iterator that iterates in the reverse order
    // both plain RNS components and the plain_upper_half_increment values. auto
    // helper_iter = reverse_iter(plain_iter, plain_upper_half_increment);
    // advance(helper_iter, -safe_cast<ptrdiff_t>(coeff_modulus_size - 1));

    // SEAL_ITERATE(helper_iter, coeff_modulus_size, [&](auto I) {
    //     SEAL_ITERATE(iter(*plain_iter, get<0>(I)), plain_coeff_count,
    //     [&](auto J) {
    //         get<1>(J) =
    //             SEAL_COND_SELECT(get<0>(J) >= plain_upper_half_threshold,
    ...
    for (size_t i = 0; i < coeff_modulus_size; i++) {
      for (size_t j = 0; j < plain_coeff_count; j++) {
        size_t plain_index =
            (coeff_modulus_size - 1 - i) * coeff_count + j;  // get<1>(J)
        size_t increment_index = (coeff_modulus_size - 1) - i;
        plain_iter[plain_index] =
            (plain_iter[j] >= plain_upper_half_threshold)
                ? (plain_iter[j] + plain_upper_half_increment[increment_index])
                : plain_iter[j];
      }
    }
  }
...
void Evaluator::transformFromNttInplace(Ciphertext &encrypted_ntt) const {
  // Verify parameters.
  if (!isMetadataValidFor(encrypted_ntt, context_) ||
      !isBufferValid(encrypted_ntt)) {
    throw invalid_argument("encrypted is not valid for encryption parameters");
  }
  ...
  if (!context_data_ptr) {
    throw invalid_argument(
        "encrypted_ntt is not valid for encryption parameters");
  }
  ...
  inverseNttNegacyclicHarvey(encrypted_ntt.data(), encrypted_ntt_size,
...
void Evaluator::applyGaloisInplace(Ciphertext &encrypted, uint32_t galois_elt,
  ...
  if (parms.scheme() == SchemeType::bfv || parms.scheme() == SchemeType::bgv) {
    ...
    galois_tool->applyGalois(encrypted.data(0), coeff_modulus_size, galois_elt,
                             coeff_modulus.data(), temp.asPointer());
    ...
    galois_tool->applyGalois(encrypted.data(1), coeff_modulus_size, galois_elt,
                             coeff_modulus.data(), temp.asPointer());
    ...
    // First transform encrypted.data(0)
    // auto encrypted_iter = iter(encrypted);
    galois_tool->applyGaloisNtt(encrypted.data(0), coeff_modulus_size,
                                galois_elt, temp.asPointer());

    // Copy result to encrypted.data(0)
    setPoly(temp.get(), coeff_count, coeff_modulus_size, encrypted.data(0));

    // Next transform encrypted.data(1)
    galois_tool->applyGaloisNtt(encrypted.data(1), coeff_modulus_size,
                                galois_elt, temp.asPointer());
  } else {
  ...
  switchKeyInplace(encrypted, temp.get(),
                   static_cast<const KSwitchKeys &>(galois_keys),
                   GaloisKeys::getIndex(galois_elt));
}

void Evaluator::rotateInternal(Ciphertext &encrypted, int steps,
                               const GaloisKeys &galois_keys) const {
  auto context_data_ptr = context_.getContextData(encrypted.parmsID());
  if (!context_data_ptr) {
    throw invalid_argument("encrypted is not valid for encryption parameters");
  ...
  if (galois_keys.hasKey(galois_tool->getEltFromStep(steps))) {
    ...
    applyGaloisInplace(encrypted, galois_tool->getEltFromStep(steps),
                       galois_keys);
  } else {
...
void Evaluator::switchKeyInplace(Ciphertext &encrypted,
                                 ConstHostPointer<uint64_t> target_iter,
                                 const KSwitchKeys &kswitch_keys,
                                 size_t kswitch_keys_index) const {
  ...
  // SEAL_ALLOCATE_GET_RNS_ITER(t_target, coeff_count, decomp_modulus_size,
  // pool);
  ...
  if (scheme == SchemeType::ckks) {
    inverseNttNegacyclicHarvey(t_target.asPointer(), decomp_modulus_size,
                               key_ntt_tables);
  }

  // Temporary result
  auto t_poly_prod =
      allocateZeroPolyArray(key_component_count, coeff_count, rns_modulus_size);
  ...
  for (size_t i = 0; i < rns_modulus_size; i++) {
    ...
    auto t_poly_lazy =
        allocateZeroPolyArray(key_component_count, coeff_count, 2);
    ...
    for (size_t j = 0; j < decomp_modulus_size; j++) {
      ...
      // Perform RNS-NTT conversion
        ...
        // Perform RNS conversion (modular reduction)
          ...
          // printf("branch b - ");
          moduloPolyCoeffs(t_target.get() + j * coeff_count, coeff_count,
                           key_modulus[key_index], t_ntt.get());
        }
      ...
      for (size_t k = 0; k < key_component_count; k++) {
        // SEAL_ITERATE(iter(key_vector[J].data(), accumulator_iter),
        // key_component_count, [&](auto K) {
        ...
        if (!lazy_reduction_counter) {
          for (size_t l = 0; l < coeff_count; l++) {
            ...
            multiplyUint64(
                t_operand[l],
                key_vector[j].data().data(k)[key_index * coeff_count + l],
                qword);

            // Accumulate product of t_operand and t_key_acc to t_poly_lazy and
            // reduce
    ...
    for (size_t k = 0; k < key_component_count; k++) {
      // SEAL_ITERATE(iter(accumulator_iter, t_poly_prod_iter),
      // key_component_count, [&](auto K) {
      HostPointer<uint64_t> accumulator = t_poly_lazy + k * poly_coeff_count;
      if (lazy_reduction_counter == lazy_reduction_summand_bound) {
        for (size_t l = 0; l < coeff_count; l++) {
          // std::cout << "*get<0>(L): " << accumulator[l * 2] << std::endl;
          t_poly_prod_iter[k * coeff_count * rns_modulus_size + l] =
              static_cast<uint64_t>(accumulator[l * 2]);
        }
      } else {
        ...
        for (size_t l = 0; l < coeff_count; l++) {
          // std::cout << "*get<0>(L): " << accumulator[l * 2] << std::endl;
          t_poly_prod_iter[k * coeff_count * rns_modulus_size + l] =
              barrettReduce128(accumulator.get() + l * 2,
  ...
  // PolyIter t_poly_prod_iter(t_poly_prod.get(), coeff_count,
  // rns_modulus_size);
  for (size_t i = 0; i < key_component_count; i++) {
    // SEAL_ITERATE(iter(encrypted, t_poly_prod_iter), key_component_count,
    // [&](auto I) {
    if (scheme == SchemeType::bgv) {
      ...
      auto t_last = t_poly_prod + coeff_count * rns_modulus_size * i +
      ...
      if (qk_inv_qp != 1) {
        multiplyPolyScalarCoeffmod(k.get(), coeff_count, qk_inv_qp,
                                   plain_modulus, k.get());
      }
      ...
      for (size_t j = 0; j < decomp_modulus_size; j++) {
        // SEAL_ITERATE(iter(I, key_modulus, modswitch_factors, key_ntt_tables),
        // decomp_modulus_size, [&](auto J) {
        size_t t_poly_prod_index =
            i * coeff_count * rns_modulus_size + j * coeff_count;
        inverseNttNegacyclicHarvey(t_poly_prod + t_poly_prod_index,
                                   key_ntt_tables[j]);
        // delta = k mod q_i
        moduloPolyCoeffs(k.get(), coeff_count, key_modulus[j], delta.get());
        // delta = k * q_k mod q_i
        multiplyPolyScalarCoeffmod(delta.get(), coeff_count, qk, key_modulus[j],
                                   delta.get());

        // c mod q_i
        moduloPolyCoeffs(t_last.get(), coeff_count, key_modulus[j],
                         c_mod_qi.get());
        // delta = c + k * q_k mod q_i
        // c_{i} = c_{i} - delta mod q_i
        const uint64_t Lqi = key_modulus[j].value() * 2;
        for (size_t k = 0; k < coeff_count; k++) {
          t_poly_prod[t_poly_prod_index + k] =
              t_poly_prod[t_poly_prod_index + k] + Lqi -
              (delta[k] + c_mod_qi[k]);
        }
        // SEAL_ITERATE(iter(delta, c_mod_qi, get<0, 1>(J)), coeff_count,
        // [Lqi](auto K) {
        ...
        multiplyPolyScalarCoeffmod(t_poly_prod + t_poly_prod_index, coeff_count,
                                   ...
                                   t_poly_prod + t_poly_prod_index);

        addPolyCoeffmod(t_poly_prod + t_poly_prod_index,
                        encrypted.data(i) + j * coeff_count, coeff_count,
                        key_modulus[j], encrypted.data(i) + j * coeff_count);
      }
    } else {
      // Lazy reduction; this needs to be then reduced mod qi
      auto t_last = t_poly_prod + coeff_count * rns_modulus_size * i +
                    decomp_modulus_size * coeff_count;
      // std::cout << "t_last diff: " << coeff_count * rns_modulus_size * i +
      // decomp_modulus_size * coeff_count << std::endl;

      // std::cout << "t_last: "; printArray(t_last.get(), coeff_count);
      inverseNttNegacyclicHarveyLazy(t_last,
                                     key_ntt_tables[key_modulus_size - 1]);
      ...
      for (size_t j = 0; j < coeff_count; j++) {
        ...
        t_last[j] = barrettReduce64(t_last[j] + qk_half,
                                    key_modulus[key_modulus_size - 1]);
      }

      // std::cout << "t_last: "; printArray(t_last.get(), coeff_count);

      for (size_t j = 0; j < decomp_modulus_size; j++) {
        // SEAL_ITERATE(iter(I, key_modulus, key_ntt_tables, modswitch_factors),
        // decomp_modulus_size, [&](auto J) { std::cout << "  ski j = " << j <<
        // std::endl;
        size_t t_poly_prod_index =
            i * coeff_count * rns_modulus_size + j * coeff_count;
        ...
        if (qk > qi) {
          // This cannot be spared. NTT only tolerates input that is less than
          // 4*modulus (i.e. qk <=4*qi).
          moduloPolyCoeffs(t_last.get(), coeff_count, key_modulus[j],
                           t_ntt.get());
        } else {
        ...
        // std::cout << "  t_ptr: " << j << " - ";
        // printArray(t_poly_prod_ptr.get(), coeff_count);

        // ((ct mod qi) - (ct mod qk)) mod qi with output in [0, 2 * qi_lazy)
        for (size_t k = 0; k < coeff_count; k++)
          t_poly_prod_ptr[k] += qi_lazy - t_ntt[k];
        // SEAL_ITERATE(
        //     iter(get<0, 1>(J), t_ntt), coeff_count, [&](auto K) { get<0>(K)
        //     += qi_lazy - get<1>(K); });

        // qk^(-1) * ((ct mod qi) - (ct mod qk)) mod qi
        multiplyPolyScalarCoeffmod(t_poly_prod_ptr, coeff_count,
                                   modswitch_factors[j], key_modulus[j],
                                   t_poly_prod_ptr);
        addPolyCoeffmod(t_poly_prod_ptr, encrypted.data(i) + j * coeff_count,
                        coeff_count, key_modulus[j],
                        encrypted.data(i) + j * coeff_count);
      }
    }

    // printf("enc %ld: ", i); printArray(encrypted.data(i), key_component_count
    // * coeff_count);
  }
}
}  // namespace seal_gpu
