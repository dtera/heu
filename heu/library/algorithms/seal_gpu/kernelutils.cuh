...
class DeviceHelper {
 public:
  ...
  const __device__ inline static uint64_t *getModulusConstRatio(
      const Modulus &m) {
...
using seal_gpu::util::ConstDevicePointer;
using seal_gpu::util::DevicePointer;
using CPointer = ConstDevicePointer<uint64_t>;
using Pointer = DevicePointer<uint64_t>;
using MPointer = ConstDevicePointer<Modulus>;
using seal_gpu::util::MultiplyUIntModOperand;
using uint128_t = unsigned __int128;
using seal_gpu::util::NTTTablesCuda;
...
__device__ inline unsigned char dAddUint64(uint64_t operand1, uint64_t operand2,
                                           unsigned char carry,
                                           uint64_t *result) {
  operand1 += operand2;
...
__device__ inline unsigned char dAddUint128(uint64_t *operand1,
                                            uint64_t *operand2,
                                            uint64_t *result) {
...
__device__ inline void dMultiplyUint64HW64(uint64_t operand1, uint64_t operand2,
                                           uint64_t *hw64) {
  *hw64 = static_cast<uint64_t>(
      ((static_cast<uint128_t>(operand1) * static_cast<uint128_t>(operand2)) >>
       64));
}

__device__ inline uint64_t dBarrettReduce64(uint64_t input,
                                            const Modulus &modulus) {
  uint64_t tmp[2];
  const std::uint64_t *const_ratio =
      DeviceHelper::getModulusConstRatio(modulus);
...
__device__ inline void dMultiplyUint64(uint64_t operand1, uint64_t operand2,
                                       uint64_t *result128) {
...
__device__ inline uint64_t dMultiplyUintMod(uint64_t x,
                                            const MultiplyUIntModOperand &y,
                                            const Modulus &modulus) {
  uint64_t tmp1, tmp2;
...
__device__ inline void dDivideUint128Inplace(uint64_t *numerator,
                                             uint64_t denominator,
                                             uint64_t *quotient) {
  uint128_t n, q;
  n = (static_cast<uint128_t>(numerator[1]) << 64) |
      (static_cast<uint128_t>(numerator[0]));
  q = n / denominator;
  n -= q * denominator;
...
__device__ inline unsigned char dSubUint64(uint64_t operand1, uint64_t operand2,
                                           uint64_t *result) {
  *result = operand1 - operand2;
...
__device__ inline uint64_t dBarrettReduce128(const uint64_t *input,
                                             const Modulus &modulus) {
...
__device__ inline std::uint64_t dModuloUint(const std::uint64_t *value,
...
__device__ inline void dMultiplyUint(const uint64_t *operand1,
                                     size_t operand1_uint64_count,
                                     uint64_t operand2,
                                     size_t result_uint64_count,
                                     uint64_t *result) {
  // Handle fast cases.
  ...
  for (size_t operand1_index = 0; operand1_index < operand1_index_max;
       operand1_index++) {
...
__device__ inline unsigned char dAddUint(const uint64_t *operand1,
                                         const uint64_t *operand2,
                                         std::size_t uint64Count,
                                         uint64_t *result) {
...
__device__ inline int dCompareUint(const uint64_t *operand1,
                                   const uint64_t *operand2,
...
__device__ inline bool dIsGreaterThanOrEqualUint(const uint64_t *operand1,
                                                 const uint64_t *operand2,
                                                 std::size_t uint64_count) {
  return dCompareUint(operand1, operand2, uint64_count) >= 0;
}

__device__ inline unsigned char dSubUint(const uint64_t *operand1,
                                         const uint64_t *operand2,
                                         std::size_t uint64Count,
                                         uint64_t *result) {
  // Unroll first iteration of loop. We assume uint64_count > 0.
...
__device__ inline void dAddUintUintMod(const std::uint64_t *operand1,
...
__device__ inline uint64_t dDotProductMod(const uint64_t *operand1,
                                          const uint64_t *operand2,
                                          size_t count,
                                          const Modulus &modulus) {
  static_assert(SEAL_MULTIPLY_ACCUMULATE_MOD_MAX >= 16,
                "SEAL_MULTIPLY_ACCUMULATE_MOD_MAX");
...
__device__ inline uint64_t dSubUintMod(std::uint64_t operand1,
                                       std::uint64_t operand2,
                                       const Modulus &modulus) {
  unsigned long long temp;
  int64_t borrow =
      static_cast<std::int64_t>(dSubUint64(operand1, operand2, 0, &temp));
  return static_cast<std::uint64_t>(temp) +
         (DeviceHelper::getModulusValue(modulus) &
          static_cast<std::uint64_t>(-borrow));
}

__device__ inline std::uint64_t dMultiplyUintMod(std::uint64_t operand1,
                                                 std::uint64_t operand2,
                                                 const Modulus &modulus) {
  uint64_t z[2];
...
__device__ inline std::uint64_t dAddUintMod(std::uint64_t operand1,
                                            std::uint64_t operand2,
                                            const Modulus &modulus) {
...
__device__ inline MultiplyUIntModOperand dSetMultiplyModOperand(
    uint64_t operand, const Modulus &modulus) {
  MultiplyUIntModOperand ret;
  ret.operand = operand;
  std::uint64_t wide_quotient[2]{0, 0};
  std::uint64_t wide_coeff[2]{0, operand};
  dDivideUint128Inplace(wide_coeff, DeviceHelper::getModulusValue(modulus),
                        wide_quotient);
...
__device__ inline std::uint64_t dMultiplyAddUintMod(
    std::uint64_t operand1, const MultiplyUIntModOperand &operand2,
    std::uint64_t operand3, const Modulus &modulus) {
  return dAddUintMod(dMultiplyUintMod(operand1, operand2, modulus),
                     dBarrettReduce64(operand3, modulus), modulus);
...
__device__ inline std::uint64_t dNegateUintMod(std::uint64_t operand,
                                               const Modulus &modulus) {
  std::int64_t non_zero = static_cast<std::int64_t>(operand != 0);
  return (DeviceHelper::getModulusValue(modulus) - operand) &
         static_cast<std::uint64_t>(-non_zero);
}

__device__ inline uint64_t dMultiplyScalarMod(uint64_t operand,
                                              std::uint64_t scalar,
                                              const Modulus &modulus) {
...
__global__ void gAddPolyCoeffmod(const uint64_t *operand1,
                                 const uint64_t *operand2, POLY_ARRAY_ARGUMENTS,
                                 const Modulus *modulus, uint64_t *result);

void kAddPolyCoeffmod(CPointer operand1, CPointer operand2,
                      POLY_ARRAY_ARGUMENTS, MPointer modulus, Pointer result);

__global__ void gDyadicConvolutionCoeffmod(
    const uint64_t *operand1, const uint64_t *operand2_reversed,
    POLY_ARRAY_ARGUMENTS, const Modulus *moduli,
    uint64_t *single_poly_result_accumulator);

void kDyadicConvolutionCoeffmod(CPointer operand1, CPointer operand2_reversed,
                                POLY_ARRAY_ARGUMENTS, MPointer moduli,
                                Pointer single_poly_result_accumulator);

__global__ void gDyadicProductCoeffmod(const uint64_t *operand1,
                                       const uint64_t *operand2,
                                       POLY_ARRAY_ARGUMENTS,
                                       const Modulus *moduli, uint64_t *output);

void kDyadicProductCoeffmod(CPointer operand1, CPointer operand2,
                            POLY_ARRAY_ARGUMENTS, MPointer moduli,
                            Pointer output);

__global__ void gDyadicSquareCoeffmod(const uint64_t *operand,
                                      size_t coeff_modulus_size,
                                      size_t poly_modulus_degree,
                                      const Modulus *moduli, uint64_t *output);

void kDyadicSquareCoeffmod(CPointer operand, size_t coeff_modulus_size,
                           size_t poly_modulus_degree, MPointer moduli,
                           Pointer output);

inline void kDyadicSquareCoeffmod(Pointer operand, size_t coeff_modulus_size,
                                  size_t poly_modulus_degree, MPointer moduli) {
  kDyadicSquareCoeffmod(operand, coeff_modulus_size, poly_modulus_degree,
                        moduli, operand);
}

__global__ void gModBoundedUsingNttTables(uint64_t *operand,
                                          POLY_ARRAY_ARGUMENTS,
                                          const NTTTablesCuda *ntt_tables);

void kModBoundedUsingNttTables(Pointer operand, POLY_ARRAY_ARGUMENTS,
                               ConstDevicePointer<NTTTablesCuda> ntt_tables);

__global__ void gModuloPolyCoeffs(const uint64_t *operand, POLY_ARRAY_ARGUMENTS,
                                  const Modulus *moduli, uint64_t *result);

void kModuloPolyCoeffs(CPointer operand, POLY_ARRAY_ARGUMENTS, MPointer moduli,
                       Pointer result);

__global__ void gMultiplyPolyScalarCoeffmod(
    const uint64_t *poly_array, POLY_ARRAY_ARGUMENTS,
    const MultiplyUIntModOperand *reduced_scalar, const Modulus *modulus,
    uint64_t *result);

void kMultiplyPolyScalarCoeffmod(CPointer poly_array, POLY_ARRAY_ARGUMENTS,
                                 uint64_t scalar, MPointer modulus,
                                 Pointer result);

__global__ void gNegatePolyCoeffmod(const uint64_t *poly_array,
                                    POLY_ARRAY_ARGUMENTS,
                                    const Modulus *modulus, uint64_t *result);

void kNegatePolyCoeffmod(CPointer poly_array, POLY_ARRAY_ARGUMENTS,
                         MPointer modulus, Pointer result);

void kNttTransferToRev(Pointer operand, size_t poly_size,
                       size_t coeff_modulus_size,
                       size_t poly_modulus_degree_power,
                       ConstDevicePointer<NTTTablesCuda> ntt_tables,
                       bool use_inv_root_powers);

void kNttTransferFromRev(Pointer operand, size_t poly_size,
                         size_t coeff_modulus_size,
                         size_t poly_modulus_degree_power,
                         ConstDevicePointer<NTTTablesCuda> ntt_tables,
                         bool use_inv_root_powers);
...
inline void kNttNegacyclicHarveyLazy(
    Pointer operand, size_t poly_size, size_t coeff_modulus_size,
    size_t poly_modulus_degree_power,
    ConstDevicePointer<NTTTablesCuda> ntt_tables) {
...
inline void kNttNegacyclicHarvey(Pointer operand, size_t poly_size,
                                 size_t coeff_modulus_size,
                                 size_t poly_modulus_degree_power,
                                 ConstDevicePointer<NTTTablesCuda> ntt_tables) {
  kNttNegacyclicHarveyLazy(operand, poly_size, coeff_modulus_size,
                           poly_modulus_degree_power, ntt_tables);
  kModBoundedUsingNttTables(operand, poly_size, coeff_modulus_size,
...
inline void kInverseNttNegacyclicHarvey(
    ...
    size_t poly_modulus_degree_power,
    ConstDevicePointer<NTTTablesCuda> ntt_tables) {
  kInverseNttNegacyclicHarveyLazy(operand, poly_size, coeff_modulus_size,
                                  poly_modulus_degree_power, ntt_tables);
  kModBoundedUsingNttTables(operand, poly_size, coeff_modulus_size,
                            1 << poly_modulus_degree_power, ntt_tables);
}

__global__ void gNttTransferToRevLayered(size_t layer, uint64_t *operand,
                                         size_t poly_size,
                                         size_t coeff_modulus_size,
                                         size_t poly_modulus_degree_power,
                                         const NTTTablesCuda *ntt_tables,
...
void kNttTransferToRevLayered(size_t layer, Pointer operand, size_t poly_size,
                              size_t coeff_modulus_size,
                              size_t poly_modulus_degree_power,
                              ConstDevicePointer<NTTTablesCuda> ntt_tables,
                              bool use_inv_root_powers);

__global__ void gNttTransferFromRevLayered(size_t layer, uint64_t *operand,
                                           size_t poly_size,
                                           size_t coeff_modulus_size,
                                           size_t poly_modulus_degree_power,
                                           const NTTTablesCuda *ntt_tables,
...
void kNttTransferFromRevLayered(size_t layer, Pointer operand, size_t poly_size,
                                size_t coeff_modulus_size,
                                size_t poly_modulus_degree_power,
                                ConstDevicePointer<NTTTablesCuda> ntt_tables,
                                bool use_inv_root_powers);

__global__ void gSetMultiplyUIntModOperand(uint64_t scalar,
                                           const Modulus *moduli, size_t n,
                                           MultiplyUIntModOperand *result);

inline void kSetPolyArray(CPointer poly, POLY_ARRAY_ARGUMENTS, Pointer result) {
...
inline void kSetZeroPolyArray(POLY_ARRAY_ARGUMENTS, Pointer result) {
  KernelProvider::memsetZero(
      result.get(), poly_size * poly_modulus_degree * coeff_modulus_size);
}

__global__ void gSubPolyCoeffmod(const uint64_t *operand1,
                                 const uint64_t *operand2, POLY_ARRAY_ARGUMENTS,
                                 const Modulus *modulus, uint64_t *result);

void kSubPolyCoeffmod(CPointer operand1, CPointer operand2,
                      POLY_ARRAY_ARGUMENTS, MPointer modulus, Pointer result);

void kNegacyclicShiftPolyCoeffmod(CPointer poly, POLY_ARRAY_ARGUMENTS,
                                  size_t shift, MPointer modulus,
                                  Pointer result);

void kMultiplyInvPolyDegreeCoeffmod(
    CPointer poly_array, POLY_ARRAY_ARGUMENTS,
    ConstDevicePointer<NTTTablesCuda> ntt_tables, MPointer modulus,
    Pointer result);
