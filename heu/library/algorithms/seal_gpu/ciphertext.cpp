...
Ciphertext &Ciphertext::operator=(const Ciphertext &assign) {
  ...
  // Then resize
  resizeInternal(assign.size_, assign.poly_modulus_degree_,
                 assign.coeff_modulus_size_);
...
void Ciphertext::reserve(const SEALContext &context, ParmsID parms_id,
  ...
  reserveInternal(size_capacity, parms.polyModulusDegree(),
                  parms.coeffModulus().size());
}

void Ciphertext::reserveInternal(size_t size_capacity,
  ...
  size_t new_data_capacity = mul_safe(
      size_capacity, mul_safe(poly_modulus_degree, coeff_modulus_size));
...
void Ciphertext::resize(const SEALContext &context, ParmsID parms_id,
                        size_t size) {
  // Verify parameters
  if (!context.parametersSet()) {
    throw invalid_argument("encryption parameters are not set correctly");
...
void Ciphertext::resizeInternal(size_t size, size_t poly_modulus_degree,
                                size_t coeff_modulus_size) {
  ...
  size_t new_data_size =
      mul_safe(size, mul_safe(poly_modulus_degree, coeff_modulus_size));
  data_.resize(new_data_size);
...
void Ciphertext::expandSeed(const SEALContext &context,
                            const UniformRandomGeneratorInfo &prng_info) {
...
//             Pointer<ct_coeff_type>::Aliasing(const_cast<ct_coeff_type
//             *>(data_.cbegin())), data_.size() / 2, false, data_.pool());

//         data_size = add_safe(
//             safe_cast<size_t>(alias_data.save_size(compr_mode_type::none)),
//             // data_(0)
...
//     {
//         data_size =
//         safe_cast<size_t>(data_.save_size(compr_mode_type::none)); // data_
//     }
...
//     return safe_cast<streamoff>(add_safe(sizeof(Serialization::SEALHeader),
//     members_size));
// }
...
//     try
//     {
//         // Throw exceptions on std::ios_base::badbit and
//         std::ios_base::failbit stream.exceptions(ios_base::badbit |
//         ios_base::failbit);

//         stream.write(reinterpret_cast<const char *>(&parms_id_),
//         sizeof(ParmsID)); seal_byte is_ntt_form_byte =
//         static_cast<seal_byte>(is_ntt_form_);
//         stream.write(reinterpret_cast<const char *>(&is_ntt_form_byte),
//         sizeof(seal_byte)); uint64_t size64 = safe_cast<uint64_t>(size_);
//         stream.write(reinterpret_cast<const char *>(&size64),
//         sizeof(uint64_t)); uint64_t poly_modulus_degree64 =
//         safe_cast<uint64_t>(poly_modulus_degree_);
//         stream.write(reinterpret_cast<const char *>(&poly_modulus_degree64),
//         sizeof(uint64_t)); uint64_t coeff_modulus_size64 =
//         safe_cast<uint64_t>(coeff_modulus_size_);
//         stream.write(reinterpret_cast<const char *>(&coeff_modulus_size64),
//         sizeof(uint64_t)); stream.write(reinterpret_cast<const char
//         *>(&scale_), sizeof(double)); stream.write(reinterpret_cast<const
//         char *>(&correction_factor_), sizeof(uint64_t));
...
//             size_t info_size =
//             static_cast<size_t>(UniformRandomGeneratorInfo::SaveSize(compr_mode_type::none));
//             info.load(reinterpret_cast<const seal_byte *>(data(1) + 1),
//             info_size);
...
//             auto alias_ptr =
//             util::Pointer<ct_coeff_type>::Aliasing(const_cast<ct_coeff_type
//             *>(data_.cbegin())); swap(alias_data.data_, alias_ptr);
...
// SEAL_MAYBE_UNUSED SEALVersion version)
// {
//     // Verify parameters
//     if (!context.parametersSet())
//     {
//         throw invalid_argument("encryption parameters are not set
//         correctly");
//     }
...
//         stream.read(reinterpret_cast<char *>(&is_ntt_form_byte),
//         sizeof(seal_byte)); uint64_t size64 = 0;
...
//         sizeof(uint64_t)); uint64_t coeff_modulus_size64 = 0;
//         stream.read(reinterpret_cast<char *>(&coeff_modulus_size64),
//         sizeof(uint64_t)); double scale = 0;
//         stream.read(reinterpret_cast<char *>(&scale), sizeof(double));
//         uint64_t correction_factor = 1;
//         if (version.major == 4)
//         {
//             stream.read(reinterpret_cast<char *>(&correction_factor),
//             sizeof(uint64_t));
//         }

//         // Set values already at this point for the metadata validity check
//         new_data.parms_id_ = parms_id;
//         new_data.is_ntt_form_ = (is_ntt_form_byte == seal_byte{}) ? false :
//         true; new_data.size_ = safe_cast<size_t>(size64);
//         new_data.poly_modulus_degree_ =
//         safe_cast<size_t>(poly_modulus_degree64);
//         new_data.coeff_modulus_size_ =
//         safe_cast<size_t>(coeff_modulus_size64); new_data.scale_ = scale;
...
//         // further means that functions reading in Ciphertext objects must
//         check
...
//         // Note that this must be done after the metadata is checked for
//         validity. auto total_uint64_count =
//             mul_safe(new_data.size_, new_data.poly_modulus_degree_,
//             new_data.coeff_modulus_size_);

//         // Reserve memory for the entire (expected) ciphertext data
//         new_data.data_.reserve(total_uint64_count);

//         // Load the data. Note that we are supplying also the expected
//         maximum
//         // size of the loaded DynArray. This is an important security measure
//         to
...
//         auto seeded_uint64_count = poly_modulus_degree64 *
//         coeff_modulus_size64;
...
//             {
//                 // We only need to load the hash value; only Blake2xb is
//                 supported prng_info.type() = prng_type::blake2xb;
//                 stream.read(reinterpret_cast<char *>(&prng_info.seed()),
