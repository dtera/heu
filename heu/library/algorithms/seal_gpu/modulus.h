...
class Modulus {
  ...
  */
  inline int bitCount() const noexcept { return bit_count_; }

  /**
  Returns the size (in 64-bit words) of the value of the current Modulus.
  */
  inline std::size_t uint64Count() const noexcept { return uint64_count_; }

  /**
  Returns a const pointer to the value of the current Modulus.
  */
  inline const uint64_t *data() const noexcept { return &value_; }

  /**
  Returns the value of the current Modulus.
  */
  inline std::uint64_t value() const noexcept { return value_; }
  ...
  */
  inline auto &constRatio() const noexcept { return const_ratio_; }
  ...
  */
  inline bool isZero() const noexcept { return value_ == 0; }

  /**
  Returns whether the value of the current Modulus is a prime number.
  */
  inline bool isPrime() const noexcept { return is_prime_; }
  ...
  */
  ...
  //     Serialization::ComprSizeEstimate(util::add_safe(sizeof(value_)),
  //     compr_mode);
  ...
  //         std::bind(&Modulus::save_members, this, _1),
  ...
  // {
  //     using namespace std::placeholders;
  //     return Serialization::Load(std::bind(&Modulus::load_members, this, _1,
  //     _2), stream, false);
  // }
...
class CoeffModulus {
  ...
  */
  static constexpr int MaxBitCount(
      ...
      SecurityLevel sec_level = SecurityLevel::tc128) noexcept {
    switch (sec_level) {
  ...
  */
  ...
  @param[in] poly_modulus_degree The value of the poly_modulus_degree encryption
  parameter
  ...
  @throws std::invalid_argument if bit_sizes is too large or if its elements
  are out of bounds
  @throws std::logic_error if LCM(2*poly_modulus_degree, plain_modulus) is more
  than 64-bit
  @throws std::logic_error if not enough suitable primes could be found
  */
  static std::vector<Modulus> Create(std::size_t poly_modulus_degree,
...
class PlainModulus {
  ...
  */
  static inline Modulus Batching(std::size_t poly_modulus_degree,
                                 int bit_size) {
  ...
  @throws std::logic_error if not enough suitable primes could be found
  */
  static inline std::vector<Modulus> Batching(std::size_t poly_modulus_degree,
                                              std::vector<int> bit_sizes) {
