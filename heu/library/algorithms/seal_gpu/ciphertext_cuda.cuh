...
class CiphertextCuda {
 public:
  ...
  inline uint64_t &correctionFactor() noexcept { return correction_factor_; }
  ...
  inline util::ConstDevicePointer<uint64_t> data() const {
    return data_.asPointer();
  }
  ...
  inline void resize(const SEALContextCuda &context, ParmsID parms_id,
                     size_t size) {
    ...
    resizeInternal(size, parms.polyModulusDegree(),
  ...
  inline const util::DeviceDynamicArray<ct_coeff_type> &dynArray() const {
    return data_;
  }
  ...
  explicit CiphertextCuda(const SEALContextCuda &context) : data_() {
    // Allocate memory but don't resize
    reserve(context, 2);
  }

  explicit CiphertextCuda(const SEALContextCuda &context, ParmsID parms_id)
      : data_() {
    // Allocate memory but don't resize
  ...
  void reserve(const SEALContextCuda &context, ParmsID parms_id,
               std::size_t size_capacity) {
    auto context_data_ptr = context.getContextData(parms_id);
    if (!context_data_ptr) {
      throw std::invalid_argument(
  ...
  inline void reserve(const SEALContextCuda &context,
                      std::size_t size_capacity) {
    auto parms_id = context.firstParmsID();
    reserve(context, parms_id, size_capacity);
  }
  ...
  inline std::size_t sizeCapacity() const noexcept {
    ...
    return poly_uint64_count ? data_.capacity() / poly_uint64_count
                             : std::size_t(0);
  }

  void save(std::ostream &stream) const;
  void saveTerms(std::ostream &stream, EvaluatorCuda &evaluator,
                 const std::vector<size_t> &termIds) const;
  void load(std::istream &stream);
  void load(std::istream &stream, const SEALContextCuda &context);
  void loadTerms(std::istream &stream, EvaluatorCuda &evaluator,
                 const std::vector<size_t> &termIds);
  ...
  static inline CiphertextCuda fromMembers(
      ParmsID parms_id, std::size_t size, std::size_t coeff_modulus_size,
      std::size_t poly_modulus_degree, bool is_ntt_form, double scale,
 ...
 private:
  void reserveInternal(std::size_t size_capacity,
    ...
    size_t new_data_capacity =
        size_capacity * poly_modulus_degree * coeff_modulus_size;
  ...
  void resizeInternal(size_t size, size_t poly_modulus_degree,
                      size_t coeff_modulus_size) {
    // std::cout << "resize internal " << size << " " << poly_modulus_degree <<
    // " " << coeff_modulus_size << std::endl;
...
class LWECiphertextCuda {
 public:
  ...
  inline std::uint64_t correctionFactor() const noexcept {
    return correction_factor_;
  }
  ...
  inline size_t polyModulusDegree() const noexcept {
    return poly_modulus_degree_;
  }

  inline size_t coeffModulusSize() const noexcept {
    return coeff_modulus_size_;
  }

  inline util::DevicePointer<uint64_t> c1() { return c1_.asPointer(); }

  inline util::ConstDevicePointer<uint64_t> c1() const {
    return c1_.asPointer();
  }

  inline util::DevicePointer<uint64_t> c0() { return c0_.asPointer(); }

  inline util::ConstDevicePointer<uint64_t> c0() const {
    return c0_.asPointer();
  }

  inline static LWECiphertextCuda fromMembers(
      ParmsID parms_id, std::size_t poly_modulus_degree,
      std::size_t coeff_modulus_size, double scale,
      std::uint64_t correction_factor,
 ...
 private:
  ...
  double scale_ = 1.0;
  std::uint64_t correction_factor_ = 1;
  util::DeviceDynamicArray<ct_coeff_type>
      c1_;  // c1.len() == coeff_modulus_size * poly_modulus_degree
  util::DeviceDynamicArray<ct_coeff_type>
      c0_;  // c2.len() == coeff_modulus_size
};
