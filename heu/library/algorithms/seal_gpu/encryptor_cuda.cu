...
EncryptorCuda::EncryptorCuda(const SEALContextCuda &context,
                             const PublicKeyCuda &public_key)
    : context_(context) {
...
void EncryptorCuda::encryptZeroInternal(ParmsID parms_id, bool is_asymmetric,
  ...
  if (parms.scheme() == SchemeType::ckks) {
    isNttForm = true;
  } else if (parms.scheme() != SchemeType::bfv &&
             parms.scheme() != SchemeType::bgv) {
  ...
  if (is_asymmetric) {
    ...
    if (prev_context_data_ptr) {
      ...
      CiphertextCuda temp;
      util::encryptZeroAsymmetric(public_key_, context_, prev_parms_id,
                                  isNttForm, temp, curandStates);

      // Modulus switching
      for (size_t i = 0; i < temp.size(); i++) {
        if (isNttForm) {
          rns_tool->divideAndRoundqLastNttInplace(
              temp.data() + i * (poly_element_count + coeff_count),
              prev_context_data.smallNTTTables());
        }
        // bfv switch-to-next
        else if (parms.scheme() != SchemeType::bgv) {
          rns_tool->divideAndRoundqLastInplace(
              temp.data() + i * (poly_element_count + coeff_count));
        }
        // bgv switch-to-next
        else {
          rns_tool->modTAndDivideqLastInplace(
              temp.data() + i * (poly_element_count + coeff_count));
        }
        kernel_util::kSetPolyArray(
            temp.data() + i * (poly_element_count + coeff_count), 1,
      ...
      util::encryptZeroAsymmetric(public_key_, context_, parms_id, isNttForm,
                                  destination, curandStates.asPointer());
    }
  } else {
    // Does not require modulus switching
    util::encryptZeroSymmetric(secret_key_, context_, parms_id, isNttForm,
                               destination, curandStates.asPointer());
  }
}

void EncryptorCuda::encryptInternal(const PlaintextCuda &plain,
                                    bool is_asymmetric,
                                    CiphertextCuda &destination) const {
  ...
  if (scheme == SchemeType::bfv) {
    ...
    // Multiply plain by scalar coeff_div_plaintext and reposition if in
    // upper-half. Result gets added into the c_0 term of ciphertext (c_0,c_1).
    multiplyAddPlainWithScalingVariant(plain, *context_.firstContextData(),
                                       destination.data());
    ...
    kernel_util::kAddPolyCoeffmod(destination_iter, plain_iter, 1,
                                  coeff_modulus_size, coeff_count,
                                  coeff_modulus, destination_iter);
    ...
    addPlainWithoutScalingVariant(plain, *context_data_ptr,
                                  DevicePointer(destination.data(0)));
  } else {
    throw invalid_argument("unsupported scheme");
  }
}
