...
namespace seal_gpu {
namespace util {
bool tryInvertUintMod(const uint64_t *operand, const uint64_t *modulus,
  ...
  // Construct a mutable copy of operand and modulus, with numerator being
  // modulus and operand being denominator. Notice that numerator > denominator.
  ...
  while (true) {
    ...
    size_t division_uint64_count =
        static_cast<size_t>(divideRoundUp(numerator_bits, bitsPerUint64));

    // Shift denominator to bring MSB in alignment with MSB of numerator.
    int denominator_shift = numerator_bits - denominator_bits;
    leftShiftUint(denominator, denominator_shift, division_uint64_count,
                  denominator);
    ...
    while (numerator_bits == denominator_bits) {
      ...
      if (subUint(numerator, denominator, division_uint64_count, difference)) {
        ...
        // Adjust quotient and remaining shifts as a result of shifting
        // numerator.
      ...
      *quotient |= 1;
      ...
      // with denominator.
      numerator_bits =
          getSignificantBitCountUint(difference, division_uint64_count);
      ...
      if (numerator_bits > 0) {
        leftShiftUint(difference, numerator_shift, division_uint64_count,
                      numerator);
    ...
    rightShiftUint(denominator, denominator_shift, division_uint64_count,
                   denominator);
    denominator_bits -= denominator_shift;
    ...
    // Correct for shifting of denominator.
    rightShiftUint(numerator, denominator_shift, division_uint64_count,
                   numerator);
    ...
    if (invert_prior_positive == invert_next_positive) {
      ...
      uint64_t borrow =
          subUint(invert_prior, invert_next, uint64_count, invert_next);
      if (borrow == 0) {
