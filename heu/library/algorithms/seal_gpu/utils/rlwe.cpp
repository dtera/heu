...
namespace seal_gpu {
namespace util {
void samplePolyTernary(shared_ptr<UniformRandomGenerator> prng,
  ...
  for (size_t i = 0; i < coeff_count; i++) {
    ...
    for (size_t j = 0; j < coeff_modulus_size; j++) {
      ...
      destination[i + j * coeff_count] =
          rand + (flag & coeff_modulus[j].value()) - 1;
    }
  }
}

void samplePolyNormal(shared_ptr<UniformRandomGenerator> prng,
                      const EncryptionParameters &parms,
                      uint64_t *destination) {
  auto coeff_modulus = parms.coeffModulus();
  size_t coeff_modulus_size = coeff_modulus.size();
  ...
  for (size_t i = 0; i < coeff_count; i++) {
    ...
    for (size_t j = 0; j < coeff_modulus_size; j++) {
      destination[i + j * coeff_count] =
          static_cast<uint64_t>(noise) + (flag & coeff_modulus[j].value());
    }
  }
}

void samplePolyCbd(shared_ptr<UniformRandomGenerator> prng,
                   const EncryptionParameters &parms, uint64_t *destination) {
  auto coeff_modulus = parms.coeffModulus();
  size_t coeff_modulus_size = coeff_modulus.size();
  size_t coeff_count = parms.polyModulusDegree();
  ...
  if (!areClose(global_variables::noise_standard_deviation, 3.2)) {
    throw logic_error(
        "centered binomial distribution only supports standard deviation 3.2; "
        "use rounded "
        "Gaussian instead");
  }
  ...
  auto cbd = [&]() {
    ...
    x[2] &= 0x1F;
    x[5] &= 0x1F;
    return hammingWeight(x[0]) + hammingWeight(x[1]) + hammingWeight(x[2]) -
           hammingWeight(x[3]) - hammingWeight(x[4]) - hammingWeight(x[5]);
  };
...
void samplePolyUniform(shared_ptr<UniformRandomGenerator> prng,
                       const EncryptionParameters &parms,
                       uint64_t *destination) {
  ...
  size_t dest_byte_count =
      mul_safe(coeff_modulus_size, mul_safe(coeff_count, sizeof(uint64_t)));
...
void encryptZeroAsymmetric(const PublicKey &public_key,
                           const SEALContext &context, ParmsID parms_id,
                           bool is_ntt_form, Ciphertext &destination) {
  ...
  for (size_t i = 0; i < coeff_modulus_size; i++) {
    ...
    for (size_t j = 0; j < encrypted_size; j++) {
      dyadicProductCoeffmod(u.get() + i * coeff_count,
      ...
      if (!is_ntt_form) {
        inverseNttNegacyclicHarvey(destination.data(j) + i * coeff_count,
                                   ntt_tables[i]);
      }
    }
  }
  ...
  for (size_t j = 0; j < encrypted_size; j++) {
    ...
    if (type == SchemeType::bgv) {
      ...
      multiplyPolyScalarCoeffmod(gaussian_iter.toConst(), coeff_modulus_size,
                                 coeff_count, plain_modulus.value(),
                                 &coeff_modulus[0], gaussian_iter);
    } else {
      if (is_ntt_form) {
    ...
    addPolyCoeffmod(gaussian_iter.toConst(), dst_iter.toConst(),
...
void encryptZeroSymmetric(const SecretKey &secret_key,
                          const SEALContext &context, ParmsID parms_id,
                          bool is_ntt_form, Ciphertext &destination) {
  // We use a fresh memory pool with `clear_on_destruction' enabled.
  ...
  //     divide_round_up(prng_info_byte_count,
  //     static_cast<size_t>(bytes_per_uint64));
  ...
  for (size_t i = 0; i < coeff_modulus_size; i++) {
    dyadicProductCoeffmod(secret_key.data().data() + i * coeff_count,
    ...
    if (type == SchemeType::bgv) {
      ...
      multiplyPolyScalarCoeffmod(noise.get() + i * coeff_count, coeff_count,
                                 plain_modulus.value(), coeff_modulus[i],
                                 noise.get() + i * coeff_count);
    }

    // c0 = as + noise
    addPolyCoeffmod(noise.get() + i * coeff_count, c0 + i * coeff_count,
                    coeff_count, coeff_modulus[i], c0 + i * coeff_count);
    // (as + noise, a) -> (-(as + noise), a),
    negatePolyCoeffmod(c0 + i * coeff_count, coeff_count, coeff_modulus[i],
                       c0 + i * coeff_count);
  }
  ...
  //     prng_info.save(reinterpret_cast<seal_byte *>(c1 + 1),
  //     prng_info_byte_count, compr_mode_type::none);
  // }
}
}  // namespace util
}  // namespace seal_gpu
