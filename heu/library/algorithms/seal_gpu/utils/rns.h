...
class RNSBase {
 ...
 public:
  ...
  inline const Modulus *base() const noexcept { return base_.get(); }
  ...
  inline const MultiplyUIntModOperand *invPuncturedProdModBaseArray()
      const noexcept {
...
class BaseConverter {
 ...
 public:
  ...
  inline std::size_t ibaseSize() const noexcept { return ibase_.size(); }

  inline std::size_t obaseSize() const noexcept { return obase_.size(); }

  inline const RNSBase &ibase() const noexcept { return ibase_; }

  inline const RNSBase &obase() const noexcept { return obase_; }

  void fastConvert(ConstHostPointer<uint64_t> in,
                   HostPointer<uint64_t> out) const;

  void fastConvertArray(ConstHostPointer<uint64_t> in,
                        HostPointer<uint64_t> out, size_t count) const;

  // The exact base convertion function, only supports obase size of 1.
  void exactConvertArray(ConstHostPointer<uint64_t> in,
                         HostPointer<uint64_t> out, size_t in_count) const;
...
class RNSTool {
 ...
 public:
  /**
  @throws std::invalid_argument if poly_modulus_degree is out of range,
  coeff_modulus is not valid, or pool is invalid.
  @throws std::logic_error if coeff_modulus and extended bases do not support
  NTT or are not coprime.
  */
  RNSTool(std::size_t poly_modulus_degree, const RNSBase &coeff_modulus,
          const Modulus &plain_modulus);

  /**
  @param[in] input Must be in RNS form, i.e. coefficient must be less than the
  associated modulus.
  */
  void divideAndRoundqLastInplace(HostPointer<uint64_t> input) const;

  void divideAndRoundqLastNttInplace(HostPointer<uint64_t> input,
                                     const NTTTables *rns_ntt_tables) const;

  /**
  Shenoy-Kumaresan conversion from Bsk to q
  */
  void fastbconvSk(ConstHostPointer<uint64_t> input,
                   HostPointer<uint64_t> destination) const;

  /**
  Montgomery reduction mod q; changes base from Bsk U {m_tilde} to Bsk
  */
  void smMrq(ConstHostPointer<uint64_t> input,
             HostPointer<uint64_t> destination) const;

  /**
  Divide by q and fast floor from q U Bsk to Bsk
  */
  void fastFloor(ConstHostPointer<uint64_t> input,
                 HostPointer<uint64_t> destination) const;

  /**
  Fast base conversion from q to Bsk U {m_tilde}
  */
  void fastbconvmTilde(ConstHostPointer<uint64_t> input,
                       HostPointer<uint64_t> destination) const;

  /**
  Compute round(t/q * |input|_q) mod t exactly
  */
  void decryptScaleAndRound(ConstHostPointer<uint64_t> phase,
                            HostPointer<uint64_t> destination) const;
  ...
  */
  ...
  Compute mod t
  */
  void decryptModt(ConstHostPointer<uint64_t> phase,
                   HostPointer<uint64_t> destination) const;

  inline auto invqLastModq() const noexcept { return inv_q_last_mod_q_.get(); }

  inline auto baseBskNttTables() const noexcept {
    // std::cout << "baseBskNttTables count = " << base_Bsk_ntt_tables_.size()
    // << std::endl;
    return base_Bsk_ntt_tables_.get();
  }

  inline auto baseq() const noexcept { return base_q_.get(); }

  inline auto baseB() const noexcept { return base_B_.get(); }

  inline auto baseBsk() const noexcept { return base_Bsk_.get(); }

  inline auto baseBskmTilde() const noexcept { return base_Bsk_m_tilde_.get(); }

  inline auto basetGamma() const noexcept { return base_t_gamma_.get(); }

  inline auto &mTilde() const noexcept { return m_tilde_; }

  inline auto &msk() const noexcept { return m_sk_; }

  inline auto &t() const noexcept { return t_; }

  inline auto &gamma() const noexcept { return gamma_; }

  inline auto &invqLastModt() const noexcept { return inv_q_last_mod_t_; }

  inline const uint64_t &qLastModt() const noexcept { return q_last_mod_t_; }
  ...
  */
  void initialize(std::size_t poly_modulus_degree, const RNSBase &q,
                  const Modulus &t);
