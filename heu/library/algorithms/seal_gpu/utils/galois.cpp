...
void GaloisTool::generateTableNtt(uint32_t galois_elt,
                                  HostArray<uint32_t> &result) const {
...
void GaloisTool::applyGalois(ConstHostPointer<uint64_t> operand,
                             uint32_t galois_elt, const Modulus &modulus,
                             HostPointer<uint64_t> result) const {
  const uint64_t modulus_value = modulus.value();
  const uint64_t coeff_count_minus_one = coeff_count_ - 1;
  uint64_t index_raw = 0;
  for (uint64_t i = 0; i <= coeff_count_minus_one;
       i++, ++operand, index_raw += galois_elt) {
...
void GaloisTool::applyGaloisNtt(ConstHostPointer<uint64_t> operand,
                                uint32_t galois_elt,
                                HostPointer<uint64_t> result) const {
