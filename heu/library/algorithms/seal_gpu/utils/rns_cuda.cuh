...
class RNSBaseCuda {
 public:
  RNSBaseCuda(const RNSBase &copy)
      : size_(copy.size_),
        ...
        inv_punctured_prod_mod_base_array_(
            copy.inv_punctured_prod_mod_base_array_) {}
  ...
  void decomposeArrayKeepOrder(DevicePointer<uint64_t> value,
  ...
  ConstDevicePointer<MultiplyUIntModOperand> invPuncturedProdModBaseArray()
      const {
 ...
 public:
  BaseConverterCuda(const BaseConverter &copy)
    ...
    for (size_t i = 0; i < obase_.size(); i++) {
      KernelProvider::copy<uint64_t>(base_change_matrix_.get() + i * isize,
                                     copy.base_change_matrix_[i].get(), isize);
    }
  }

  void fastConvertArray(ConstDevicePointer<uint64_t> in,
                        DevicePointer<uint64_t> out, size_t count) const;
  void exactConvertArray(ConstDevicePointer<uint64_t> in,
                         DevicePointer<uint64_t> out, size_t count) const;
...
class RNSToolCuda {
 public:
  ...
  void fastbconvSk(ConstDevicePointer<uint64_t> input,
                   DevicePointer<uint64_t> destination) const;

  void smMrq(ConstDevicePointer<uint64_t> input,
             DevicePointer<uint64_t> destination) const;

  void fastFloor(ConstDevicePointer<uint64_t> input,
                 DevicePointer<uint64_t> destination) const;

  void fastbconvmTilde(ConstDevicePointer<uint64_t> input,
                       DevicePointer<uint64_t> destination) const;

  void decryptScaleAndRound(ConstDevicePointer<uint64_t> phase,
                            DevicePointer<uint64_t> destination) const;

  void modTAndDivideqLastInplace(DevicePointer<uint64_t> input) const;

  void decryptModt(ConstDevicePointer<uint64_t> phase,
                   DevicePointer<uint64_t> destination) const;
