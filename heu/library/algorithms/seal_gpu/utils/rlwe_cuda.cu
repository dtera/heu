...
__global__ void gSamplePolyTenary(curandState *states,
  ...
  FOR_N(j, coeff_modulus_size) {
    ...
    if (r == 2)
      destination[gindex + j * coeff_count] = modulus_value - 1;
    else
      destination[gindex + j * coeff_count] = r;
  }
}

inline void kSamplePolyTenary(DevicePointer<curandState> states,
                              size_t coeff_modulus_size, size_t coeff_count,
                              ConstDevicePointer<Modulus> modulus,
                              DevicePointer<uint64_t> destination) {
  KERNEL_CALL(gSamplePolyTenary, coeff_count)
  (states.get(), coeff_modulus_size, coeff_count, modulus.get(),
   destination.get());
}
...
__global__ void gSamplePolyNormal(curandState *states,
                                  size_t coeff_modulus_size, size_t coeff_count,
                                  const Modulus *modulus,
                                  uint64_t *destination) {
  GET_INDEX_COND_RETURN(coeff_count);
  ...
  FOR_N(j, coeff_modulus_size) {
    uint64_t modulus_value = DeviceHelper::getModulusValue(modulus[j]);
    if (ri >= 0)
      destination[gindex + j * coeff_count] = ri;
    else
      destination[gindex + j * coeff_count] =
          modulus_value - static_cast<uint64_t>(-ri);
  }
}

inline void kSamplePolyNormal(DevicePointer<curandState> states,
                              size_t coeff_modulus_size, size_t coeff_count,
                              ConstDevicePointer<Modulus> modulus,
                              DevicePointer<uint64_t> destination) {
...
__global__ void gSamplePolyCbd(curandState *states, size_t coeff_modulus_size,
                               size_t coeff_count, const Modulus *modulus,
                               uint64_t *destination) {
  GET_INDEX_COND_RETURN(coeff_count);
  unsigned char x[6];
  ...
  x[2] &= 0x1f;
  x[5] &= 0x1f;
  int r = dHammingWeight(x[0]) + dHammingWeight(x[1]) + dHammingWeight(x[2]) -
          dHammingWeight(x[3]) - dHammingWeight(x[4]) - dHammingWeight(x[5]);
  FOR_N(j, coeff_modulus_size) {
    uint64_t modulus_value = DeviceHelper::getModulusValue(modulus[j]);
    if (r >= 0)
      destination[gindex + j * coeff_count] = r;
      ...
      destination[gindex + j * coeff_count] =
          modulus_value - static_cast<uint64_t>(-r);
  }
}

inline void kSamplePolyCbd(DevicePointer<curandState> states,
                           size_t coeff_modulus_size, size_t coeff_count,
                           ConstDevicePointer<Modulus> modulus,
                           DevicePointer<uint64_t> destination) {
...
__global__ void gSamplePolyUniform(curandState *states,
  ...
  FOR_N(j, coeff_modulus_size) {
    destination[gindex + j * coeff_count] =
        curand_uniform_double(states + gindex) *
        DeviceHelper::getModulusValue(modulus[j]);
  }
}
...
void encryptZeroAsymmetric(const PublicKeyCuda &public_key,
  ...
  // c[j] = public_key[j] * u + e[j] in BFV/CKKS = public_key[j] * u + p * e[j]
  // in BGV where e[j] <-- chi, u <-- R_3
  ...
  sampler::kSamplePolyTenary(curandStates, coeff_modulus_size, coeff_count,
                             coeff_modulus, u);

  // c[j] = u * public_key[j]
  kernel_util::kNttNegacyclicHarvey(u.get(), 1, coeff_modulus_size, coeff_power,
                                    ntt_tables);
  for (size_t j = 0; j < encrypted_size; j++) {
    kernel_util::kDyadicProductCoeffmod(u, public_key.data().data(j), 1,
                                        coeff_modulus_size, coeff_count,
                                        coeff_modulus, destination.data(j));
    if (!is_ntt_form) {
      kernel_util::kInverseNttNegacyclicHarvey(
          destination.data(j), 1, coeff_modulus_size, coeff_power, ntt_tables);
    }
  }
  ...
  // in BGV,
  for (size_t j = 0; j < encrypted_size; j++) {
    sampler::kSamplePolyCbd(curandStates, coeff_modulus_size, coeff_count,
                            coeff_modulus, u);
    ...
    if (type == SchemeType::bgv) {
      if (is_ntt_form)
        kernel_util::kNttNegacyclicHarveyLazy(
            gaussian_iter, 1, coeff_modulus_size, coeff_power, ntt_tables);
      kernel_util::kMultiplyPolyScalarCoeffmod(
          gaussian_iter, 1, coeff_modulus_size, coeff_count,
          plain_modulus.value(), coeff_modulus, gaussian_iter);
    } else {
      if (is_ntt_form)
        kernel_util::kNttNegacyclicHarvey(gaussian_iter, 1, coeff_modulus_size,
                                          coeff_power, ntt_tables);
    }
    kernel_util::kAddPolyCoeffmod(gaussian_iter, destination.data(j), 1,
                                  coeff_modulus_size, coeff_count,
                                  coeff_modulus, destination.data(j));
  }
}
...
void encryptZeroSymmetric(const SecretKeyCuda &secret_key,
  ...
  bootstrap_prng->generate(prng_seed_byte_count,
                           reinterpret_cast<byte *>(public_prng_seed.data()));
  ...
  uint64_t seed;
  while (true) {
    seed = (static_cast<uint64_t>(ciphertext_prng->generate()) << 32) +
           static_cast<uint64_t>(ciphertext_prng->generate());
    if (seed) break;
  }
  ...
  sampler::kSamplePolyUniform(newStates.get(), coeff_modulus_size, coeff_count,
                              coeff_modulus, c1.get());
  ...
  kernel_util::kDyadicProductCoeffmod(secret_key.data().data(), c1, 1,
                                      coeff_modulus_size, coeff_count,
                                      coeff_modulus, c0);
  if (is_ntt_form) {
    kernel_util::kNttNegacyclicHarvey(noise.get(), 1, coeff_modulus_size,
                                      coeff_power, ntt_tables);
  } else {
    kernel_util::kInverseNttNegacyclicHarvey(c0, 1, coeff_modulus_size,
                                             coeff_power, ntt_tables);
  }
  if (type == SchemeType::bgv) {
    kernel_util::kMultiplyPolyScalarCoeffmod(noise, 1, coeff_modulus_size,
                                             coeff_count, plain_modulus.value(),
                                             coeff_modulus, noise);
  }
  kernel_util::kAddPolyCoeffmod(noise, c0, 1, coeff_modulus_size, coeff_count,
                                coeff_modulus, c0);
  kernel_util::kNegatePolyCoeffmod(c0, 1, coeff_modulus_size, coeff_count,
                                   coeff_modulus, c0);
  if (!is_ntt_form) {
    kernel_util::kInverseNttNegacyclicHarvey(c1, 1, coeff_modulus_size,
                                             coeff_power, ntt_tables);
  }
}
}  // namespace util
