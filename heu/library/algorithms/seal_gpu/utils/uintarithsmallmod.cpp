...
uint64_t exponentiateUintMod(uint64_t operand, uint64_t exponent,
...
void divideUintModInplace(uint64_t *numerator, const Modulus &modulus,
  ...
  if (uint64_count == 2) {
    ...
    *quotient = *numerator / modulus.value();
    ...
    multiplyUint(x1, uint64_count - 2, &modulus.constRatio()[0], 2,
                 uint64_count, quot);  // x1*A.
    multiplyUint(x1, uint64_count - 2, modulus.constRatio()[2],
                 uint64_count - 1, rem);                          // x1*B
    addUint(rem, uint64_count - 1, x2, 2, 0, uint64_count, rem);  // x1*B + x2;

    size_t remainder_uint64_count =
        getSignificantUint64CountUint(rem, uint64_count);
...
uint64_t dotProductMod(const uint64_t *operand1, const uint64_t *operand2,
                       size_t count, const Modulus &modulus) {
  ...
  switch (count) {
    ...
    default:
      accumulator[0] =
          dotProductMod(operand1 + 16, operand2 + 16, count - 16, modulus);
      goto largest_case;
  };
