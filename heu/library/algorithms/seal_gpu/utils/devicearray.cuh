...
class ConstDevicePointer {
 ...
 public:
  ...
  ConstDevicePointer(const DeviceArray<T> &arr) : ptr(arr.get()) {}
};
...
class DeviceArray {
 ...
 public:
  ...
  DeviceArray(T *data, size_t length) : data(data), len(length) {}
  ...
  DeviceArray &operator=(DeviceArray &&a) {
    if (data)
      MemoryPoolCuda::Free(reinterpret_cast<void *>(data), len * sizeof(T));
    data = a.data;
    len = a.len;
    a.data = nullptr;
    a.len = 0;
  ...
  ConstDevicePointer<T> asPointer() const {
    return ConstDevicePointer<T>(data);
  }

  DevicePointer<T> operator+(size_t d) { return DevicePointer(data + d); }

  ConstDevicePointer<T> operator+(size_t d) const {
    return ConstDevicePointer(data + d);
  }

  __device__ inline T deviceAt(size_t id) const { return data[id]; }

  __device__ inline T *deviceGet() const { return data; }
...
class DeviceDynamicArray {
 ...
 public:
  ...
  DeviceDynamicArray(size_t capacity, size_t size)
      : internal(capacity), size_(size) {}
  ...
  DeviceDynamicArray(const HostDynamicArray<T> &h)
      : size_(h.size()), internal(h.internal) {}

  DeviceDynamicArray &operator=(const DeviceDynamicArray &copy) {
    if (copy.size_ <= size_) {
      KernelProvider::copyOnDevice(internal.get(), copy.internal.get(),
                                   copy.size_);
      size_ = copy.size();
    } else {
      size_ = copy.size();
  ...
  ConstDevicePointer<T> asPointer() const { return internal.asPointer(); }
