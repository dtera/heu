...
class DynArray {
  ...
  */
  DynArray(MemoryPoolHandle pool = MemoryManager::GetPool())
      : pool_(std::move(pool)) {
    if (!pool_) {
  ...
  */
  ...
  @throws std::invalid_argument if pool is uninitialized
  */
  explicit DynArray(util::Pointer<T> &&ptr, std::size_t capacity,
                    std::size_t size, bool fill_zero,
                    MemoryPoolHandle pool = MemoryManager::GetPool())
  ...
  */
  SEAL_NODISCARD inline T *begin() noexcept { return data_.get(); }

  /**
  Returns a constant pointer to the beginning of the array data.
  */
  SEAL_NODISCARD inline const T *begin() const noexcept { return cbegin(); }

  /**
  Returns a constant pointer to the beginning of the array data.
  */
  SEAL_NODISCARD inline const T *cbegin() const noexcept { return data_.get(); }

  /**
  Returns a pointer to the end of the array data.
  */
  SEAL_NODISCARD inline T *end() noexcept { return begin() + size_; }

  /**
  Returns a constant pointer to the end of the array data.
  */
  SEAL_NODISCARD inline const T *end() const noexcept { return cend(); }
  ...
  */
  SEAL_NODISCARD inline bool empty() const noexcept { return (size_ == 0); }
  ...
  */
  SEAL_NODISCARD inline std::size_t size() const noexcept { return size_; }
  ...
  */
  SEAL_NODISCARD inline MemoryPoolHandle pool() const noexcept { return pool_; }
  ...
  */
  SEAL_NODISCARD inline std::streamoff save_size(
    ...
    return util::safe_cast<std::streamoff>(
        util::add_safe(sizeof(Serialization::SEALHeader), members_size));
  }
  ...
  */
  inline std::streamoff save(
    ...
    return Serialization::Save(std::bind(&DynArray<T>::save_members, this, _1),
                               save_size(compr_mode_type::none), stream,
                               compr_mode, false);
  }
  ...
  */
  inline std::streamoff load(std::istream &stream,
    ...
    return Serialization::Load(
        std::bind(&DynArray<T>::load_members, this, _1, _2, in_size_bound),
        stream, false);
  }
  ...
  */
  inline std::streamoff save(
      seal_byte *out, std::size_t size,
      compr_mode_type compr_mode = Serialization::compr_mode_default) const {
    using namespace std::placeholders;
    return Serialization::Save(std::bind(&DynArray<T>::save_members, this, _1),
                               save_size(compr_mode_type::none), out, size,
                               compr_mode, false);
  }
  ...
  */
  inline std::streamoff load(const seal_byte *in, std::size_t size,
    ...
    using namespace std::placeholders;
    return Serialization::Load(
        std::bind(&DynArray<T>::load_members, this, _1, _2, in_size_bound), in,
        size, false);
  }
 ...
 private:
  ...
  void load_members(std::istream &stream, SEAL_MAYBE_UNUSED SEALVersion version,
