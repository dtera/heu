...
namespace seal_gpu {
namespace util {
inline void incrementUintMod(const std::uint64_t *operand,
...
inline void decrementUintMod(const std::uint64_t *operand,
                             const std::uint64_t *modulus,
                             std::size_t uint64_count, std::uint64_t *result) {
...
inline void negateUintMod(const std::uint64_t *operand,
                          const std::uint64_t *modulus,
                          std::size_t uint64_count, std::uint64_t *result) {
  if (isZeroUint(operand, uint64_count)) {
    ...
    // Otherwise, we know operand > 0 and < modulus so subtract modulus -
    // operand.
    subUint(modulus, operand, uint64_count, result);
  }
}

inline void div2UintMod(const std::uint64_t *operand,
  ...
  if (*operand & 1) {
    ...
    if (carry) {
      setBitUint(result, uint64_count,
                 static_cast<int>(uint64_count) * bitsPerUint64 - 1);
    }
  } else {
...
inline void addUintUintMod(const std::uint64_t *operand1,
...
inline void subUintUintMod(const std::uint64_t *operand1,
                           const std::uint64_t *operand2,
                           const std::uint64_t *modulus,
                           std::size_t uint64_count, std::uint64_t *result) {
...
bool tryInvertUintMod(const std::uint64_t *operand,
                      const std::uint64_t *modulus, std::size_t uint64_count,
                      std::uint64_t *result);
}  // namespace util
}  // namespace seal_gpu
