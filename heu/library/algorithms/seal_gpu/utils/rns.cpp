...
RNSBase::RNSBase(const RNSBase &copy) : size_(copy.size_) {
  ...
  setUint(copy.punctured_prod_array_.get(), size_ * size_,
          punctured_prod_array_.get());

  // FIXME: allocate related action
  inv_punctured_prod_mod_base_array_ =
      std::move(HostArray<MultiplyUIntModOperand>(size_));
  std::copy_n(copy.inv_punctured_prod_mod_base_array_.get(), size_,
              inv_punctured_prod_mod_base_array_.get());
}
...
bool RNSBase::initialize() {
  ...
  if (size_ > 1) {
    ...
    for (std::size_t i = 0; i < size_; i++) {
      multiplyManyUint64Except(rnsbase_values.get(), size_, i,
                               punctured_prod_array_.get() + i * size_);
    }

    // Compute the full product
    // FIXME:: allocate related action
    auto temp_mpi = HostArray<uint64_t>(size_);
    multiplyUint(punctured_prod_array_.get(), size_, base_[0].value(), size_,
                 temp_mpi.get());
    ...
    for (std::size_t i = 0; i < size_; i++) {
      uint64_t temp =
          moduloUint(punctured_prod_array_.get() + i * size_, size_, base_[i]);
...
void RNSBase::decomposeArray(uint64_t *value, size_t count) const {
  ...
  if (size_ > 1) {
    ...
    // Copy the input array into a temporary location and set a StrideIter
    // pointing to it Note that the stride size is size_
    ...
    for (std::size_t i = 0; i < size_; i++) {
      ...
      for (std::size_t j = 0; j < count; j++) {
        ...
        // Reduce the multi-precision integer modulo the base element and write
        // to value_out
        value[i * count + j] =
            moduloUint(value_copy.get() + j * size_, size_, base_[i]);
...
void RNSBase::compose(uint64_t *value) const {
  ...
  if (size_ > 1) {
    ...
    // Compose an array of integers (one per base element) into a single
    // multi-precision integer
    // FIXME: allocate related action
    auto temp_mpi = HostArray<uint64_t>(size_);
    for (std::size_t i = 0; i < size_; i++) {
      uint64_t temp_prod = multiplyUintMod(
          temp_value[i], inv_punctured_prod_mod_base_array_[i], base_[i]);
      multiplyUint(punctured_prod_array_.get() + i * size_, size_, temp_prod,
                   size_, temp_mpi.get());
...
void RNSBase::composeArray(uint64_t *value, size_t count) const {
  ...
  if (size_ > 1) {
    ...
    // Compose an array of RNS integers into a single array of multi-precision
    // integers
    // FIXME: allocate related action
    auto temp_mpi = HostArray<uint64_t>(size_);
    for (size_t i = 0; i < count; i++) {
      for (size_t j = 0; j < size_; j++) {
        uint64_t temp_prod =
            multiplyUintMod(temp_array[i * size_ + j],
                            inv_punctured_prod_mod_base_array_[j], base_[j]);
        multiplyUint(punctured_prod_array_.get() + j * size_, size_, temp_prod,
                     size_, temp_mpi.get());
        addUintUintMod(temp_mpi.get(), value + i * size_, base_prod_.get(),
                       size_, value + i * size_);
      }
    }
  }
...
void BaseConverter::fastConvert(ConstHostPointer<uint64_t> in,
                                HostPointer<uint64_t> out) const {
  ...
  for (size_t i = 0; i < ibase_size; i++) {
    temp[i] = multiplyUintMod(in[i], ibase_.invPuncturedProdModBaseArray()[i],
                              ibase_.base()[i]);
  }
  for (size_t i = 0; i < obase_size; i++) {
    out[i] = dotProductMod(temp.get(), base_change_matrix_[i].get(), ibase_size,
                           obase_.base()[i]);
  }
}

void BaseConverter::fastConvertArray(ConstHostPointer<uint64_t> in,
                                     HostPointer<uint64_t> out,
                                     size_t count) const {
#ifdef SEAL_DEBUG
  ...
  for (size_t i = 0; i < ibase_size; i++) {
    ...
    if (ibase_.invPuncturedProdModBaseArray()[i].operand == 1) {
      ...
      for (size_t j = 0; j < count; j++) {
        ...
        temp[j * ibase_size + ibase_index] =
            barrettReduce64(in[i * count + j], ibase_.base()[i]);
      };
    } else {
      // Multiplication needed
      for (size_t j = 0; j < count; j++) {
        // Multiply coefficient of in with
        // ibase_.inv_punctured_prod_mod_base_array_ element
        temp[j * ibase_size + ibase_index] = multiplyUintMod(
            in[i * count + j], ibase_.invPuncturedProdModBaseArray()[i],
            ibase_.base()[i]);
      }
    }
  }
  ...
  for (size_t i = 0; i < obase_size; i++) {
    for (size_t j = 0; j < count; j++) {
      ...
      out[i * count + j] = dotProductMod(temp.get() + j * ibase_size,
                                         base_change_matrix_[i].get(),
                                         ibase_size, obase_.base()[i]);
    }
  }
}

// See "An Improved RNS Variant of the BFV Homomorphic Encryption Scheme"
// (CT-RSA 2019) for details
void BaseConverter::exactConvertArray(ConstHostPointer<uint64_t> in,
                                      HostPointer<uint64_t> out,
  ...
  for (size_t i = 0; i < ibase_size; i++) {
    // SEAL_ITERATE(
    //     iter(in, ibase_.inv_punctured_prod_mod_base_array(), ibase_.base(),
    //     size_t(0)), ibase_size,
    //     [&](auto I) {
    // The current ibase index
    size_t ibase_index = i;
    ...
    if (ibase_.invPuncturedProdModBaseArray()[i].operand == 1) {
      ...
      for (size_t j = 0; j < count; j++) {
        ...
        // Reduce modulo ibase element
        temp[j * ibase_size + ibase_index] =
            barrettReduce64(in[i * count + j], ibase_modulus);
        double_t dividend =
            static_cast<double_t>(temp[j * ibase_size + ibase_index]);
  ...
  for (size_t j = 0; j < count; j++) {
    ...
    // Compute the base conversion sum modulo obase element
    auto sum_mod_obase = dotProductMod(temp.get() + j * ibase_size,
                                       base_change_matrix_first, ibase_size, p);
    // Minus v*[q]_{p} mod p
...
void BaseConverter::initialize() {
  ...
  // FIXME: allocate related action
  base_change_matrix_ =
      std::move(HostArray<HostArray<uint64_t>>(obase_.size()));

  for (size_t i = 0; i < obase_.size(); i++) {
    // SEAL_ITERATE(iter(base_change_matrix_, obase_.base()), obase_.size(),
    // [&](auto I) { Create the base-change matrix columns
    // FIXME: allocate related action
    base_change_matrix_[i] = std::move(HostArray<uint64_t>(ibase_.size()));

    for (size_t j = 0; j < ibase_.size(); j++) {
      // Base-change matrix contains the punctured products of ibase elements
      // modulo the obase
      base_change_matrix_[i][j] =
          moduloUint(ibase_.puncturedProdArray() + ibase_.size() * j,
                     ibase_.size(), obase_.base()[i]);
    }
  }
}

RNSTool::RNSTool(size_t poly_modulus_degree, const RNSBase &coeff_modulus,
                 const Modulus &plain_modulus) {
  initialize(poly_modulus_degree, coeff_modulus, plain_modulus);
}

void RNSTool::initialize(size_t poly_modulus_degree, const RNSBase &q,
                         const Modulus &t) {
  // Return if q is out of bounds
  if (q.size() < SEAL_COEFF_MOD_COUNT_MIN ||
      q.size() > SEAL_COEFF_MOD_COUNT_MAX) {
    throw invalid_argument("rnsbase is invalid");
  }
  ...
  // In some cases we might need to increase the size of the base B by one,
  // namely we require K * n * t * q^2 < q * prod(B) * m_sk, where K takes into
  // account cross terms when larger size ciphertexts are used, and n is the
  // "delta factor" for the ring. We reserve 32 bits for K * n. Here the coeff
  // modulus primes q_i are bounded to be SEAL_USER_MOD_BIT_COUNT_MAX (60) bits,
  // and all primes in B and m_sk are SEAL_INTERNAL_MOD_BIT_COUNT (61) bits.
  int total_coeff_bit_count =
      getSignificantBitCountUint(q.baseProd(), q.size());

  size_t base_B_size = base_q_size;
  if (32 + t_.bitCount() + total_coeff_bit_count >=
      SEAL_INTERNAL_MOD_BIT_COUNT * safe_cast<int>(base_q_size) +
          SEAL_INTERNAL_MOD_BIT_COUNT) {
    base_B_size++;
  }
  ...
  auto baseconv_primes =
      getPrimes(mul_safe(size_t(2), coeff_count_), SEAL_INTERNAL_MOD_BIT_COUNT,
  ...
  // Generate the Bsk NTTTables; these are used for NTT after base extension to
  // Bsk
  try {
    base_Bsk_ntt_tables_ = CreateNTTTables(
        coeff_count_power,
        vector<Modulus>(base_Bsk_->base(), base_Bsk_->base() + base_Bsk_size));
  ...
  base_q_to_m_tilde_conv_ =
      HostObject(new BaseConverter(*base_q_, RNSBase({m_tilde_})));
  ...
  base_B_to_m_sk_conv_ =
      HostObject(new BaseConverter(*base_B_, RNSBase({m_sk_})));

  if (!base_t_gamma_.isNull()) {
    // Set up BaseConverter for q --> {t, gamma}
    base_q_to_t_gamma_conv_ =
        HostObject(new BaseConverter(*base_q_, *base_t_gamma_));
  }
  ...
  for (size_t i = 0; i < base_q_size; i++) {
    prod_B_mod_q_[i] =
        moduloUint(base_B_->baseProd(), base_B_size, base_q_->base()[i]);
  }
  ...
  for (size_t i = 0; i < base_Bsk_size; i++) {
    prod_q_mod_Bsk_[i] =
        moduloUint(base_q_->baseProd(), base_q_size, base_Bsk_->base()[i]);
  }
  ...
  if (!base_t_gamma_.isNull()) {
    ...
    for (size_t i = 0; i < base_q_size; i++) {
      // SEAL_ITERATE(iter(prod_t_gamma_mod_q_, base_q_->base()), base_q_size,
      // [&](auto I) {
      prod_t_gamma_mod_q_[i].set(
          multiplyUintMod((*base_t_gamma_)[0].value(),
                          (*base_t_gamma_)[1].value(), base_q_->base()[i]),
          base_q_->base()[i]);
    }

    // Compute -prod(q)^(-1) mod {t, gamma}
    // FIXME: allocate related action
    neg_inv_q_mod_t_gamma_ =
        HostArray<MultiplyUIntModOperand>(base_t_gamma_size);
    for (size_t i = 0; i < base_t_gamma_size; i++) {
      const Modulus &b = base_t_gamma_->base()[i];
      neg_inv_q_mod_t_gamma_[i].operand =
          moduloUint(base_q_->baseProd(), base_q_size, b);
      if (!tryInvertUintMod(neg_inv_q_mod_t_gamma_[i].operand, b,
                            neg_inv_q_mod_t_gamma_[i].operand)) {
        throw logic_error("invalid rns bases");
      }
      neg_inv_q_mod_t_gamma_[i].set(
          negateUintMod(neg_inv_q_mod_t_gamma_[i].operand, b), b);
    }
  }
  ...
  for (size_t i = 0; i < base_q_size - 1; i++) {
    // SEAL_ITERATE(iter(inv_q_last_mod_q_, base_q_->base()), base_q_size - 1,
    // [&](auto I) {
    if (!tryInvertUintMod((*base_q_)[base_q_size - 1].value(),
                          base_q_->base()[i], temp)) {
      throw logic_error("invalid rns bases");
    }
  ...
  if (t_.value() != 0) {
    if (!tryInvertUintMod(base_q_->base()[base_q_size - 1].value(), t_,
                          inv_q_last_mod_t_)) {
      throw logic_error("invalid rns bases");
    }

    q_last_mod_t_ =
        barrettReduce64(base_q_->base()[base_q_size - 1].value(), t_);
  }
}
...
void RNSTool::divideAndRoundqLastInplace(HostPointer<uint64_t> input) const {
  ...
  addPolyScalarCoeffmod(last_input, coeff_count_, half, last_modulus,
                        last_input);
  // FIXME: allocate related action
  auto temp = HostArray<uint64_t>(coeff_count_);
  for (size_t i = 0; i < base_q_size - 1; i++) {
    // SEAL_ITERATE(iter(input, inv_q_last_mod_q_, base_q_->base()), base_q_size
    // - 1, [&](auto I) {
    ...
    subPolyScalarCoeffmod(temp.asPointer(), coeff_count_, half_mod, b,
                          temp.asPointer());
    // (ct mod qi) - (ct mod qk) mod qi
    subPolyCoeffmod(input + i * coeff_count_, temp.asPointer(), coeff_count_, b,
                    input + i * coeff_count_);
    // qk^(-1) * ((ct mod qi) - (ct mod qk)) mod qi
    multiplyPolyScalarCoeffmod(input + i * coeff_count_, coeff_count_,
                               inv_q_last_mod_q_[i], b,
                               input + i * coeff_count_);
  }
}
...
void RNSTool::divideAndRoundqLastNttInplace(
  ...
  for (size_t i = 0; i < base_q_size - 1; i++) {
    // SEAL_ITERATE(iter(input, inv_q_last_mod_q_, base_q_->base(),
    // rns_ntt_tables), base_q_size - 1, [&](auto I) {
    const Modulus &b = base_q_->base()[i];
    // (ct mod qk) mod qi
    ...
    // Lazy subtraction here. ntt_negacyclic_harvey_lazy can take 0 < x < 4*qi
    // input.
    ...
    for (size_t j = 0; j < coeff_count_; j++)
      input[i * coeff_count_ + j] += qi_lazy - temp[j];
...
void RNSTool::fastbconvSk(ConstHostPointer<uint64_t> input,
                          HostPointer<uint64_t> destination) const {
  /*
  ...
  */
  ...
  // Take the m_sk part of input, subtract from temp, and multiply by
  // inv_prod_B_mod_m_sk_ Note: input_sk is allocated in input[base_B_size]
  // FIXME: allocate related action
  auto alpha_sk = HostArray<uint64_t>(coeff_count_);
  for (size_t i = 0; i < coeff_count_; i++) {
    // SEAL_ITERATE(iter(alpha_sk, temp, input[base_B_size]), coeff_count_,
    // [&](auto I) { It is not necessary for the negation to be reduced modulo
    // the small prime
    alpha_sk[i] = multiplyUintMod(
        temp[i] + (m_sk_.value() - input[base_B_size * coeff_count_ + i]),
        inv_prod_B_mod_m_sk_, m_sk_);
  }

  // alpha_sk is now ready for the Shenoy-Kumaresan conversion; however, note
  // that our alpha_sk here is not a centered reduction, so we need to apply a
  // correction below.
  const uint64_t m_sk_div_2 = m_sk_.value() >> 1;
  for (size_t i = 0; i < base_q_size; i++) {
    // SEAL_ITERATE(iter(prod_B_mod_q_, base_q_->base(), destination),
    // base_q_size, [&](auto I) {
    const Modulus &b = base_q_->base()[i];
    // Set up the multiplication helpers
    MultiplyUIntModOperand prod_B_mod_q_elt;
    prod_B_mod_q_elt.set(prod_B_mod_q_[i], b);
    // printf("i=%lu, %llu, %llu\n", i, prod_B_mod_q_elt.operand,
    // prod_B_mod_q_elt.quotient);

    MultiplyUIntModOperand neg_prod_B_mod_q_elt;
    neg_prod_B_mod_q_elt.set(b.value() - prod_B_mod_q_[i], b);
    // printf("i=%lu, x %llu, %llu\n", i, neg_prod_B_mod_q_elt.operand,
    // neg_prod_B_mod_q_elt.quotient);
    ...
    for (size_t j = 0; j < coeff_count_; j++) {
      ...
      if (alpha_sk[j] > m_sk_div_2) {
        dest = multiplyAddUintMod(negateUintMod(alpha_sk[j], m_sk_),
                                  prod_B_mod_q_elt, dest, b);
      }
      // No correction needed
      else {
        // It is not necessary for the negation to be reduced modulo the small
        // prime
...
void RNSTool::smMrq(ConstHostPointer<uint64_t> input,
                    HostPointer<uint64_t> destination) const {
  /*
  ...
  */
  ...
  ConstHostPointer<uint64_t> input_m_tilde =
  ...
  multiplyPolyScalarCoeffmod(input_m_tilde, coeff_count_,
                             neg_inv_prod_q_mod_m_tilde_, m_tilde_,
                             r_m_tilde.asPointer());

  for (size_t i = 0; i < base_Bsk_size; i++) {
    // iter(input, prod_q_mod_Bsk_, inv_m_tilde_mod_Bsk_, base_Bsk_->base(),
    // destination), base_Bsk_size,
    // [&](auto I) {
    const Modulus &b = base_Bsk_->base()[i];
    MultiplyUIntModOperand prod_q_mod_Bsk_elt;
    prod_q_mod_Bsk_elt.set(prod_q_mod_Bsk_[i], b);
    for (size_t j = 0; j < coeff_count_; j++) {
      // SEAL_ITERATE(iter(get<0>(I), r_m_tilde, get<4>(I)), coeff_count_,
      // [&](auto J) { We need centered reduction of r_m_tilde modulo Bsk. Note
      // that m_tilde is chosen to be a power of two so we have '>=' below.
      ...
      destination[i * coeff_count_ + j] =
          multiplyUintMod(multiplyAddUintMod(temp, prod_q_mod_Bsk_elt,
                                             input[i * coeff_count_ + j], b),
                          inv_m_tilde_mod_Bsk_[i], b);
    }
  }
}

void RNSTool::fastFloor(ConstHostPointer<uint64_t> input,
                        HostPointer<uint64_t> destination) const {
  /*
  ...
  */
  ...
  for (size_t i = 0; i < base_Bsk_size; i++) {
    // SEAL_ITERATE(iter(input, inv_prod_q_mod_Bsk_, base_Bsk_->base(),
    // destination), base_Bsk_size, [&](auto I) {
    for (size_t j = 0; j < coeff_count_; j++) {
      ...
      destination[i * coeff_count_ + j] = multiplyUintMod(
          input[i * coeff_count_ + j] + (base_Bsk_->base()[i].value() -
                                         destination[i * coeff_count_ + j]),
          inv_prod_q_mod_Bsk_[i], base_Bsk_->base()[i]);
    }
  }
}

void RNSTool::fastbconvmTilde(ConstHostPointer<uint64_t> input,
                              HostPointer<uint64_t> destination) const {
  /*
  Require: Input in q
  ...
  */
  ...
  // This is to facilitate Montgomery reduction in the next step of
  // multiplication This is NOT an ideal approach: as mentioned in BEHZ16,
  // multiplication by m_tilde can be easily merge into the base conversion
  // operation; however, then we could not use the BaseConverter as below
  // without modifications.
  // FIXME: allocate related action
  auto temp = HostArray<uint64_t>(coeff_count_ * base_q_size);
  // SEAL_ALLOCATE_GET_RNS_ITER(temp, coeff_count_, base_q_size, pool);
  multiplyPolyScalarCoeffmod(input, base_q_size, coeff_count_, m_tilde_.value(),
                             base_q_->base(), temp.asPointer());

  // Now convert to Bsk
  base_q_to_Bsk_conv_->fastConvertArray(temp.asPointer(), destination,
                                        coeff_count_);

  // Finally convert to {m_tilde}
  base_q_to_m_tilde_conv_->fastConvertArray(
      temp.asPointer(), destination + base_Bsk_size * coeff_count_,
      coeff_count_);
}

void RNSTool::decryptScaleAndRound(ConstHostPointer<uint64_t> input,
                                   HostPointer<uint64_t> destination) const {
  size_t base_q_size = base_q_->size();
  ...
  auto temp = HostArray<uint64_t>(coeff_count_ * base_q_size);
  for (size_t i = 0; i < base_q_size; i++) {
    // SEAL_ITERATE(iter(input, prod_t_gamma_mod_q_, base_q_->base(), temp),
    // base_q_size, [&](auto I) {
    multiplyPolyScalarCoeffmod(input + i * coeff_count_, coeff_count_,
                               prod_t_gamma_mod_q_[i], base_q_->base()[i],
                               temp + coeff_count_ * i);
  }
  ...
  // SEAL_ALLOCATE_GET_RNS_ITER(temp_t_gamma, coeff_count_, base_t_gamma_size,
  // pool);

  // Convert from q to {t, gamma}
  base_q_to_t_gamma_conv_->fastConvertArray(
      temp.asPointer(), temp_t_gamma.asPointer(), coeff_count_);

  // Multiply by -prod(q)^(-1) mod {t, gamma}
  for (size_t i = 0; i < base_t_gamma_size; i++) {
    // SEAL_ITERATE(
    // iter(temp_t_gamma, neg_inv_q_mod_t_gamma_, base_t_gamma_->base(),
    // temp_t_gamma), base_t_gamma_size,
    // [&](auto I) {
    multiplyPolyScalarCoeffmod(temp_t_gamma + i * coeff_count_, coeff_count_,
                               neg_inv_q_mod_t_gamma_[i],
                               base_t_gamma_->base()[i],
                               temp_t_gamma + i * coeff_count_);
  }
  ...
  // Now compute the subtraction to remove error and perform final
  // multiplication by gamma inverse mod t
  for (size_t i = 0; i < coeff_count_; i++) {
    // SEAL_ITERATE(iter(temp_t_gamma[0], temp_t_gamma[1], destination),
    // coeff_count_, [&](auto I) { Need correction because of centered mod
    if (temp_t_gamma[coeff_count_ + i] > gamma_div_2) {
      // Compute -(gamma - a) instead of (a - gamma)
      destination[i] = addUintMod(
          temp_t_gamma[i],
          barrettReduce64(gamma_.value() - temp_t_gamma[coeff_count_ + i], t_),
          t_);
    }
    // No correction needed
    else {
      destination[i] =
          subUintMod(temp_t_gamma[i],
                     barrettReduce64(temp_t_gamma[coeff_count_ + i], t_), t_);
    }
...
void RNSTool::modTAndDivideqLastInplace(HostPointer<uint64_t> input) const {
  ...
  moduloPolyCoeffs(input + (modulus_size - 1) * coeff_count_, coeff_count_,
                   plain_modulus, neg_c_last_mod_t.asPointer());
  negatePolyCoeffmod(neg_c_last_mod_t.asPointer(), coeff_count_, plain_modulus,
                     neg_c_last_mod_t.asPointer());
  if (inv_q_last_mod_t_ != 1) {
    // neg_c_last_mod_t *= q_last^(-1) (mod t)
    multiplyPolyScalarCoeffmod(neg_c_last_mod_t.asPointer(), coeff_count_,
                               inv_q_last_mod_t_, plain_modulus,
                               neg_c_last_mod_t.asPointer());
  }
  ...
  for (size_t i = 0; i < modulus_size - 1; i++) {
    // SEAL_ITERATE(iter(input, curr_modulus, inv_q_last_mod_q_), modulus_size -
    // 1, [&](auto I) { delta_mod_q_i = neg_c_last_mod_t (mod q_i)
    moduloPolyCoeffs(neg_c_last_mod_t.asPointer(), coeff_count_,
                     curr_modulus[i], delta_mod_q_i.asPointer());

    // delta_mod_q_i *= q_last (mod q_i)
    multiplyPolyScalarCoeffmod(delta_mod_q_i.asPointer(), coeff_count_,
                               last_modulus_value, curr_modulus[i],
                               delta_mod_q_i.asPointer());

    // c_i = c_i - c_last - neg_c_last_mod_t * q_last (mod 2q_i)
    const uint64_t two_times_q_i = curr_modulus[i].value() << 1;
    for (size_t j = 0; j < coeff_count_; j++) {
      // SEAL_ITERATE(iter(get<0>(I), delta_mod_q_i, input[modulus_size - 1]),
      // coeff_count_, [&](auto J) {
      input[i * coeff_count_ + j] +=
          two_times_q_i -
          barrettReduce64(input[(modulus_size - 1) * coeff_count_ + j],
                          curr_modulus[i]) -
          delta_mod_q_i[j];
    }
    ...
    multiplyPolyScalarCoeffmod(input + i * coeff_count_, coeff_count_,
                               inv_q_last_mod_q_[i], curr_modulus[i],
                               input + i * coeff_count_);
  }
}

void RNSTool::decryptModt(ConstHostPointer<uint64_t> phase,
                          HostPointer<uint64_t> destination) const {
  // Use exact base convension rather than convert the base through the compose
  // API
