...
void GaloisToolCuda::generateTableNtt(uint32_t galois_elt,
                                      DeviceArray<uint32_t> &result) const {
  ...
  for (size_t i = coeff_count_; i < coeff_count_ << 1; i++) {
    uint32_t reversed =
        reverseBits(safe_cast<uint32_t>(i), coeff_count_power_ + 1);
    uint64_t index_raw =
        (static_cast<uint64_t>(galois_elt) * static_cast<uint64_t>(reversed)) >>
        1;
    index_raw &= static_cast<uint64_t>(coeff_count_minus_one);
    *temp_ptr++ =
        reverseBits(static_cast<uint32_t>(index_raw), coeff_count_power_);
  }
...
vector<uint32_t> GaloisToolCuda::getEltsFromSteps(
    const vector<int> &steps) const {
  vector<uint32_t> galois_elts;
  transform(steps.begin(), steps.end(), back_inserter(galois_elts),
            [&](auto s) { return this->getEltFromStep(s); });
  return galois_elts;
}
...
__global__ void gApplyGalois(const uint64_t *operand, size_t poly_size,
                             size_t coeff_modulus_size, size_t coeff_count,
                             size_t coeff_power, uint32_t galois_elt,
  ...
  FOR_N(rns_index, coeff_modulus_size) {
    ...
    FOR_N(poly_index, poly_size) {
      uint64_t result_value =
          operand[(poly_index * coeff_modulus_size + rns_index) * coeff_count +
                  gindex];
      if ((index_raw >> coeff_power) & 1) {
        int64_t non_zero = (result_value != 0);
        result_value =
            (modulus_value - result_value) & static_cast<uint64_t>(-non_zero);
      }
      result[(poly_index * coeff_modulus_size + rns_index) * coeff_count +
             index] = result_value;
    }
  }
}

void GaloisToolCuda::applyGalois(ConstDevicePointer<uint64_t> operand,
                                 size_t poly_size, size_t coeff_modulus_size,
                                 uint32_t galois_elt,
                                 ConstDevicePointer<Modulus> modulus,
                                 DevicePointer<uint64_t> result) const {
  KERNEL_CALL(gApplyGalois, coeff_count_)
  (operand.get(), poly_size, coeff_modulus_size, coeff_count_,
   getPowerOfTwo(coeff_count_), galois_elt, modulus.get(), result.get());
}

__global__ void gApplyGaloisNtt(const uint64_t *operand, size_t poly_size,
                                size_t coeff_modulus_size, size_t coeff_count,
                                const uint32_t *table, uint64_t *result) {
  GET_INDEX_COND_RETURN(coeff_count);
  FOR_N(rns_index, coeff_modulus_size) {
    FOR_N(poly_index, poly_size) {
...
void GaloisToolCuda::applyGaloisNtt(ConstDevicePointer<uint64_t> operand,
                                    size_t poly_size, size_t coeff_modulus_size,
                                    uint32_t galois_elt,
                                    DevicePointer<uint64_t> result) const {
  ...
  (operand.get(), poly_size, coeff_modulus_size, coeff_count_, table.get(),
   result.get());
}
}  // namespace util
}  // namespace seal_gpu
