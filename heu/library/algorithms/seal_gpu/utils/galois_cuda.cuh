...
class GaloisToolCuda {
 public:
  GaloisToolCuda(int coeff_count_power) { initialize(coeff_count_power); }

  void applyGalois(ConstDevicePointer<uint64_t> operand, std::size_t poly_size,
  ...
  void applyGaloisNtt(ConstDevicePointer<uint64_t> operand,
                      std::size_t poly_size, std::size_t coeff_modulus_size,
 ...
 private:
  ...
  void generateTableNtt(std::uint32_t galois_elt,
                        DeviceArray<std::uint32_t> &result) const;
