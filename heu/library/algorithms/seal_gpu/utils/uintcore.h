...
std::string uintToDecString(const uint64_t *value, std::size_t uint64_count);

inline void hexStringToUint(const char *hex_string, int char_count,
                            std::size_t uint64_count, uint64_t *result) {
  const char *hex_string_ptr = hex_string + char_count;
  for (std::size_t uint64_index = 0; uint64_index < uint64_count;
       uint64_index++) {
    uint64_t value = 0;
    for (int bit_index = 0; bit_index < bitsPerUint64;
         bit_index += bitsPerNibble) {
...
inline void setUint(uint64_t value, std::size_t uint64_count,
                    uint64_t *result) {
  *result++ = value;
...
inline void setUint(const uint64_t *value, std::size_t uint64_count,
                    uint64_t *result) {
...
inline bool isEqualUint(const uint64_t *value, std::size_t uint64_count,
                        uint64_t scalar) {
  if (*value++ != scalar) {
    return false;
  }
...
// #endif
// #endif
inline bool isBitSetUint(const uint64_t *value,
                         std::size_t uint64_count SEAL_MAYBE_UNUSED,
                         int bit_index) {
  int uint64_index = bit_index / bitsPerUint64;
  int sub_bit_index = bit_index - uint64_index * bitsPerUint64;
  return ((value[static_cast<std::size_t>(uint64_index)] >> sub_bit_index) &
          1) != 0;
}

inline void setBitUint(uint64_t *value,
                       std::size_t uint64_count SEAL_MAYBE_UNUSED,
                       int bit_index) {
  int uint64_index = bit_index / bitsPerUint64;
...
#endif
#endif
inline int getSignificantBitCountUint(const uint64_t *value,
  ...
  return static_cast<int>(uint64_count - 1) * bitsPerUint64 +
         getSignificantBitCount(*value);
}

inline std::size_t getSignificantUint64CountUint(const uint64_t *value,
                                                 std::size_t uint64_count) {
  value += uint64_count - 1;
...
inline std::size_t getNonzeroUint64CountUint(const uint64_t *value,
                                             std::size_t uint64_count) {
...
inline int getPowerOfTwo(uint64_t value) {
  ...
  unsigned long result =
      63UL - static_cast<unsigned long>(__builtin_clzll(value));
  return static_cast<int>(result);
}

inline void filterHighbitsUint(uint64_t *operand, std::size_t uint64_count,
                               int bit_count) {
  ...
  *operand++ &= (uint64_t(1) << subbit_index) - 1;
  for (int long_index = uint64_index + 1; unsigned_lt(long_index, uint64_count);
       long_index++) {
    *operand++ = 0;
  }
}
...
inline HostArray<uint64_t> duplicateUintIfNeeded(const uint64_t *input,
                                                 std::size_t uint64_count,
                                                 std::size_t new_uint64_count,
                                                 bool force) {
  // FIXME: allocate related action
  if (!force && uint64_count >= new_uint64_count)
    return HostArray<uint64_t>(input, uint64_count);
...
inline int compareUint(const uint64_t *operand1, const uint64_t *operand2,
                       std::size_t uint64_count) {
  int result = 0;
...
inline int compareUint(const uint64_t *operand1,
                       std::size_t operand1_uint64_count,
...
inline bool isGreaterThanUint(const uint64_t *operand1,
                              const uint64_t *operand2,
                              std::size_t uint64_count) {
  return compareUint(operand1, operand2, uint64_count) > 0;
}

inline bool isGreaterThanOrEqualUint(const uint64_t *operand1,
                                     const uint64_t *operand2,
                                     std::size_t uint64_count) {
  return compareUint(operand1, operand2, uint64_count) >= 0;
}

inline bool isLessThanUint(const uint64_t *operand1, const uint64_t *operand2,
                           std::size_t uint64_count) {
  return compareUint(operand1, operand2, uint64_count) < 0;
}

inline bool isLessThanOrEqualUint(const uint64_t *operand1,
                                  const uint64_t *operand2,
                                  std::size_t uint64_count) {
  return compareUint(operand1, operand2, uint64_count) <= 0;
}

inline bool isEqualUint(const uint64_t *operand1, const uint64_t *operand2,
                        std::size_t uint64_count) {
  return compareUint(operand1, operand2, uint64_count) == 0;
}
...
inline bool isGreaterThanUint(const uint64_t *operand1,
                              ...
                              const uint64_t *operand2,
                              std::size_t operand2_uint64_count) {
  return compareUint(operand1, operand1_uint64_count, operand2,
...
inline bool isLessThanUint(const uint64_t *operand1,
                           std::size_t operand1_uint64_count,
                           const uint64_t *operand2,
                           std::size_t operand2_uint64_count) {
  return compareUint(operand1, operand1_uint64_count, operand2,
...
inline bool isEqualUint(const uint64_t *operand1,
                        std::size_t operand1_uint64_count,
                        const uint64_t *operand2,
                        std::size_t operand2_uint64_count) {
  return compareUint(operand1, operand1_uint64_count, operand2,
