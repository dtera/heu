...
vector<uint64_t> conjugateClasses(uint64_t modulus,
...
vector<uint64_t> multiplicativeOrders(vector<uint64_t> conjugate_classes,
                                      uint64_t modulus) {
...
void babystepGiantstep(uint64_t modulus, vector<uint64_t> &baby_steps,
                       vector<uint64_t> &giant_steps) {
...
bool isPrimitiveRoot(uint64_t root, uint64_t degree, const Modulus &modulus) {
  ...
  // is -1 modulo modulus.
  return exponentiateUintMod(root, degree >> 1, modulus) ==
         (modulus.value() - 1);
}

bool tryPrimitiveRoot(uint64_t degree, const Modulus &modulus,
  ...
  // size_entire_group must be divisible by degree, or otherwise the primitive
  // root does not exist in integers modulo modulus
  ...
  do {
    ...
    destination =
        exponentiateUintMod(destination, size_quotient_group, modulus);
  } while (!isPrimitiveRoot(destination, degree, modulus) &&
           (attempt_counter < attempt_counter_max));

  return isPrimitiveRoot(destination, degree, modulus);
}

bool tryMinimalPrimitiveRoot(uint64_t degree, const Modulus &modulus,
                             uint64_t &destination) {
  uint64_t root;
