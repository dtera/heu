 ...
 public:
  ...
  NTTTablesCuda(const NTTTables &copy)
      : root_(copy.root_),
        ...
        root_powers_(copy.root_powers_),
        inv_root_powers_(copy.inv_root_powers_) {}

  __device__ inline std::uint64_t getRoot() const { return root_; }

  const __device__ inline MultiplyUIntModOperand *getFromRootPowers() const {
    return root_powers_.deviceGet();
  }

  const __device__ inline MultiplyUIntModOperand *getFromInvRootPowers() const {
    return inv_root_powers_.deviceGet();
  }

  // __device__ inline MultiplyUIntModOperand getFromRootPowers(std::size_t
  // index) const
  // {
  //     return root_powers_[index];
  // }

  // __device__ inline MultiplyUIntModOperand getFromInvRootPowers(std::size_t
  // index) const
  // {
  //     return inv_root_powers_[index];
  // }

  const __device__ inline MultiplyUIntModOperand &invDegreeModulo() const {
    return inv_degree_modulo_;
  }

  const __device__ inline Modulus &modulus() const { return modulus_; }

  __device__ inline int coeffCountPower() const { return coeff_count_power_; }

  __device__ inline std::size_t coeffCount() const { return coeff_count_; }
