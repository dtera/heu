...
void NTTTables::initialize(int coeff_count_power, const Modulus &modulus) {
  ...
  power = inv_root_;
  for (size_t i = 1; i < coeff_count_; i++) {
    inv_root_powers_[reverseBits(i - 1, coeff_count_power_) + 1].set(power,
                                                                     modulus_);
    power = multiplyUintMod(power, root, modulus_);
  }
...
//     // LegacyInputIterator allows reference to be equal to value_type so we
//     can construct
...
//     NTTTablesCreateIter &operator=(const NTTTablesCreateIter &assign) =
//     default;
...
//     {
//         return (compare.index_ == index_) && (coeff_count_power_ ==
//         compare.coeff_count_power_);
//     }
...
void nttNegacyclicHarveyLazy(HostPointer<uint64_t> operand,
  ...
  tables.nttHandler().transformToRev(operand.get(), tables.coeffCountPower(),
                                     tables.getFromRootPowers());
}
...
void inverseNttNegacyclicHarveyLazy(HostPointer<uint64_t> operand,
  ...
  tables.nttHandler().transformFromRev(operand.get(), tables.coeffCountPower(),
                                       tables.getFromInvRootPowers(),
                                       &inv_degree_modulo);
}
