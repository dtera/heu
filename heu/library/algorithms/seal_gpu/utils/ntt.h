...
namespace util {
template <>
class Arithmetic<std::uint64_t, MultiplyUIntModOperand,
                 MultiplyUIntModOperand> {
 public:
  Arithmetic() {}

  Arithmetic(const Modulus &modulus)
      : modulus_(modulus), two_times_modulus_(modulus.value() << 1) {}

  inline std::uint64_t add(const std::uint64_t &a,
                           const std::uint64_t &b) const {
    return a + b;
  }

  inline std::uint64_t sub(const std::uint64_t &a,
                           const std::uint64_t &b) const {
    return a + two_times_modulus_ - b;
  }

  inline std::uint64_t mulRoot(const std::uint64_t &a,
                               const MultiplyUIntModOperand &r) const {
    return multiplyUintModLazy(a, r, modulus_);
  }

  inline std::uint64_t mulScalar(const std::uint64_t &a,
...
class NTTTables {
  using ModArithLazy =
      Arithmetic<uint64_t, MultiplyUIntModOperand, MultiplyUIntModOperand>;
  using NTTHandler =
      DWTHandler<std::uint64_t, MultiplyUIntModOperand, MultiplyUIntModOperand>;
 ...
 public:
  ...
  NTTTables(NTTTables &copy)
      : root_(copy.root_),
        ...
        modulus_(copy.modulus_),
        inv_degree_modulo_(copy.inv_degree_modulo_),
        root_powers_(copy.coeff_count_),
        inv_root_powers_(copy.coeff_count_) {
  ...
  inline std::uint64_t getRoot() const { return root_; }
  ...
  inline int coeffCountPower() const { return coeff_count_power_; }

  inline std::size_t coeffCount() const { return coeff_count_; }

  const NTTHandler &nttHandler() const { return ntt_handler_; }
 ...
 private:
  ...
  // Holds 1~(n-1)-th powers of inv_root_ in scrambled order, the 0-th power is
  // left unset.
...
*/
HostArray<NTTTables> CreateNTTTables(int coeff_count_power,
                                     const std::vector<Modulus> &modulus);

void nttNegacyclicHarveyLazy(HostPointer<uint64_t> operand,
                             const NTTTables &tables);

inline void nttNegacyclicHarveyLazy(HostPointer<uint64_t> operand,
...
void nttNegacyclicHarvey(HostPointer<uint64_t> operand,
                         const NTTTables &tables);

inline void nttNegacyclicHarvey(HostPointer<uint64_t> operand,
                                std::size_t coeff_modulus_size,
                                const NTTTables *tables) {
  std::size_t d = (1 << tables[0].coeffCountPower());
  for (std::size_t i = 0; i < coeff_modulus_size; i++) {
...
void inverseNttNegacyclicHarveyLazy(HostPointer<uint64_t> operand,
                                    const NTTTables &tables);

inline void inverseNttNegacyclicHarveyLazy(HostPointer<uint64_t> operand,
                                           std::size_t coeff_modulus_size,
                                           const NTTTables *tables) {
  // assert(tables.length() > 0);
  std::size_t d = (1 << tables[0].coeffCountPower());
...
void inverseNttNegacyclicHarvey(HostPointer<uint64_t> operand,
                                const NTTTables &tables);

inline void inverseNttNegacyclicHarvey(HostPointer<uint64_t> operand,
                                       std::size_t coeff_modulus_size,
                                       const NTTTables *tables) {
  // assert(tables.length() > 0);
  std::size_t d = (1 << tables[0].coeffCountPower());
