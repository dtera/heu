...
namespace seal_gpu {
namespace util {
void multiplyUint(const uint64_t *operand1, size_t operand1_uint64_count,
  ...
  operand1_uint64_count =
      getSignificantUint64CountUint(operand1, operand1_uint64_count);
  operand2_uint64_count =
      getSignificantUint64CountUint(operand2, operand2_uint64_count);

  // More fast cases
  if (operand1_uint64_count == 1) {
    multiplyUint(operand2, operand2_uint64_count, *operand1,
                 result_uint64_count, result);
    return;
  }
  if (operand2_uint64_count == 1) {
    multiplyUint(operand1, operand1_uint64_count, *operand2,
                 result_uint64_count, result);
    return;
  }
  ...
  for (size_t operand1_index = 0; operand1_index < operand1_index_max;
    ...
    size_t operand2_index_max =
        min(operand2_uint64_count, result_uint64_count - operand1_index);
...
void multiplyUint(const uint64_t *operand1, size_t operand1_uint64_count,
                  uint64_t operand2, size_t result_uint64_count,
                  uint64_t *result) {
  // Handle fast cases.
...
void divideUintInplace(uint64_t *numerator, const uint64_t *denominator,
                       size_t uint64_count, uint64_t *quotient) {
  if (!uint64_count) {
    return;
  }
  ...
  uint64_count =
      safe_cast<size_t>(divideRoundUp(numerator_bits, bitsPerUint64));
  ...
  leftShiftUint(denominator, denominator_shift, uint64_count,
                shifted_denominator);
...
void divide_uint128_uint64_inplace_generic(uint64_t *numerator,
  ...
  while (numerator_bits == denominator_bits) {
    ...
    // (as opposed to fractional) bits.
    int numerator_shift =
        min(denominator_bits - numerator_bits, remaining_shifts);
...
void divideUint192Inplace(uint64_t *numerator, uint64_t denominator,
  ...
  leftShiftUint192(shifted_denominator.data(), denominator_shift,
  ...
  while (numerator_bits == denominator_bits) {
    ...
    // Even though MSB of numerator and denominator are aligned,
    // still possible numerator < shifted_denominator.
    if (subUint(numerator, shifted_denominator.data(), uint64_count,
                difference.data())) {
      // numerator < shifted_denominator and MSBs are aligned,
      // so current quotient bit is zero and next one is definitely one.
      if (remaining_shifts == 0) {
    ...
    // Determine amount to shift numerator to bring MSB in alignment with
    // denominator.
    numerator_bits =
        getSignificantBitCountUint(difference.data(), uint64_count);
    int numerator_shift = denominator_bits - numerator_bits;
    if (numerator_shift > remaining_shifts) {
      // Clip the maximum shift to determine only the integer
      // (as opposed to fractional) bits.
