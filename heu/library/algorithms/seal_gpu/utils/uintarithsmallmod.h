...
*/
inline std::uint64_t incrementUintMod(std::uint64_t operand,
                                      const Modulus &modulus) {
  operand++;
  return operand - (modulus.value() &
                    static_cast<std::uint64_t>(-static_cast<std::int64_t>(
                        operand >= modulus.value())));
}
...
*/
inline std::uint64_t decrementUintMod(std::uint64_t operand,
                                      const Modulus &modulus) {
...
*/
inline std::uint64_t negateUintMod(std::uint64_t operand,
                                   const Modulus &modulus) {
...
Correctness: operand must be even and at most (2 * modulus - 2) or odd and at
most (modulus - 2).
...
*/
inline std::uint64_t addUintMod(std::uint64_t operand1, std::uint64_t operand2,
                                const Modulus &modulus) {
  // Sum of operands modulo Modulus can never wrap around 2^64
  operand1 += operand2;
  return (operand1 >= modulus.value()) ? (operand1 - modulus.value())
                                       : (operand1);
}

/**
Returns (operand1 - operand2) mod modulus.
Correctness: (operand1 - operand2) must be at most (modulus - 1) and at least
(-modulus).
@param[in] operand1 Should be at most (modulus - 1).
@param[in] operand2 Should be at most (modulus - 1).
*/
inline std::uint64_t subUintMod(std::uint64_t operand1, std::uint64_t operand2,
                                const Modulus &modulus) {
  unsigned long long temp;
  std::int64_t borrow = static_cast<std::int64_t>(
      SEAL_SUB_BORROW_UINT64(operand1, operand2, 0, &temp));
...
*/
inline std::uint64_t barrettReduce128(const uint64_t *input,
                                      const Modulus &modulus) {
...
*/
inline std::uint64_t multiplyUintMod(std::uint64_t operand1,
...
/**
This struct contains a operand and a precomputed quotient: (operand << 64) /
modulus, for a specific modulus. When passed to multiply_uint_mod, a faster
variant of Barrett reduction will be performed. Operand must be less than
modulus.
*/
...
inline std::ostream &operator<<(std::ostream &o,
                                const MultiplyUIntModOperand &p) {
...
*/
inline std::uint64_t multiplyUintMod(std::uint64_t x, MultiplyUIntModOperand y,
                                     const Modulus &modulus) {
#ifdef SEAL_DEBUG
...
This is a highly-optimized variant of Barrett reduction and reduce to [0, 2 *
modulus - 1]. Correctness: modulus should be at most 63-bit, and y must be less
than modulus.
*/
inline std::uint64_t multiplyUintModLazy(std::uint64_t x,
...
Correctness: Follows the condition of barrett_reduce_128.
*/
inline void moduloUintInplace(std::uint64_t *value,
...
Correctness: Follows the condition of barrett_reduce_128.
*/
inline std::uint64_t moduloUint(const std::uint64_t *value,
                                std::size_t value_uint64_count,
                                const Modulus &modulus) {
  if (value_uint64_count == 1) {
...
Correctness: Follows the condition of barrett_reduce_128.
*/
inline std::uint64_t multiplyAddUintMod(std::uint64_t operand1,
                                        std::uint64_t operand2,
...
*/
inline std::uint64_t multiplyAddUintMod(std::uint64_t operand1,
                                        MultiplyUIntModOperand operand2,
                                        std::uint64_t operand3,
                                        const Modulus &modulus) {
  return addUintMod(multiplyUintMod(operand1, operand2, modulus),
                    barrettReduce64(operand3, modulus), modulus);
}

inline bool tryInvertUintMod(std::uint64_t operand, const Modulus &modulus,
                             std::uint64_t &result) {
...
Correctness: Follows the condition of barrett_reduce_128.
*/
std::uint64_t exponentiateUintMod(std::uint64_t operand, std::uint64_t exponent,
                                  const Modulus &modulus);
...
Correctness: Follows the condition of barrett_reduce_128.
*/
void divideUintModInplace(std::uint64_t *numerator, const Modulus &modulus,
...
Correctness: Follows the condition of barrett_reduce_128.
*/
std::uint64_t dotProductMod(const std::uint64_t *operand1,
                            const std::uint64_t *operand2, std::size_t count,
                            const Modulus &modulus);
}  // namespace util
}  // namespace seal_gpu
