...
string uintToHexString(const uint64_t *value, size_t uint64_count) {
  ...
  size_t num_nibbles =
      mul_safe(uint64_count, static_cast<size_t>(nibblesPerUint64));
  string output(num_nibbles, '0');

  // Iterate through each uint64 in array and set string with correct nibbles in
  // hex.
  ...
  for (size_t i = 0; i < uint64_count; i++) {
    ...
    for (size_t j = 0; j < nibblesPerUint64; j++) {
      ...
      if (nibble != 0) {
        // If nibble is not zero, then update string and save this pos to
...
string uintToDecString(const uint64_t *value, size_t uint64_count) {
  ...
  while (!isZeroUint(remainderptr, uint64_count)) {
    ...
    char digit =
        static_cast<char>(remainderptr[0] + static_cast<uint64_t>('0'));
    output += digit;
