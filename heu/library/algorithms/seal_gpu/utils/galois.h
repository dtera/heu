...
class GaloisTool {
 public:
  GaloisTool(int coeff_count_power) { initialize(coeff_count_power); }

  void applyGalois(ConstHostPointer<uint64_t> operand, std::uint32_t galois_elt,
                   const Modulus &modulus, HostPointer<uint64_t> result) const;

  inline void applyGalois(ConstHostPointer<uint64_t> operand,
                          std::size_t coeff_modulus_size,
                          std::uint32_t galois_elt, const Modulus *modulus,
                          HostPointer<uint64_t> result) const {
    for (size_t i = 0; i < coeff_modulus_size; i++) {
      this->applyGalois(operand + i * coeff_count_, galois_elt, modulus[i],
                        result + i * coeff_count_);
    }
  }

  void applyGalois(ConstHostPointer<uint64_t> operand, std::size_t poly_size,
                   std::size_t coeff_modulus_size, std::uint32_t galois_elt,
                   const Modulus *modulus, HostPointer<uint64_t> result) const {
    size_t d = coeff_modulus_size * coeff_count_;
    for (size_t i = 0; i < poly_size; i++) {
      this->applyGalois(operand + i * d, coeff_modulus_size, galois_elt,
                        modulus, result + i * d);
    }
  }

  void applyGaloisNtt(ConstHostPointer<uint64_t> operand,
  ...
  void applyGaloisNtt(ConstHostPointer<uint64_t> operand,
                      ...
                      HostPointer<uint64_t> result) const {
    for (size_t i = 0; i < coeff_modulus_size; i++) {
      this->applyGaloisNtt(operand + i * coeff_count_, galois_elt,
                           result + i * coeff_count_);
    }
  }

  void applyGaloisNtt(ConstHostPointer<uint64_t> operand, std::size_t poly_size,
                      std::size_t coeff_modulus_size, std::uint32_t galois_elt,
                      HostPointer<uint64_t> result) const {
    size_t d = coeff_modulus_size * coeff_count_;
    for (size_t i = 0; i < poly_size; i++) {
      this->applyGaloisNtt(operand + i * d, coeff_modulus_size, galois_elt,
                           result + i * d);
    }
  }
  ...
  */
  std::vector<std::uint32_t> getEltsFromSteps(
 ...
 private:
  ...
  void generateTableNtt(std::uint32_t galois_elt,
                        HostArray<std::uint32_t> &result) const;
