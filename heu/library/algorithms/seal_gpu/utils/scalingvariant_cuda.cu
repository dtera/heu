...
__global__ void gAddPlainWithoutScalingVariant(const uint64_t *plain_data,
  ...
  FOR_N(i, coeff_modulus_size) {
    uint64_t m =
        kernel_util::dBarrettReduce64(plain_data[gindex], coeff_modulus[i]);
    destination[i * coeff_count + gindex] = kernel_util::dAddUintMod(
        destination[i * coeff_count + gindex], m, coeff_modulus[i]);
  }
}
...
__global__ void gSubPlainWithoutScalingVariant(const uint64_t *plain_data,
                                               size_t plain_coeff_count,
                                               size_t coeff_count,
                                               size_t coeff_modulus_size,
                                               const Modulus *coeff_modulus,
  ...
  FOR_N(i, coeff_modulus_size) {
    uint64_t m =
        kernel_util::dBarrettReduce64(plain_data[gindex], coeff_modulus[i]);
    destination[i * coeff_count + gindex] = kernel_util::dSubUintMod(
        destination[i * coeff_count + gindex], m, coeff_modulus[i]);
  }
}
...
__global__ void gMultiplyAddPlainWithScalingVariant(
    ...
    uint64_t plain_modulus_value, uint64_t plain_upper_half_threshold,
    size_t coeff_modulus_size, const Modulus *coeff_modulus,
    const MultiplyUIntModOperand *coeff_div_plain_modulus, uint64_t q_mod_t,
    uint64_t *destination) {
  GET_INDEX_COND_RETURN(plain_coeff_count);
  uint64_t prod[2]{0, 0};
  ...
  unsigned char carry =
      kernel_util::dAddUint64(*prod, plain_upper_half_threshold, numerator);
  ...
  for (size_t j = 0; j < coeff_modulus_size; j++) {
    uint64_t scaled_rounded_coeff = kernel_util::dMultiplyAddUintMod(
        plain_data[gindex], coeff_div_plain_modulus[j], fix[0],
        coeff_modulus[j]);
    destination[j * coeff_count + gindex] =
        kernel_util::dAddUintMod(destination[j * coeff_count + gindex],
                                 scaled_rounded_coeff, coeff_modulus[j]);
  }
}
...
void multiplyAddPlainWithScalingVariant(
  ...
  (plain_data, plain_coeff_count, coeff_count, plain_modulus.value(),
   plain_upper_half_threshold, coeff_modulus_size, coeff_modulus.get(),
   coeff_div_plain_modulus.get(), q_mod_t, destination.get());
}
...
__global__ void gMultiplySubPlainWithScalingVariant(
  ...
  for (size_t j = 0; j < coeff_modulus_size; j++) {
    uint64_t scaled_rounded_coeff = kernel_util::dMultiplyAddUintMod(
        ...
        coeff_modulus[j]);
    destination[j * coeff_count + gindex] =
        kernel_util::dSubUintMod(destination[j * coeff_count + gindex],
                                 scaled_rounded_coeff, coeff_modulus[j]);
  }
}
