...
class CKKSEncoderCuda {
 public:
  ...
  inline void encode(const std::vector<std::complex<double>> &values,
  ...
  inline void encodePolynomial(const std::vector<double> &values,
                               ParmsID parms_id, double scale,
                               PlaintextCuda &destination) {
    encodePolynomialInternal(values.data(), values.size(), parms_id, scale,
                             destination);
  }

  inline void encodePolynomial(const std::vector<double> &values, double scale,
                               PlaintextCuda &destination) {
    encodePolynomial(values, context_.firstParmsID(), scale, destination);
  }

  inline void encode(double value, ParmsID parms_id, double scale,
                     PlaintextCuda &destination) {
  ...
  inline void encode(std::complex<double> value, ParmsID parms_id, double scale,
                     PlaintextCuda &destination) {
    encodeInternal(value, parms_id, scale, destination);
  }

  inline void encode(std::complex<double> value, double scale,
                     PlaintextCuda &destination) {
    encode(value, context_.firstParmsID(), scale, destination);
  }

  inline void encode(std::int64_t value, ParmsID parms_id,
                     PlaintextCuda &destination) {
  ...
  inline void decode(const PlaintextCuda &plain,
                     std::vector<std::complex<double>> &destination) {
    destination.resize(slots_);
    decodeInternal(plain, destination.data());
  }

  inline void decodePolynomial(const PlaintextCuda &plain,
                               std::vector<double> &destination) {
    destination.resize(slots_ * 2);
    decodePolynomialInternal(plain, destination.data());
  }

  inline std::size_t slotCount() const noexcept { return slots_; }

 private:
  void encodeInternal(const std::complex<double> *values,
                      std::size_t values_size, ParmsID parms_id, double scale,
                      PlaintextCuda &destination);

  void encodePolynomialInternal(const double *coeffs, std::size_t values_size,
                                ParmsID parms_id, double scale,
                                PlaintextCuda &destination);

  void decodeInternal(const PlaintextCuda &plain,
                      std::complex<double> *destination);

  void decodePolynomialInternal(const PlaintextCuda &plain,
                                double *destination);

  void encodeInternal(double value, ParmsID parms_id, double scale,
                      PlaintextCuda &destination);

  inline void encodeInternal(std::complex<double> value, ParmsID parms_id,
                             double scale, PlaintextCuda &destination) {
  ...
  void encodeInternal(std::int64_t value, ParmsID parms_id,
                      PlaintextCuda &destination);
