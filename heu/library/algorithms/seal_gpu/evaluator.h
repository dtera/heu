...
ring Z_T[x]/(X^N+1), where T is the plaintext modulus and X^N+1 is the
polynomial modulus, this is the ring where the arithmetic operations will take
place. BatchEncoder (batching) provider an alternative possibly more convenient
view of the plaintext elements as 2-by-(N2/2) matrices of integers modulo the
plaintext modulus. In the batching view the arithmetic operations act on the
matrices element-wise. Some of the operations only apply in the batching view,
such as matrix row and column rotations. Other operations such as
relinearization have no semantic meaning but are necessary for performance
reasons.

@par Arithmetic Operations
The core operations are arithmetic operations, in particular multiplication and
addition of ciphertexts. In addition to these, we also provide negation,
...
computation are plaintext elements rather than ciphertexts. For this we provide
fast "plain" operations: plain addition, plain subtraction, and plain
multiplication.
...
keys are needed), and changes the size of the ciphertext down to 2 (minimum
size). For most use-cases only one relinearization key suffices, in which case
...
Plain multiplication and addition operations of any type do not change the size.
Relinearization requires relinearization keys to have been generated.

@par Rotations
When batching is enabled, we provide operations for rotating the plaintext
matrix rows cyclically left or right, and for rotating the columns (swapping the
rows). Rotations require Galois keys to have been generated.

@par Other Operations
We also provide operations for transforming ciphertexts to NTT form and back,
and for transforming plaintext polynomials to NTT form. These can be used in a
very fast plain multiplication variant, that assumes the inputs to be in NTT
...
@par NTT form
When using the BFV/BGV scheme (SchemeType::bfv/bgv), all plaintexts and
ciphertexts should remain by default in the usual coefficient representation,
i.e., not in NTT form. When using the CKKS scheme (SchemeType::ckks), all
plaintexts and ciphertexts should remain by default in NTT form. We call these
scheme-specific NTT states the "default NTT form". Some functions, such as add,
work even if the inputs are not in the default state, but others, such as
multiply, will throw an exception. The output of all evaluation functions will
be in the same state as the input(s), with the exception of the transformTo_ntt
and transformFrom_ntt functions, which change the state. Ideally, unless these
...
class Evaluator {
  ...
  */
  inline void negate(const Ciphertext &encrypted,
  ...
  NTT forms
  @throws std::invalid_argument if encrypted1 and encrypted2 are at different
  level or scale
  @throws std::logic_error if result ciphertext is transparent
  */
  ...
  */
  ...
  @throws std::logic_error if result ciphertext is transparent
  */
  void addMany(const std::vector<Ciphertext> &encrypteds,
               Ciphertext &destination) const;
  ...
  */
  ...
  @param[in] encrypted1 The ciphertext to subtract from
  @param[in] encrypted2 The ciphertext to subtract
  @param[out] destination The ciphertext to overwrite with the subtraction
  result
  @throws std::invalid_argument if encrypted1 or encrypted2 is not valid for the
  encryption parameters
  @throws std::invalid_argument if encrypted1 and encrypted2 are in different
  ...
  */
  ...
  @throws std::invalid_argument if encrypted1 or encrypted2 is not valid for the
  encryption parameters
  @throws std::invalid_argument if encrypted1 or encrypted2 is not in the
  default NTT form
  @throws std::invalid_argument if encrypted1 and encrypted2 are at different
  level
  @throws std::invalid_argument if the output scale is too large for the
  encryption parameters
  @throws std::invalid_argument if pool is uninitialized
  @throws std::logic_error if result ciphertext is transparent
  */
  void multiplyInplace(Ciphertext &encrypted1,
  ...
  @param[in] encrypted1 The first ciphertext to multiply
  @param[in] encrypted2 The second ciphertext to multiply
  @param[out] destination The ciphertext to overwrite with the multiplication
  result
  @param[in] pool The MemoryPoolHandle pointing to a valid memory pool
  @throws std::invalid_argument if encrypted1 or encrypted2 is not valid for the
  encryption parameters
  ...
  */
  ...
  stores the result in the destination parameter. Dynamic memory allocations in
  ...
  @throws std::logic_error if result ciphertext is transparent
  */
  inline void square(const Ciphertext &encrypted,
                     Ciphertext &destination) const {
    destination = encrypted;
  ...
  /**
  Relinearizes a ciphertext. This functions relinearizes encrypted, reducing its
  size down to 2. If the size of encrypted is K+1, the given relinearization
  keys need to have size at least K-1. Dynamic memory allocations in the process
  ...
  @throws std::logic_error if result ciphertext is transparent
  */
  inline void relinearizeInplace(Ciphertext &encrypted,
  ...
  /**
  Relinearizes a ciphertext. This functions relinearizes encrypted, reducing its
  size down to 2, and stores the result in the destination parameter. If the
  size of encrypted is K+1, the given relinearization keys need to have size at
  least K-1. Dynamic memory allocations in the process are allocated from the
  memory pool pointed to by the given MemoryPoolHandle.

  @param[in] encrypted The ciphertext to relinearize
  @param[in] relin_keys The relinearization keys
  @param[out] destination The ciphertext to overwrite with the relinearized
  result
  @param[in] pool The MemoryPoolHandle pointing to a valid memory pool
  @throws std::invalid_argument if encrypted or relin_keys is not valid for the
  encryption parameters
  ...
  */
  ...
  parameter. Dynamic memory allocations in the process are allocated from the
  memory pool pointed to by the given MemoryPoolHandle.

  @param[in] encrypted The ciphertext to be switched to a smaller modulus
  @param[in] pool The MemoryPoolHandle pointing to a valid memory pool
  @param[out] destination The ciphertext to overwrite with the modulus switched
  result
  @throws std::invalid_argument if encrypted is not valid for the encryption
  parameters
  @throws std::invalid_argument if encrypted is not in the default NTT form
  @throws std::invalid_argument if encrypted is already at lowest level
  @throws std::invalid_argument if the scale is too large for the new encryption
  parameters
  @throws std::invalid_argument if pool is uninitialized
  @throws std::logic_error if result ciphertext is transparent
  */
  void modSwitchToNext(const Ciphertext &encrypted,
                       Ciphertext &destination) const;

  /**
  Given a ciphertext encrypted modulo q_1...q_k, this function switches the
  modulus down to q_1...q_{k-1}. Dynamic memory allocations in the process are
  ...
  @throws std::logic_error if result ciphertext is transparent
  */
  inline void modSwitchToNextInplace(Ciphertext &encrypted) const {
    modSwitchToNext(encrypted, encrypted);
  }

  /**
  Modulus switches an NTT transformed plaintext from modulo q_1...q_k down to
  modulo q_1...q_{k-1}.
  ...
  */
  inline void modSwitchToNextInplace(Plaintext &plain) const {
    ...
    if (!isValidFor(plain, context_)) {
      throw std::invalid_argument(
          "plain is not valid for encryption parameters");
    }
  ...
  /**
  Modulus switches an NTT transformed plaintext from modulo q_1...q_k down to
  modulo q_1...q_{k-1} and stores the result in the destination parameter.

  @param[in] plain The plaintext to be switched to a smaller modulus
  @param[in] pool The MemoryPoolHandle pointing to a valid memory pool
  @param[out] destination The plaintext to overwrite with the modulus switched
  result
  @throws std::invalid_argument if plain is not in NTT form
  @throws std::invalid_argument if plain is not valid for the encryption
  parameters
  ...
  @throws std::invalid_argument if pool is uninitialized
  */
  inline void modSwitchToNext(const Plaintext &plain,
  ...
  parameters
  @throws std::invalid_argument if encrypted is already at lower level in
  modulus chain than the parameters corresponding to parms_id
  @throws std::invalid_argument if the scale is too large for the new encryption
  parameters
  @throws std::invalid_argument if pool is uninitialized
  @throws std::logic_error if result ciphertext is transparent
  */
  void modSwitchToInplace(Ciphertext &encrypted, ParmsID parms_id) const;
  ...
  @throws std::logic_error if result ciphertext is transparent
  */
  inline void modSwitchTo(const Ciphertext &encrypted, ParmsID parms_id,
                          Ciphertext &destination) const {
    destination = encrypted;
  ...
  @throws std::invalid_argument if parms_id is not valid for the encryption
  parameters
  @throws std::invalid_argument if plain is already at lower level in modulus
  chain than the parameters corresponding to parms_id
  @throws std::invalid_argument if the scale is too large for the new encryption
  parameters
  */
  ...
  /**
  Given an NTT transformed plaintext modulo q_1...q_k, this function switches
  the modulus down until the parameters reach the given parms_id and stores the
  ...
  parameters
  */
  inline void modSwitchTo(const Plaintext &plain, ParmsID parms_id,
                          Plaintext &destination) const {
    destination = plain;
    modSwitchToInplace(destination, parms_id);
  }
  ...
  /**
  Given a ciphertext encrypted modulo q_1...q_k, this function switches the
  modulus down to q_1...q_{k-1}, scales the message down accordingly, and stores
  the result in the destination parameter. Dynamic memory allocations in the
  ...
  @throws std::logic_error if result ciphertext is transparent
  */
  void rescaleToNext(const Ciphertext &encrypted,
                     Ciphertext &destination) const;
  ...
  @throws std::logic_error if result ciphertext is transparent
  */
  inline void rescaleToNextInplace(Ciphertext &encrypted) const {
  ...
  @throws std::logic_error if result ciphertext is transparent
  */
  void rescaleToInplace(Ciphertext &encrypted, ParmsID parms_id) const;
  ...
  @throws std::logic_error if result ciphertext is transparent
  */
  inline void rescaleTo(const Ciphertext &encrypted, ParmsID parms_id,
                        Ciphertext &destination) const {
    destination = encrypted;
  ...
  /**
  Multiplies several ciphertexts together. This function computes the product of
  several ciphertext given as an std::vector and stores the result in the
  destination parameter. The multiplication is done in a depth-optimal order,
  and relinearization is performed automatically after every multiplication in
  ...
  @throws std::invalid_argument if encrypteds is empty
  @throws std::invalid_argument if ciphertexts or relin_keys are not valid for
  ...
  @throws std::logic_error if result ciphertext is transparent
  */
  void multiplyMany(const std::vector<Ciphertext> &encrypteds,
                    const RelinKeys &relin_keys, Ciphertext &destination) const;

  /**
  Exponentiates a ciphertext. This functions raises encrypted to a power.
  Dynamic memory allocations in the process are allocated from the memory pool
  pointed to by the given MemoryPoolHandle. The exponentiation is done in a
  depth-optimal order, and relinearization is performed automatically after
  every multiplication in the process. In relinearization the given
  ...
  @throws std::logic_error if result ciphertext is transparent
  */
  void exponentiateInplace(Ciphertext &encrypted, std::uint64_t exponent,
                           const RelinKeys &relin_keys) const;
  ...
  relinearization is performed automatically after every multiplication in the
  ...
  */
  ...
  encryption parameters
  @throws std::invalid_argument if encrypted or plain is not in the default NTT
  form
  @throws std::invalid_argument if encrypted and plain are at different level or
  scale
  @throws std::logic_error if result ciphertext is transparent
  */
  ...
  /**
  Adds a ciphertext and a plaintext. This function adds a ciphertext and a
  plaintext and stores the result in the destination parameter. Note that in
  many cases it can be much more efficient to perform any computations on raw
  unencrypted data before encoding it, rather than using this function to
  ...
  @throws std::logic_error if result ciphertext is transparent
  */
  inline void addPlain(const Ciphertext &encrypted, const Plaintext &plain,
                       Ciphertext &destination) const {
    destination = encrypted;
  ...
  */
  ...
  from a ciphertext and stores the result in the destination parameter.
  ...
  @throws std::logic_error if result ciphertext is transparent
  */
  inline void subPlain(const Ciphertext &encrypted, const Plaintext &plain,
                       Ciphertext &destination) const {
    destination = encrypted;
  ...
  /**
  Multiplies a ciphertext with a plaintext. The plaintext cannot be identically
  0. Dynamic memory allocations in the process are allocated from the memory
  ...
  @throws std::invalid_argument if encrypted or plain is not valid for the
  encryption parameters
  @throws std::invalid_argument if encrypted and plain are in different NTT
  forms
  @throws std::invalid_argument if the output scale is too large for the
  encryption parameters
  @throws std::invalid_argument if pool is uninitialized
  @throws std::logic_error if result ciphertext is transparent
  */
  void multiplyPlainInplace(Ciphertext &encrypted,
                            const Plaintext &plain) const;

  /**
  Multiplies a ciphertext with a plaintext. This function multiplies a
  ciphertext with a plaintext and stores the result in the destination
  parameter. The plaintext cannot be identically 0. Dynamic memory allocations
  in the process are allocated from the memory pool pointed to by the given
  MemoryPoolHandle.
  ...
  @throws std::logic_error if result ciphertext is transparent
  */
  inline void multiplyPlain(const Ciphertext &encrypted, const Plaintext &plain,
                            Ciphertext &destination) const {
    destination = encrypted;
  ...
  parms_id. For the operation to be valid, the plaintext must have degree less
  than poly_modulus_degree and each coefficient must be less than the plaintext
  ...
  @throws std::invalid_argument if pool is uninitialized
  */
  void transformToNttInplace(Plaintext &plain, ParmsID parms_id) const;
  ...
  operation to be valid, the plaintext must have degree less than
  ...
  @param[in] plain The plaintext to transform
  @param[in] parms_id The parms_id with respect to which the NTT is done
  @param[out] destinationNTT The plaintext to overwrite with the transformed
  result
  @param[in] pool The MemoryPoolHandle pointing to a valid memory pool
  @throws std::invalid_argument if plain is already in NTT form
  @throws std::invalid_argument if plain or parms_id is not valid for the
  ...
  @throws std::invalid_argument if pool is uninitialized
  */
  inline void transformToNtt(const Plaintext &plain, ParmsID parms_id,
                             Plaintext &destination_ntt) const {
  ...
  */
  ...
  /**
  Transforms a ciphertext to NTT domain. This functions applies David Harvey's
  Number Theoretic Transform separately to each polynomial of a ciphertext. The
  result is stored in the destination_ntt parameter.

  @param[in] encrypted The ciphertext to transform
  @param[out] destination_ntt The ciphertext to overwrite with the transformed
  result
  @throws std::invalid_argument if encrypted is not valid for the encryption
  parameters
  @throws std::invalid_argument if encrypted is already in NTT form
  @throws std::logic_error if result ciphertext is transparent
  */
  inline void transformToNtt(const Ciphertext &encrypted,
                             Ciphertext &destination_ntt) const {
  ...
  */
  ...
  @param[in] encrypted_ntt The ciphertext to transform
  @param[out] destination The ciphertext to overwrite with the transformed
  result
  @throws std::invalid_argument if encrypted_ntt is not valid for the encryption
  parameters
  @throws std::invalid_argument if encrypted_ntt is not in NTT form
  @throws std::logic_error if result ciphertext is transparent
  */
  inline void transformFromNtt(const Ciphertext &encrypted_ntt,
                               Ciphertext &destination) const {
  ...
  automorphism, an appropriate set of Galois keys must also be provided. Dynamic
  memory allocations in the process are allocated from the memory pool pointed
  to by the given MemoryPoolHandle.

  The desired Galois automorphism is given as a Galois element, and must be an
  odd integer in the interval [1, M-1], where M = 2*N, and N =
  poly_modulus_degree. Used with batching, a Galois element 3^i % M corresponds
  to a cyclic row rotation i steps to the left, and a Galois element 3^(N/2-i) %
  M corresponds to a cyclic row rotation i steps to the right. The Galois
  element M-1 corresponds to a column rotation (row swap) in BFV/BGV, and
  complex conjugation in CKKS. In the polynomial view (not batching), a Galois
  automorphism by a Galois element p changes Enc(plain(x)) to Enc(plain(x^p)).
  ...
  @throws std::logic_error if result ciphertext is transparent
  */
  void applyGaloisInplace(Ciphertext &encrypted, std::uint32_t galois_elt,
                          const GaloisKeys &galois_keys) const;

  /**
  Applies a Galois automorphism to a ciphertext and writes the result to the
  destination parameter. To evaluate the Galois automorphism, an appropriate set
  of Galois keys must also be provided. Dynamic memory allocations in the
  process are allocated from the memory pool pointed to by the given
  MemoryPoolHandle.
  ...
  @throws std::logic_error if result ciphertext is transparent
  */
  inline void applyGalois(const Ciphertext &encrypted, std::uint32_t galois_elt,
  ...
  of the batched matrix is 2-by-(N/2), where N is the degree of the polynomial
  modulus, the number of steps to rotate must have absolute value at most N/2-1.
  Dynamic memory allocations in the process are allocated from the memory pool
  pointed to by the given MemoryPoolHandle.
  ...
  @throws std::logic_error if result ciphertext is transparent
  */
  inline void rotateRowsInplace(Ciphertext &encrypted, int steps,
                                const GaloisKeys &galois_keys) const {
  ...
  Rotates plaintext matrix rows cyclically. When batching is used with the
  BFV/BGV scheme, this function rotates the encrypted plaintext matrix rows
  cyclically to the left (steps > 0) or to the right (steps < 0) and writes the
  result to the destination parameter. Since the size of the batched matrix is
  2-by-(N/2), where N is the degree of the polynomial modulus, the number of
  steps to rotate must have absolute value at most N/2-1. Dynamic memory
  allocations in the process are allocated from the memory pool pointed to by
  the given MemoryPoolHandle.
  ...
  @throws std::logic_error if result ciphertext is transparent
  */
  inline void rotateRows(const Ciphertext &encrypted, int steps,
                         const GaloisKeys &galois_keys,
                         Ciphertext &destination) const {
    destination = encrypted;
  ...
  /**
  Rotates plaintext matrix columns cyclically. When batching is used with the
  BFV scheme, this function rotates the encrypted plaintext matrix columns
  cyclically. Since the size of the batched matrix is 2-by-(N/2), where N is the
  degree of the polynomial modulus, this means simply swapping the two rows.
  Dynamic memory allocations in the process are allocated from the memory pool
  pointed to by the given MemoryPoolHandle.
  ...
  @throws std::logic_error if result ciphertext is transparent
  */
  inline void rotateColumnsInplace(Ciphertext &encrypted,
                                   const GaloisKeys &galois_keys) const {
    auto scheme = context_.keyContextData()->parms().scheme();
    if (scheme != SchemeType::bfv && scheme != SchemeType::bgv) {
      throw std::logic_error("unsupported scheme");
  ...
  /**
  Rotates plaintext matrix columns cyclically. When batching is used with the
  BFV/BGV scheme, this function rotates the encrypted plaintext matrix columns
  cyclically, and writes the result to the destination parameter. Since the size
  of the batched matrix is 2-by-(N/2), where N is the degree of the polynomial
  modulus, this means simply swapping the two rows. Dynamic memory allocations
  in the process are allocated from the memory pool pointed to by the given
  MemoryPoolHandle.
  ...
  @throws std::logic_error if result ciphertext is transparent
  */
  inline void rotateColumns(const Ciphertext &encrypted,
                            const GaloisKeys &galois_keys,
                            Ciphertext &destination) const {
    destination = encrypted;
  ...
  rotates the encrypted plaintext vector cyclically to the left (steps > 0) or
  to the right (steps < 0). Since the size of the batched matrix is 2-by-(N/2),
  where N is the degree of the polynomial modulus, the number of steps to rotate
  must have absolute value at most N/2-1. Dynamic memory allocations in the
  process are allocated from the memory pool pointed to by the given
  MemoryPoolHandle.
  ...
  @throws std::logic_error if result ciphertext is transparent
  */
  inline void rotateVectorInplace(Ciphertext &encrypted, int steps,
                                  const GaloisKeys &galois_keys) const {
  ...
  Rotates plaintext vector cyclically. When using the CKKS scheme, this function
  rotates the encrypted plaintext vector cyclically to the left (steps > 0) or
  to the right (steps < 0) and writes the result to the destination parameter.
  Since the size of the batched matrix is 2-by-(N/2), where N is the degree of
  the polynomial modulus, the number of steps to rotate must have absolute value
  at most N/2-1. Dynamic memory allocations in the process are allocated from
  ...
  @throws std::logic_error if result ciphertext is transparent
  */
  inline void rotateVector(const Ciphertext &encrypted, int steps,
                           const GaloisKeys &galois_keys,
                           Ciphertext &destination) const {
    destination = encrypted;
  ...
  @throws std::logic_error if result ciphertext is transparent
  */
  inline void complexConjugateInplace(Ciphertext &encrypted,
                                      const GaloisKeys &galois_keys) const {
    if (context_.keyContextData()->parms().scheme() != SchemeType::ckks) {
      throw std::logic_error("unsupported scheme");
    }
  ...
  /**
  Complex conjugates plaintext slot values. When using the CKKS scheme, this
  function complex conjugates all values in the underlying plaintext, and writes
  the result to the destination parameter. Dynamic memory allocations in the
  process are allocated from the memory pool pointed to by the given
  MemoryPoolHandle.
  ...
  @throws std::logic_error if result ciphertext is transparent
  */
  inline void complexConjugate(const Ciphertext &encrypted,
                               const GaloisKeys &galois_keys,
                               Ciphertext &destination) const {
    destination = encrypted;
 ...
 private:
  ...
  void relinearizeInternal(Ciphertext &encrypted, const RelinKeys &relin_keys,
                           std::size_t destination_size) const;

  void modSwitchScaleToNext(const Ciphertext &encrypted,
                            Ciphertext &destination) const;

  void modSwitchDropToNext(const Ciphertext &encrypted,
                           Ciphertext &destination) const;

  void modSwitchDropToNext(Plaintext &plain) const;

  void rotateInternal(Ciphertext &encrypted, int steps,
                      const GaloisKeys &galois_keys) const;

  inline void conjugateInternal(Ciphertext &encrypted,
                                const GaloisKeys &galois_keys) const {
    // Verify parameters.
  ...
  void switchKeyInplace(Ciphertext &encrypted,
                        util::ConstHostPointer<uint64_t> target_iter,
  ...
  void multiplyPlainNtt(Ciphertext &encrypted_ntt,
