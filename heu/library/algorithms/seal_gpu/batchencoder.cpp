...
BatchEncoder::BatchEncoder(const SEALContext &context) : context_(context) {
  ...
  if (context_data.parms().scheme() != SchemeType::bfv &&
      context_data.parms().scheme() != SchemeType::bgv) {
  ...
  if (context_data.qualifiers().using_batching) {
    ...
    // Fill the vector of roots of unity with all distinct odd powers of
    // generator. These are all the primitive (2*slots_)-th roots of unity in
    // integers modulo parms.plain_modulus().
...
void BatchEncoder::populateRootsOfUnityVector(
  ...
  for (size_t i = 1; i < slots_; i++) {
    roots_of_unity_[i] =
        multiplyUintMod(roots_of_unity_[i - 1], generator_sq, modulus);
  }
}
...
void BatchEncoder::populateMatrixRepsIndexMap() {
  ...
  for (size_t i = 0; i < row_size; i++) {
    ...
    matrix_reps_index_map_[i] =
        safe_cast<size_t>(util::reverseBits(index1, logn));
    matrix_reps_index_map_[row_size | i] =
        safe_cast<size_t>(util::reverseBits(index2, logn));
...
void BatchEncoder::encode(const vector<uint64_t> &values_matrix,
  ...
  inverseNttNegacyclicHarvey(destination.data(),
...
void BatchEncoder::encode(const vector<int64_t> &values_matrix,
  ...
  for (size_t i = 0; i < values_matrix_size; i++) {
    *(destination.data() + matrix_reps_index_map_[i]) =
        (values_matrix[i] < 0)
            ? (modulus + static_cast<uint64_t>(values_matrix[i]))
...
void BatchEncoder::decode(const Plaintext &plain,
