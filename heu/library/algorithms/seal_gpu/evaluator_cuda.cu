...
#define KERNEL_CALL(funcname, n)                      \
  size_t block_count = kernel_util::ceilDiv_(n, 256); \
  funcname<<<block_count, 256>>>
#define POLY_ARRAY_ARGUMENTS \
...
#define GET_INDEX_COND_RETURN(n)                         \
...
inline bool areClose(double value1, double value2) {
  ...
  return std::fabs(value1 - value2) <
         std::numeric_limits<double>::epsilon() * scale_factor;
}
...
inline bool isScaleWithinBounds(
    double scale,
    const SEALContextCuda::ContextDataCuda &context_data) noexcept {
...
inline auto balanceCorrectionFactors(uint64_t factor1, uint64_t factor2,
  ...
  while (a != 0) {
    ...
    uint64_t a_mod =
        util::barrettReduce64(static_cast<uint64_t>(abs(a)), plain_modulus);
    if (a < 0) {
      a_mod = util::negateUintMod(a_mod, plain_modulus);
    }
    uint64_t b_mod =
        util::barrettReduce64(static_cast<uint64_t>(abs(b)), plain_modulus);
    if (b < 0) {
      b_mod = util::negateUintMod(b_mod, plain_modulus);
    }
    if (a_mod != 0 &&
        util::gcd(a_mod, t) == 1)  // which also implies gcd(b_mod, t) == 1
    {
  ...
  return std::make_tuple(util::multiplyUintMod(e1, factor1, plain_modulus), e1,
                         e2);
}

[[maybe_unused]] void printDeviceArray(const DeviceArray<uint64_t> &r,
...
[[maybe_unused]] void printDeviceArray(const uint64_t *r, size_t count,
                                       bool dont_compress = false) {
...
void EvaluatorCuda::negateInplace(CiphertextCuda &encrypted) const {
  ...
  kernel_util::kNegatePolyCoeffmod(
      encrypted.data(), encrypted_size, coeff_modulus.size(),
      parms.polyModulusDegree(), coeff_modulus, encrypted.data());
}

void EvaluatorCuda::addInplace(CiphertextCuda &encrypted1,
                               const CiphertextCuda &encrypted2) const {
  encrypted1.seed() = 0;
  ...
  if (encrypted1.correctionFactor() != encrypted2.correctionFactor()) {
    ...
    kernel_util::kMultiplyPolyScalarCoeffmod(
        encrypted1.data(), encrypted1.size(), coeff_modulus_size, coeff_count,
        std::get<1>(factors), coeff_modulus.asPointer(), encrypted1.data());

    CiphertextCuda &encrypted2_copy = temp_ciphertext_;
    encrypted2_copy = encrypted2;
    kernel_util::kMultiplyPolyScalarCoeffmod(
        encrypted2.data(), encrypted2.size(), coeff_modulus_size, coeff_count,
        std::get<2>(factors), coeff_modulus.asPointer(),
        encrypted2_copy.data());
    ...
    // Add ciphertexts
    kernel_util::kAddPolyCoeffmod(encrypted1.data(), encrypted2.data(),
                                  min_count, coeff_modulus_size, coeff_count,
    ...
    if (encrypted1_size < encrypted2_size) {
      kernel_util::kSetPolyArray(
          encrypted2.data(min_count), encrypted2_size - encrypted1_size,
          coeff_count, coeff_modulus_size, encrypted1.data(encrypted1_size));
    }
  }
}

void EvaluatorCuda::subInplace(CiphertextCuda &encrypted1,
                               const CiphertextCuda &encrypted2) const {
  encrypted1.seed() = 0;
  ...
  if (encrypted1.correctionFactor() != encrypted2.correctionFactor()) {
    ...
    encrypted1.resize(context_, context_data.parmsID(), max_count);
    // Add ciphertexts
    kernel_util::kSubPolyCoeffmod(encrypted1.data(), encrypted2.data(),
                                  min_count, coeff_modulus_size, coeff_count,
                                  coeff_modulus.asPointer(), encrypted1.data());
    ...
    if (encrypted1_size < encrypted2_size) {
      kernel_util::kNegatePolyCoeffmod(
          encrypted2.data(min_count), encrypted2_size - encrypted1_size,
          coeff_count, coeff_modulus_size, coeff_modulus,
          encrypted1.data(encrypted1_size));
    }
  }
}

void EvaluatorCuda::multiplyInplace(CiphertextCuda &encrypted1,
                                    const CiphertextCuda &encrypted2) const {
  encrypted1.seed() = 0;
...
void EvaluatorCuda::bfvMultiply(CiphertextCuda &encrypted1,
                                const CiphertextCuda &encrypted2) const {
  ...
  auto encrypted1_q =
      encrypted1_q_.ensure(encrypted1_size * coeff_count * base_q_size);
  auto encrypted1_Bsk =
      encrypted1_Bsk_.ensure(encrypted1_size * coeff_count * base_Bsk_size);

  auto encrypted2_q =
      encrypted2_q_.ensure(encrypted2_size * coeff_count * base_q_size);
  auto encrypted2_Bsk =
      encrypted2_Bsk_.ensure(encrypted2_size * coeff_count * base_Bsk_size);

  auto temp =
      temp_.ensure(encrypted1_size * coeff_count * base_Bsk_m_tilde_size);
  kernel_util::kSetPolyArray(encrypted1.data(), encrypted1_size, base_q_size,
                             coeff_count, encrypted1_q.get());
  kernel_util::kNttNegacyclicHarveyLazy(encrypted1_q.get(), encrypted1_size,
                                        base_q_size, coeff_power,
                                        base_q_ntt_tables);
  for (size_t i = 0; i < encrypted1_size; i++) {
    rns_tool->fastbconvmTilde(
        encrypted1.data(i),
        temp.get() + i * coeff_count * base_Bsk_m_tilde_size);
    rns_tool->smMrq(temp.get() + i * coeff_count * base_Bsk_m_tilde_size,
                    encrypted1_Bsk.get() + i * coeff_count * base_Bsk_size);
  }
  kernel_util::kNttNegacyclicHarveyLazy(encrypted1_Bsk.get(), encrypted1_size,
                                        base_Bsk_size, coeff_power,
                                        base_Bsk_ntt_tables);

  temp = temp_.ensure(encrypted2_size * coeff_count * base_Bsk_m_tilde_size);
  kernel_util::kSetPolyArray(encrypted2.data(), encrypted2_size, base_q_size,
                             coeff_count, encrypted2_q.get());
  kernel_util::kNttNegacyclicHarveyLazy(encrypted2_q.get(), encrypted2_size,
                                        base_q_size, coeff_power,
                                        base_q_ntt_tables);
  ...
  kernel_util::kNttNegacyclicHarveyLazy(encrypted2_Bsk.get(), encrypted2_size,
                                        base_Bsk_size, coeff_power,
                                        base_Bsk_ntt_tables);

  auto temp_dest_q = temp_dest_q_.ensure(dest_size * coeff_count * base_q_size);
  auto temp_dest_Bsk =
      temp_dest_Bsk_.ensure(dest_size * coeff_count * base_Bsk_size);
  KernelProvider::memsetZero(temp_dest_q.get(),
                             dest_size * coeff_count * base_q_size);
  KernelProvider::memsetZero(temp_dest_Bsk.get(),
                             dest_size * coeff_count * base_Bsk_size);
  ...
  for (size_t i = 0; i < dest_size; i++) {
...

    {
      ...
      kernel_util::kDyadicConvolutionCoeffmod(
          shifted_in1_iter, shifted_reversed_in2_iter, steps, base_q_size,
          coeff_count, base_q, shifted_out_iter);
    }
    {
      ...
      kernel_util::kDyadicConvolutionCoeffmod(
          shifted_in1_iter, shifted_reversed_in2_iter, steps, base_Bsk_size,
          coeff_count, base_Bsk, shifted_out_iter);
    }
  }

  kernel_util::kInverseNttNegacyclicHarveyLazy(temp_dest_q.get(), dest_size,
                                               base_q_size, coeff_power,
                                               base_q_ntt_tables);
  kernel_util::kInverseNttNegacyclicHarveyLazy(temp_dest_Bsk.get(), dest_size,
                                               base_Bsk_size, coeff_power,
                                               base_Bsk_ntt_tables);

  auto temp_q_Bsk =
      temp_q_Bsk_.ensure(coeff_count * (base_q_size + base_Bsk_size));
  auto temp_Bsk = temp_Bsk_.ensure(coeff_count * base_Bsk_size);
  for (size_t i = 0; i < dest_size; i++) {
    kernel_util::kMultiplyPolyScalarCoeffmod(
        temp_dest_q + i * coeff_count * base_q_size, 1, base_q_size,
        coeff_count, plain_modulus, base_q, temp_q_Bsk);
    kernel_util::kMultiplyPolyScalarCoeffmod(
        temp_dest_Bsk + i * coeff_count * base_Bsk_size, 1, base_Bsk_size,
        coeff_count, plain_modulus, base_Bsk,
...
void EvaluatorCuda::ckksMultiply(CiphertextCuda &encrypted1,
                                 const CiphertextCuda &encrypted2) const {
  if (!(encrypted1.isNttForm() && encrypted2.isNttForm()))
  ...
  KernelProvider::memsetZero(temp.get(),
                             dest_size * coeff_count * coeff_modulus_size);
  ...
  kernel_util::kSetPolyArray(temp, dest_size, coeff_modulus_size, coeff_count,
                             encrypted1.data());
...
void EvaluatorCuda::bgvMultiply(CiphertextCuda &encrypted1,
                                const CiphertextCuda &encrypted2) const {
  if (encrypted1.isNttForm() || encrypted2.isNttForm()) {
    throw std::invalid_argument(
        "encryped1 or encrypted2 must be not in NTT form");
  }
  ...
  kernel_util::kNttNegacyclicHarvey(encrypted1.data(), encrypted1_size,
                                    coeff_modulus_size, coeff_power, ntt_table);
  ...
  if (&encrypted1 == &encrypted2) {
    ...
    kernel_util::kNttNegacyclicHarvey(encrypted2_cpy.data(), encrypted2_size,
                                      coeff_modulus_size, coeff_power,
                                      ntt_table);
  ...
  kernel_util::kSetPolyArray(temp.get(), dest_size, coeff_modulus_size,
                             coeff_count, encrypted1.data());
  kernel_util::kInverseNttNegacyclicHarvey(encrypted1.data(), encrypted1.size(),
                                           coeff_modulus_size, coeff_power,
                                           ntt_table);
...
void EvaluatorCuda::bfvSquare(CiphertextCuda &encrypted) const {
  ...
  auto encrypted_q =
      encrypted1_q_.ensure(encrypted_size * coeff_count * base_q_size);
  auto encrypted_Bsk =
      encrypted1_Bsk_.ensure(encrypted_size * coeff_count * base_Bsk_size);

  auto temp = temp_.ensure(coeff_count * base_Bsk_m_tilde_size);
  for (size_t i = 0; i < encrypted_size; i++) {
    kernel_util::kSetPolyArray(
        encrypted.data(i), 1, base_q_size, coeff_count,
        encrypted_q.get() + i * coeff_count * base_q_size);
    kernel_util::kNttNegacyclicHarveyLazy(
        encrypted_q.get() + i * coeff_count * base_q_size, 1, base_q_size,
        coeff_power, base_q_ntt_tables);
    rns_tool->fastbconvmTilde(encrypted.data(i), temp.get());
    rns_tool->smMrq(temp.get(),
                    encrypted_Bsk.get() + i * coeff_count * base_Bsk_size);
    kernel_util::kNttNegacyclicHarveyLazy(
        encrypted_Bsk.get() + i * coeff_count * base_Bsk_size, 1, base_Bsk_size,
        coeff_power, base_Bsk_ntt_tables);
  }
  ...
  kernel_util::kDyadicSquareCoeffmod(encrypted_q, base_q_size, coeff_count,
                                     base_q, temp_dest_q);
  kernel_util::kDyadicSquareCoeffmod(encrypted_Bsk, base_Bsk_size, coeff_count,
                                     base_Bsk, temp_dest_Bsk);
...
void EvaluatorCuda::ckksSquare(CiphertextCuda &encrypted) const {
  ...
  // Compute c1^2
  kernel_util::kDyadicSquareCoeffmod(encrypted.data(), coeff_modulus_size,
...
void EvaluatorCuda::bgvSquare(CiphertextCuda &encrypted) const {
  ...
  // Convert c0 and c1 to ntt
  kernel_util::kNttNegacyclicHarvey(encrypted.data(), encrypted_size,
                                    coeff_modulus_size, coeff_power, ntt_table);
  ...
  kernel_util::kDyadicSquareCoeffmod(encrypted_iter.get(), coeff_modulus_size,
                                     coeff_count, coeff_modulus, temp);

  // Set the final result
  kernel_util::kSetPolyArray(temp.get(), dest_size, coeff_count,
                             coeff_modulus_size, encrypted.data());

  // Convert the final output to Non-NTT form
  kernel_util::kInverseNttNegacyclicHarvey(
      encrypted.data(), dest_size, coeff_modulus_size, coeff_power, ntt_table);
...
void EvaluatorCuda::relinearizeInternal(CiphertextCuda &encrypted,
                                        const RelinKeysCuda &relin_keys,
  ...
  if (destination_size == encrypted_size) return;
  ...
  for (size_t i = 0; i < relins_needed; i++) {
    ...
    this->switchKeyInplace(encrypted, encrypted_iter,
                           static_cast<const KSwitchKeysCuda &>(relin_keys),
  ...
  // std::cout << "relin internal size after = " << encrypted.dynArray().size()
  // << std::endl;
}

void EvaluatorCuda::modSwitchScaleToNext(const CiphertextCuda &encrypted,
  ...
  for (size_t i = 0; i < encrypted_size; i++) {
    kernel_util::kSetPolyArray(encrypted_copy.data(i), 1,
...
void EvaluatorCuda::modSwitchDropToNext(const CiphertextCuda &encrypted,
                                        CiphertextCuda &destination) const {
  destination.seed() = 0;
  // Assuming at this point encrypted is already validated.
  auto context_data_ptr = context_.getContextData(encrypted.parmsID());
  ...
  if (&encrypted == &destination) {
    ...
    auto temp =
        temp_.ensure(encrypted_size * coeff_count * next_coeff_modulus_size);

    // Copy data over to temp; only copy the RNS components relevant after
    // modulus drop
    for (size_t i = 0; i < encrypted_size; i++) {
      kernel_util::kSetPolyArray(
          encrypted.data(i), 1, next_coeff_modulus_size, coeff_count,
    ...
    kernel_util::kSetPolyArray(temp, encrypted_size, coeff_count,
    ...
    // after modulus drop
    for (size_t i = 0; i < encrypted_size; i++) {
      kernel_util::kSetPolyArray(encrypted.data(i), 1, next_coeff_modulus_size,
                                 coeff_count, destination.data(i));
    }
  }
...
void EvaluatorCuda::modSwitchToNext(const CiphertextCuda &encrypted,
                                    CiphertextCuda &destination) const {
  destination.seed() = 0;
...
void EvaluatorCuda::modSwitchToInplace(CiphertextCuda &encrypted,
...
void EvaluatorCuda::modSwitchToInplace(PlaintextCuda &plain,
                                       ParmsID parms_id) const {
  // Verify parameters.
  auto context_data_ptr = context_.getContextData(plain.parmsID());
  auto targetContextData_ptr = context_.getContextData(parms_id);
...
__global__ void gSwitchKeyInplaceUtilA(
    uint64_t *t_poly_lazy, size_t coeff_count, size_t key_component_count,
    const uint64_t *key_vector_j, size_t key_poly_coeff_size,
    const uint64_t *t_operand, size_t key_index, const Modulus *key_modulus) {
  ...
  FOR_N(k, key_component_count) {
    ...
    kernel_util::dMultiplyUint64(
        t_operand[gindex], key_vector_j_k[key_index * coeff_count + gindex],
        qword);
    auto accumulator_l = t_poly_lazy + k * coeff_count * 2 + 2 * gindex;
    kernel_util::dAddUint128(qword, accumulator_l, qword);
    accumulator_l[0] =
        kernel_util::dBarrettReduce128(qword, key_modulus[key_index]);
    accumulator_l[1] = 0;
  }
}
...
__global__ void gSwitchKeyInplaceUtilC(const uint64_t *t_poly_lazy,
                                       size_t coeff_count,
  ...
  FOR_N(k, key_component_count) {
    ...
    t_poly_prod_iter[k * coeff_count * rns_modulus_size + gindex] =
        static_cast<uint64_t>(accumulator[gindex * 2]);
  }
}

__global__ void gSwitchKeyInplaceUtilD(
    const uint64_t *t_poly_lazy, size_t coeff_count, size_t key_component_count,
    size_t rns_modulus_size, uint64_t *t_poly_prod_iter, size_t key_index,
    const Modulus *key_modulus) {
  GET_INDEX_COND_RETURN(coeff_count);
  FOR_N(k, key_component_count) {
    const uint64_t *accumulator = t_poly_lazy + k * coeff_count * 2;
    t_poly_prod_iter[k * coeff_count * rns_modulus_size + gindex] =
        kernel_util::dBarrettReduce128(accumulator + gindex * 2,
                                       key_modulus[key_index]);
  }
}

__global__ void gSwitchKeyInplaceUtilE(
    const uint64_t *t_last, uint64_t *t_poly_prod_i, size_t coeff_count,
    const Modulus *plain_modulus, const Modulus *key_modulus,
    size_t decomp_modulus_size, size_t rns_modulus_size, uint64_t qk_inv_qp,
    uint64_t qk, const MultiplyUIntModOperand *modswitch_factors,
  ...
  FOR_N(j, decomp_modulus_size) {
    ...
    target = kernel_util::dMultiplyUintMod(target, modswitch_factors[j],
                                           key_modulus[j]);
    encrypted_i[j * coeff_count + gindex] = kernel_util::dAddUintMod(
        target, encrypted_i[j * coeff_count + gindex], key_modulus[j]);
  }
}

__global__ void gSwitchKeyInplaceUtilF(
    uint64_t *t_last, size_t coeff_count, const Modulus *qk,
    const Modulus *key_modulus, uint64_t qk_half, size_t decomp_modulus_size,
    uint64_t
        *t_ntt  // t_ntt should be at least coeff_count * decomp_modulus_size
) {
  GET_INDEX_COND_RETURN(coeff_count);
  t_last[gindex] = kernel_util::dBarrettReduce64(t_last[gindex] + qk_half, *qk);
  FOR_N(j, decomp_modulus_size) {
    const Modulus &qi = key_modulus[j];
    if (DeviceHelper::getModulusValue(*qk) >
        DeviceHelper::getModulusValue(qi)) {
      t_ntt[j * coeff_count + gindex] =
          kernel_util::dBarrettReduce64(t_last[gindex], qi);
    } else {
      t_ntt[j * coeff_count + gindex] = t_last[gindex];
    }
    uint64_t fix = DeviceHelper::getModulusValue(qi) -
                   kernel_util::dBarrettReduce64(qk_half, key_modulus[j]);
    t_ntt[j * coeff_count + gindex] += fix;
  }
}

__global__ void gSwitchKeyInplaceUtilG(
    uint64_t *t_poly_prod_i, const uint64_t *t_ntt, size_t coeff_count,
    uint64_t *encrypted_i, bool is_ckks, size_t decomp_modulus_size,
    const Modulus *key_modulus,
  ...
  FOR_N(j, decomp_modulus_size) {
    ...
    dest +=
        ((is_ckks) ? (qi << 2) : (qi << 1)) - t_ntt[j * coeff_count + gindex];
    dest = kernel_util::dMultiplyUintMod(dest, modswitch_factors[j],
                                         key_modulus[j]);
    encrypted_i[j * coeff_count + gindex] = kernel_util::dAddUintMod(
...
void EvaluatorCuda::switchKeyInplace(CiphertextCuda &encrypted,
                                     ConstDevicePointer<uint64_t> target_iter,
                                     const KSwitchKeysCuda &kswitch_keys,
  ...
  // KernelProvider::copy(plain_modulus_cuda_support, &(parms.plainModulus()),
  // 1);
  ...
  auto t_target =
      switch_key_t_target_.ensure(coeff_count * decomp_modulus_size);
  kernel_util::kSetPolyArray(target_iter.get(), 1, decomp_modulus_size,
                             coeff_count, t_target.get());

  // std::cout << "t_target: "; printDeviceArray(t_target);

  // In CKKS t_target is in NTT form; switch back to normal form
  if (scheme == SchemeType::ckks)
    kernel_util::kInverseNttNegacyclicHarvey(t_target, 1, decomp_modulus_size,
                                             coeff_power, key_ntt_tables);

  // Temporary result
  auto t_poly_prod = switch_key_t_poly_prod_.ensure(
      key_component_count * coeff_count * rns_modulus_size);
  KernelProvider::memsetZero(
      t_poly_prod.get(), key_component_count * coeff_count * rns_modulus_size);
  ...
  for (size_t i = 0; i < rns_modulus_size; i++) {
    ...
    auto t_poly_lazy =
        switch_key_t_poly_lazy_.ensure(key_component_count * coeff_count * 2);
    KernelProvider::memsetZero(t_poly_lazy.get(),
    ...
    for (size_t j = 0; j < decomp_modulus_size; j++) {
      ...
      if ((scheme == SchemeType::ckks) && (i == j))
        ...
        if (key_modulus_cpu[j] <= key_modulus_cpu[key_index]) {
          // printf("branch a - ");
          kernel_util::kSetPolyArray(t_target.get() + j * coeff_count, 1, 1,
                                     coeff_count, t_ntt.get());
        } else {
          // printf("branch b - ");
          kernel_util::kModuloPolyCoeffs(t_target.get() + j * coeff_count, 1, 1,
                                         coeff_count, key_modulus + key_index,
                                         t_ntt.get());
        }
        // printDeviceArray(t_ntt.get(), coeff_count);
        kernel_util::kNttNegacyclicHarveyLazy(t_ntt.get(), 1, 1, coeff_power,
                                              key_ntt_tables + key_index);
        t_operand = t_ntt.get();
      }

      // std::cout << "  j = " << j << std::endl;
      // std::cout << "  t_operand: "; printDeviceArray(t_operand.get(),
      // coeff_count);
      ...
      if (!lazy_reduction_counter) {
        ...
        (t_poly_lazy.get(), coeff_count, key_component_count,
         key_vector[j].data().data().get(), key_vector_poly_coeff_size,
    ...
    if (lazy_reduction_counter == lazy_reduction_summand_bound) {
      ...
      (t_poly_lazy.get(), coeff_count, key_component_count, rns_modulus_size,
       t_poly_prod_iter);
    } else {
      KERNEL_CALL(gSwitchKeyInplaceUtilD, coeff_count)
      (t_poly_lazy.get(), coeff_count, key_component_count, rns_modulus_size,
       t_poly_prod_iter, key_index, key_modulus.get());
    }
  }
  ...
  for (size_t i = 0; i < key_component_count; i++) {
    if (scheme == SchemeType::bgv) {
      ...
      kernel_util::kInverseNttNegacyclicHarvey(
          t_last, 1, 1, coeff_power, key_ntt_tables + (key_modulus_size - 1));
      kernel_util::kInverseNttNegacyclicHarvey(
          t_poly_prod + i * coeff_count * rns_modulus_size, 1,
          decomp_modulus_size, coeff_power, key_ntt_tables);
      ...
      (t_last.get(), t_poly_prod.get() + i * coeff_count * rns_modulus_size,
       ...
       decomp_modulus_size, rns_modulus_size, qk_inv_qp, qk, modswitch_factors,
       encrypted.data(i).get());
    } else {
      // Lazy reduction; this needs to be then reduced mod qi
      auto t_last = t_poly_prod + coeff_count * rns_modulus_size * i +
                    decomp_modulus_size * coeff_count;
      ...
      KernelProvider::memsetZero(t_ntt.get(),
                                 decomp_modulus_size * coeff_count);
      ...
      (t_last.get(), coeff_count, key_modulus.get() + (key_modulus_size - 1),
       ...
       decomp_modulus_size, t_ntt.get());

      if (scheme == SchemeType::ckks)
        kernel_util::kNttNegacyclicHarveyLazy(t_ntt, 1, decomp_modulus_size,
                                              coeff_power, key_ntt_tables);
      ...
      //     std::cout << "  t_ptr: " << j << " - ";
      //     printDeviceArray(t_poly_prod.get() + i * coeff_count *
      //     rns_modulus_size + j * coeff_count, coeff_count);
      // }

      gSwitchKeyInplaceUtilG<<<block_count, 256>>>(
          t_poly_prod.get() + i * coeff_count * rns_modulus_size, t_ntt.get(),
          coeff_count, encrypted.data(i).get(), scheme == SchemeType::ckks,
          decomp_modulus_size, key_modulus.get(), modswitch_factors);
    }
    // printf("enc %ld: ", i); printDeviceArray(encrypted.data(i).get(),
    // key_component_count * coeff_count);
  }
}
...
void EvaluatorCuda::applyKeySwitchingInplace(
  ...
  kernel_util::kSetPolyArray(encrypted.data(1), 1, encrypted.coeffModulusSize(),
                             encrypted.polyModulusDegree(), target.get());
  kernel_util::kSetZeroPolyArray(1, encrypted.coeffModulusSize(),
                                 encrypted.polyModulusDegree(),
                                 encrypted.data(1));
  switchKeyInplace(encrypted, target, kswitch_keys, 0);
}

void EvaluatorCuda::rescaleToNext(const CiphertextCuda &encrypted,
                                  CiphertextCuda &destination) const {
  destination.seed() = 0;
  if (context_.lastParmsID() == encrypted.parmsID()) {
    throw invalid_argument("end of modulus switching chain reached");
...
void EvaluatorCuda::rescaleToInplace(CiphertextCuda &encrypted,
                                     ParmsID parms_id) const {
  encrypted.seed() = 0;
...
void EvaluatorCuda::multiplyMany(const std::vector<CiphertextCuda> &encrypteds,
                                 const RelinKeysCuda &relin_keys,
                                 CiphertextCuda &destination) const {
  destination.seed() = 0;
  // Verify parameters.
  ...
  for (size_t i = 0; i < encrypteds.size() - 1; i += 2) {
    ...
    if (encrypteds[i].data().get() == encrypteds[i + 1].data().get()) {
      // printf("square %lld %lld\n", encrypteds[i].data().get(), encrypteds[i +
      // 1].data().get());
...
void EvaluatorCuda::exponentiateInplace(CiphertextCuda &encrypted,
                                        uint64_t exponent,
                                        const RelinKeysCuda &relin_keys) const {
  encrypted.seed() = 0;
  // Verify parameters.
  auto context_data_ptr = context_.getContextData(encrypted.parmsID());
  if (!context_data_ptr) {
  ...
  std::vector<CiphertextCuda> exp_vector(static_cast<size_t>(exponent),
                                         encrypted);
  multiplyMany(exp_vector, relin_keys, encrypted);
}

void EvaluatorCuda::addPlainInplace(CiphertextCuda &encrypted,
  ...
  switch (parms.scheme()) {
    ...
    case SchemeType::ckks: {
      ...
      kernel_util::kAddPolyCoeffmod(encrypted_iter, plain_iter, 1,
                                    coeff_modulus_size, coeff_count,
                                    coeff_modulus, encrypted_iter);
      break;
    }
    ...
    case SchemeType::bgv: {
      ...
      kernel_util::kMultiplyPolyScalarCoeffmod(
          plain.data(), 1, 1, plain.coeffCount(), encrypted.correctionFactor(),
...
void EvaluatorCuda::subPlainInplace(CiphertextCuda &encrypted,
                                    const PlaintextCuda &plain) const {
  encrypted.seed() = 0;
  ...
  switch (parms.scheme()) {
    ...
    case SchemeType::ckks: {
      DevicePointer encrypted_iter(encrypted.data());
      ConstDevicePointer plain_iter(plain.data());
      kernel_util::kSubPolyCoeffmod(encrypted_iter, plain_iter, 1,
                                    coeff_modulus_size, coeff_count,
                                    coeff_modulus, encrypted_iter);
      break;
    }
...
void EvaluatorCuda::multiplyPlainInplace(CiphertextCuda &encrypted,
                                         const PlaintextCuda &plain) const {
  encrypted.seed() = 0;
...
__global__ void gMultiplyPlainNormalUtilA(
    const uint64_t *plain_data, size_t plain_coeff_count,
    const uint64_t *plain_upper_half_increment,
    uint64_t plain_upper_half_threshold, size_t coeff_modulus_size,
    uint64_t *temp) {
  GET_INDEX_COND_RETURN(plain_coeff_count);
  uint64_t plain_value = plain_data[gindex];
  if (plain_value >= plain_upper_half_threshold) {
    kernel_util::dAddUint(plain_upper_half_increment, coeff_modulus_size,
                          plain_value, temp + coeff_modulus_size * gindex);
  } else {
...
__global__ void gMultiplyPlainNormalUtilB(
    const uint64_t *plain_data, size_t plain_coeff_count, size_t coeff_count,
    const uint64_t *plain_upper_half_increment,
    uint64_t plain_upper_half_threshold, size_t coeff_modulus_size,
    uint64_t *temp) {
  GET_INDEX_COND_RETURN(plain_coeff_count);
  ...
  FOR_N(i, coeff_modulus_size) {
    temp[i * coeff_count + gindex] =
        plain_value >= plain_upper_half_threshold
            ? (plain_value + plain_upper_half_increment[i])
            : (plain_value);
  }
}

void EvaluatorCuda::multiplyPlainNormal(CiphertextCuda &encrypted,
                                        const PlaintextCuda &plain) const {
  encrypted.seed() = 0;
  // Extract encryption parameters.
  auto &context_data = *context_.getContextData(encrypted.parmsID());
  ...
  if (!context_data.qualifiers().using_fast_plain_lift) {
    ...
    (plain.data(), plain_coeff_count, coeff_count,
     plain_upper_half_increment.get(), plain_upper_half_threshold,
     coeff_modulus_size, temp.get());
  }
  ...
  kernel_util::kNttNegacyclicHarvey(temp.get(), 1, coeff_modulus_size,
                                    coeff_power, ntt_tables);

  for (size_t i = 0; i < encrypted_size; i++) {
    auto target_ptr = encrypted.data(i);
    kernel_util::kNttNegacyclicHarveyLazy(target_ptr, 1, coeff_modulus_size,
                                          coeff_power, ntt_tables);
    kernel_util::kDyadicProductCoeffmod(target_ptr, temp, 1, coeff_modulus_size,
                                        coeff_count, coeff_modulus, target_ptr);
    kernel_util::kInverseNttNegacyclicHarveyLazy(
        target_ptr, 1, coeff_modulus_size, coeff_power, ntt_tables);
  }
...
void EvaluatorCuda::multiplyPlainNtt(CiphertextCuda &encrypted_ntt,
                                     const PlaintextCuda &plain_ntt) const {
  ...
  for (size_t i = 0; i < encrypted_ntt_size; i++) {
    kernel_util::kDyadicProductCoeffmod(encrypted_ntt.data(i), plain_ntt_iter,
                                        1, coeff_modulus_size, coeff_count,
                                        coeff_modulus, encrypted_ntt.data(i));
  }
...
__global__ void gTransformToNttInplace(
    uint64_t *plain, size_t plain_coeff_count, size_t coeff_count,
    const uint64_t *plain_upper_half_increment,
    uint64_t plain_upper_half_threshold, size_t coeff_modulus_size) {
  GET_INDEX_COND_RETURN(plain_coeff_count);
  FOR_N(i, coeff_modulus_size) {
    ...
    plain[plain_index] =
        (plain[gindex] >= plain_upper_half_threshold)
            ? (plain[gindex] + plain_upper_half_increment[increment_index])
            : plain[gindex];
  }
}

void EvaluatorCuda::transformToNttInplace(PlaintextCuda &plain,
                                          ParmsID parms_id) const {
  ...
  if (!context_data.qualifiers().using_fast_plain_lift) {
    ...
    kernel_util::kSetPolyArray(temp.get(), 1, coeff_count, coeff_modulus_size,
                               plain.data());
  } else {
  ...
  kernel_util::kNttNegacyclicHarvey(plain.data(), 1, coeff_modulus_size,
                                    getPowerOfTwo(coeff_count), ntt_tables);
...
void EvaluatorCuda::transformFromNttInplace(
    CiphertextCuda &encrypted_ntt) const {
  encrypted_ntt.seed() = 0;
  ...
  kernel_util::kInverseNttNegacyclicHarvey(
      encrypted_ntt.data(), encrypted_ntt_size, coeff_modulus_size,
      getPowerOfTwo(coeff_count), ntt_tables);
...
void EvaluatorCuda::applyGaloisInplace(
    CiphertextCuda &encrypted, uint32_t galois_elt,
    const GaloisKeysCuda &galois_keys) const {
  encrypted.seed() = 0;
  ...
  if (parms.scheme() == SchemeType::bfv || parms.scheme() == SchemeType::bgv) {
    ...
    galois_tool->applyGalois(encrypted.data(0), 1, coeff_modulus_size,
                             galois_elt, coeff_modulus.asPointer(), temp);

    // Copy result to encrypted.data(0)
    kernel_util::kSetPolyArray(temp, 1, coeff_count, coeff_modulus_size,
                               encrypted.data(0));

    // Next transform encrypted.data(1)
    galois_tool->applyGalois(encrypted.data(1), 1, coeff_modulus_size,
                             galois_elt, coeff_modulus.asPointer(), temp);
    ...
    // First transform encrypted.data(0)
    // auto encrypted_iter = iter(encrypted);
    galois_tool->applyGaloisNtt(encrypted.data(0), 1, coeff_modulus_size,
                                galois_elt, temp);
    ...
    // Next transform encrypted.data(1)
    galois_tool->applyGaloisNtt(encrypted.data(1), 1, coeff_modulus_size,
                                galois_elt, temp);
  } else {
  ...
  kernel_util::kSetZeroPolyArray(1, coeff_modulus_size, coeff_count,
                                 encrypted.data(1));
  ...
  switchKeyInplace(encrypted, temp,
                   static_cast<const KSwitchKeysCuda &>(galois_keys),
                   GaloisKeys::getIndex(galois_elt));
}

void EvaluatorCuda::rotateInternal(CiphertextCuda &encrypted, int steps,
                                   const GaloisKeysCuda &galois_keys) const {
  encrypted.seed() = 0;
  auto context_data_ptr = context_.getContextData(encrypted.parmsID());
  if (!context_data_ptr) {
...
__global__ void assembleLWECopyC0(const uint64_t *c0, uint64_t *data,
                                  size_t offset, size_t coeff_count,
...
CiphertextCuda EvaluatorCuda::assembleLWE(const LWECiphertextCuda &encrypted,
                                          size_t term) const {
  auto context_data = context_.getContextData(encrypted.parmsID());
  ...
  kernel_util::kNegacyclicShiftPolyCoeffmod(
      encrypted.c1().get(), 1, coeff_modulus_size, poly_modulus_degree, term,
      coeff_modulus.asPointer(), data.get() + poly_len);
  // copy c0
  KERNEL_CALL(assembleLWECopyC0, coeff_modulus_size)
  (encrypted.c0().get(), data.get(), term, poly_modulus_degree,
   coeff_modulus_size);
  return CiphertextCuda::fromMembers(
      encrypted.parmsID(), 2, coeff_modulus_size, poly_modulus_degree, false,
      encrypted.scale(), encrypted.correctionFactor(),
      util::DeviceDynamicArray(std::move(data)));
}

__global__ void extractLWECopyC0(const uint64_t *data, size_t offset,
                                 uint64_t *c0, size_t coeff_count,
                                 size_t coeff_modulus_size) {
  GET_INDEX_COND_RETURN(coeff_modulus_size);
  if (gindex < coeff_modulus_size) {
...
LWECiphertextCuda EvaluatorCuda::extractLWE(const CiphertextCuda &encrypted,
                                            size_t term) const {
  if (encrypted.size() != 2) {
  ...
  if (encrypted.isNttForm()) {
    ...
    auto c1 =
        kernel_util::kAllocateZero(poly_modulus_degree, coeff_modulus_size);
    kernel_util::kNegacyclicShiftPolyCoeffmod(
        encrypted.data(1), 1, coeff_modulus_size, poly_modulus_degree, shift,
        coeff_modulus, c1);
    // gather c0
    auto c0 = kernel_util::kAllocateZero(coeff_modulus_size);
    KERNEL_CALL(extractLWECopyC0, coeff_modulus_size)
    (encrypted.data(0).get(), term, c0.get(), poly_modulus_degree,
     coeff_modulus_size);
    return LWECiphertextCuda::fromMembers(
        encrypted.parmsID(), poly_modulus_degree, coeff_modulus_size,
        encrypted.scale(), encrypted.correctionFactor(), std::move(c1),
        std::move(c0));
  }
}

void EvaluatorCuda::fieldTraceInplace(CiphertextCuda &encrypted,
                                      const GaloisKeysCuda &automorphism_keys,
                                      size_t logn) const {
...
void EvaluatorCuda::divideByPolyModulusDegreeInplace(CiphertextCuda &encrypted,
                                                     uint64_t mul) const {
  auto context_data = context_.getContextData(encrypted.parmsID());
  ...
  kernel_util::kMultiplyInvPolyDegreeCoeffmod(
      encrypted.data(), size, modulus.size(),
      context_data->parms().polyModulusDegree(), ntt_tables, modulus,
      encrypted.data());
  if (mul > 1) {
    kernel_util::kMultiplyPolyScalarCoeffmod(
        encrypted.data(), size, modulus.size(),
        context_data->parms().polyModulusDegree(), mul, modulus,
        encrypted.data());
  }
}

CiphertextCuda EvaluatorCuda::packLWECiphertexts(
    const std::vector<LWECiphertextCuda> &lwes,
    GaloisKeysCuda &automorphism_keys) const {
  ...
  kernel_util::kSetZeroPolyArray(2, coeff_modulus_size, poly_modulus_degree,
                                 zero_rlwe.data());
...
void EvaluatorCuda::negacyclicShift(const CiphertextCuda &encrypted,
                                    size_t shift,
                                    CiphertextCuda &destination) const {
  ...
  auto coeff_modulus = context_data->parms().coeffModulus();
  kernel_util::kNegacyclicShiftPolyCoeffmod(
      encrypted.data(), encrypted.size(), coeff_modulus_size,
      poly_modulus_degree, shift, coeff_modulus, destination.data());
}
