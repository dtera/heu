...
vector<Modulus> CoeffModulus::BFVDefault(size_t poly_modulus_degree,
  ...
  switch (sec_level) {
    case SecurityLevel::tc128:
      return util::global_variables::GetDefaultCoeffModulus128().at(
          poly_modulus_degree);

    case SecurityLevel::tc192:
      return util::global_variables::GetDefaultCoeffModulus192().at(
          poly_modulus_degree);

    case SecurityLevel::tc256:
      return util::global_variables::GetDefaultCoeffModulus256().at(
          poly_modulus_degree);
...
vector<Modulus> CoeffModulus::Create(size_t poly_modulus_degree,
  ...
  uint64_t factor = util::mul_safe(
      uint64_t(2), util::safe_cast<uint64_t>(poly_modulus_degree));
  for (const auto &table_elt : count_table) {
    prime_table[table_elt.first] =
        util::getPrimes(factor, table_elt.first, table_elt.second);
  }
