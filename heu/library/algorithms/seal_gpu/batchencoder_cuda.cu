...
BatchEncoderCuda::BatchEncoderCuda(const SEALContextCuda &context)
    : context_(context) {
...
__global__ void gEncodeUnsignedUtil(const uint64_t *values_matrix, size_t slots,
  ...
  destination[matrix_reps_index_map[gindex]] =
      (gindex < values_matrix_size) ? values_matrix[gindex] : 0;
}

void BatchEncoderCuda::encode(const vector<uint64_t> &values_matrix,
  ...
  KernelProvider::copy(values_matrix_device.get(), &values_matrix[0],
                       values_matrix_size);

  KERNEL_CALL(gEncodeUnsignedUtil, slots_)
  (values_matrix_device.get(), slots_, values_matrix_size, destination.data(),
   matrix_reps_index_map_.get());

  kernel_util::kInverseNttNegacyclicHarvey(destination.data(), 1, 1,
                                           getPowerOfTwo(slots_),
                                           context_data.plainNTTTables());
}

__global__ void gEncodeSignedUtil(const int64_t *values_matrix, size_t slots,
                                  size_t values_matrix_size,
                                  uint64_t *destination,
                                  const uint64_t *matrix_reps_index_map,
                                  uint64_t modulus) {
...
void BatchEncoderCuda::encode(const vector<int64_t> &values_matrix,
                              PlaintextCuda &destination) const {
  auto &context_data = *context_.firstContextData();
...
void BatchEncoderCuda::encodePolynomial(
...
void BatchEncoderCuda::encodePolynomial(const std::vector<std::int64_t> &values,
                                        PlaintextCuda &destination) const {
  auto &context_data = *context_.firstContextData();
  uint64_t modulus = context_data.parms().plainModulus().value();
  ...
  for (size_t i = 0; i < values_matrix_size; i++) {
    if (values[i] < 0)
      vs[i] = modulus - ((-values[i]) % modulus);
    else
      vs[i] = values[i] % modulus;
  }
  for (size_t i = values_matrix_size; i < slots_; i++) vs[i] = 0;
...
__global__ void gDecodeUnsignedUtil(const uint64_t *temp_dest, size_t slots,
                                    uint64_t *destination,
                                    const uint64_t *matrix_reps_index_map) {
  GET_INDEX_COND_RETURN(slots);
...
void BatchEncoderCuda::decode(const PlaintextCuda &plain,
                              vector<uint64_t> &destination) const {
  if (plain.isNttForm()) {
  ...
  kernel_util::kSetZeroPolyArray(1, 1, slots_ - plain_coeff_count,
                                 temp_dest + plain_coeff_count);

  // Transform destination using negacyclic NTT.
  kernel_util::kNttNegacyclicHarvey(temp_dest.get(), 1, 1,
                                    getPowerOfTwo(slots_),
                                    context_data.plainNTTTables());
  ...
  KernelProvider::retrieve(destination.data(), destination_device.get(),
                           slots_);
}

__global__ void gDecodeSignedUtil(const uint64_t *temp_dest, size_t slots,
                                  int64_t *destination,
                                  const uint64_t *matrix_reps_index_map,
                                  uint64_t modulus) {
  GET_INDEX_COND_RETURN(slots);
  ...
  destination[gindex] =
      (value > plain_modulus_div_two)
          ? (static_cast<int64_t>(value) - static_cast<int64_t>(modulus))
          : static_cast<int64_t>(value);
}

void BatchEncoderCuda::decode(const PlaintextCuda &plain,
                              vector<int64_t> &destination) const {
  if (plain.isNttForm()) {
    throw invalid_argument("plain cannot be in NTT form");
  }
...
void BatchEncoderCuda::decodePolynomial(
    const PlaintextCuda &plain, std::vector<std::uint64_t> &destination) const {
  size_t plain_coeff_count = min(plain.coeffCount(), slots_);
...
void BatchEncoderCuda::decodePolynomial(
    const PlaintextCuda &plain, std::vector<std::int64_t> &destination) const {
  destination.resize(slots_);
