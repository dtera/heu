...
namespace seal_gpu {
namespace {
void polyInftyNormCoeffmod(ConstHostPointer<uint64_t> poly,
                           size_t coeff_uint64_count, size_t coeff_count,
...
Decryptor::Decryptor(const SEALContext &context, const SecretKey &secret_key)
  ...
  setPoly(secret_key.data().data(), coeff_count, coeff_modulus_size,
...
void Decryptor::bfvDecrypt(const Ciphertext &encrypted,
...
void Decryptor::ckksDecrypt(const Ciphertext &encrypted,
                            Plaintext &destination) {
  if (!encrypted.isNttForm()) {
...
void Decryptor::bgvDecrypt(const Ciphertext &encrypted,
                           Plaintext &destination) {
  if (encrypted.isNttForm()) {
    throw invalid_argument("encrypted cannot be in NTT form");
  }
  ...
  if (encrypted.correctionFactor() != 1) {
    ...
    multiplyPolyScalarCoeffmod(ConstHostPointer(destination.data()),
                               coeff_count, fix, plain_modulus,
                               HostPointer(destination.data()));
  }
...
void Decryptor::computeSecretKeyArray(size_t max_power) {
  ...
  auto secret_key_array =
      (allocatePolyArray(new_size, coeff_count, coeff_modulus_size));
  setPolyArray(secret_key_array_.get(), old_size, coeff_count,
  ...
  for (size_t i = 0; i < new_size - old_size; i++) {
    dyadicProductCoeffmod(
        ...
        secret_key_array.asPointer(), coeff_modulus_size, coeff_count,
        &coeff_modulus[0],
...
void Decryptor::dotProductCtSkArray(const Ciphertext &encrypted,
  ...
  if (encrypted_size == 2) {
    ...
    if (isNttForm) {
      for (size_t i = 0; i < coeff_modulus_size; i++) {
        ...
        // iter(c0, c1, secret_key_array, coeff_modulus, destination),
        // coeff_modulus_size, [&](auto I) { put < c_1 * s > mod q in
        // destination
        dyadicProductCoeffmod(c1 + d, secret_key_array + d, coeff_count,
                              coeff_modulus[i], destination + d);
        // add c_0 to the result; note that destination should be in the same
        // (NTT) form as encrypted
        addPolyCoeffmod(destination + d, c0 + d, coeff_count, coeff_modulus[i],
                        destination + d);
      }
    } else {
      for (size_t i = 0; i < coeff_modulus_size; i++) {
        size_t d = i * coeff_count;
        // iter(c0, c1, secret_key_array, coeff_modulus, ntt_tables,
        // destination), coeff_modulus_size,
        ...
        dyadicProductCoeffmod(destination + d, secret_key_array + d,
                              coeff_count, coeff_modulus[i], destination + d);
        // Transform back
    ...
    // destination Now do the dot product of encrypted_copy and the secret key
    // array using NTT. The secret key powers are already NTT transformed.
    auto encrypted_copy = HostArray<uint64_t>((encrypted_size - 1) *
    ...
    setPolyArray(encrypted.data(1), encrypted_size - 1, coeff_count,
                 coeff_modulus_size, encrypted_copy.get());

    // Transform c_1, c_2, ... to NTT form unless they already are
    if (!isNttForm) {
      nttNegacyclicHarveyLazy(encrypted_copy.asPointer(), encrypted_size - 1,
                              coeff_modulus_size, ntt_tables);
    }

    // Compute dyadic product with secret power array
    for (size_t i = 0; i < encrypted_size - 1; i++) {
      dyadicProductCoeffmod(encrypted_copy + i * poly_coeff_count,
                            secret_key_array_ + i * key_poly_coeff_count,
    ...
    for (size_t i = 0; i < encrypted_size - 1; i++) {
      addPolyCoeffmod(destination, encrypted_copy + i * poly_coeff_count,
                      coeff_modulus_size, coeff_count, &coeff_modulus[0],
                      destination);
    }
    ...
    // Finally add c_0 to the result; note that destination should be in the
    // same (NTT) form as encrypted
    addPolyCoeffmod(destination, encrypted.data(), coeff_modulus_size,
                    coeff_count, &coeff_modulus[0], destination);
  }
}
...
int Decryptor::invariantNoiseBudget(const Ciphertext &encrypted) {
  ...
  // in destination_poly.
  // Now do the dot product of encrypted_copy and the secret key array using
  // NTT. The secret key powers are already NTT transformed.
  ...
  if (scheme == SchemeType::bfv) {
    multiplyPolyScalarCoeffmod(noise_poly.asPointer(), coeff_modulus_size,
                               coeff_count, plain_modulus.value(),
                               &coeff_modulus[0], noise_poly.asPointer());
  }
  ...
  // coeff_modulus_size);
  polyInftyNormCoeffmod(noise_poly.get(), coeff_modulus_size, coeff_count,
                        context_data.totalCoeffModulus(), norm.get());
