...
void random_bytes(std::byte *buf, size_t count) {
  ...
  NTSTATUS status =
      BCryptGenRandom(NULL, reinterpret_cast<unsigned char *>(buf),
                      safe_cast<ULONG>(count), BCRYPT_USE_SYSTEM_PREFERRED_RNG);
...
//         stream.write(reinterpret_cast<const char *>(&type_),
//         sizeof(PRNGType)); stream.write(reinterpret_cast<const char
//         *>(seed_.data()), prng_seed_byte_count);
//     }
...
//         // Read the seed data
//         stream.read(reinterpret_cast<char *>(info.seed_.data()),
...
shared_ptr<UniformRandomGenerator> UniformRandomGeneratorInfo::make_prng()
    const {
  switch (type_) {
...
void UniformRandomGenerator::generate(size_t byte_count,
...
auto UniformRandomGeneratorFactory::DefaultFactory()
    -> shared_ptr<UniformRandomGeneratorFactory> {
  static shared_ptr<UniformRandomGeneratorFactory> default_factory{
      new Blake2xbPRNGFactory()};
  return default_factory;
}

void Blake2xbPRNG::refill_buffer() {
  // Fill the randomness buffer
  if (blake2xb(buffer_.get(), buffer_size_, &counter_, sizeof(counter_),
               seed_.get(), seed_.length() * sizeof(uint64_t)) != 0) {
...
void Shake256PRNG::refill_buffer() {
  ...
  shake256(reinterpret_cast<uint8_t *>(buffer_.get()), buffer_size_,
           reinterpret_cast<const uint8_t *>(seed_ext.data()),
           seed_ext.size() * util::bytesPerUint64);
