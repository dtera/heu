...
class PlaintextCuda {
 public:
  ...
  explicit PlaintextCuda(std::size_t capacity, std::size_t coeff_count)
  ...
  PlaintextCuda(const std::string &hex_poly) : data_() { operator=(hex_poly); }
  ...
  */
  inline void setZero(std::size_t start_coeff) {
    ...
    KernelProvider::memsetZero<uint64_t>(data_.get() + start_coeff,
                                         data_.size() - start_coeff);
  }
  ...
  */
  inline pt_coeff_type *data() { return data_.get(); }

  /**
  Returns a const pointer to the beginning of the plaintext polynomial.
  */
  inline const pt_coeff_type *data() const { return data_.get(); }
  ...
  */
  ...
  //         return 0;
  //     }
  //     return util::getSignificantUint64CountUint(data_.cbegin(),
  //     coeff_count_);
  // }
  ...
  */
  ...
  //     if (isNttForm())
  //     {
  //         throw std::invalid_argument("cannot convert NTT transformed
  //         plaintext to string");
  //     }
