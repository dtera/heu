...
class KeyGeneratorCuda {
  ...
  */
  ...
  parameters
  */
  KeyGeneratorCuda(const SEALContextCuda &context,
                   const SecretKeyCuda &secret_key)
      : host(context.host(), secret_key.toHost()) {}

  /**
  Returns a const reference to the secret key.
  */
  SecretKeyCuda secretKey() const { return SecretKeyCuda(host.secretKey()); }
  ...
  */
  inline void createGaloisKeys(const std::vector<int> &steps,
                               GaloisKeysCuda &destination) {
    GaloisKeys ret;
    host.createGaloisKeys(steps, ret);
    destination = ret;
  }
  ...
  */
  ...
  //     assert(secret_key_.data().dynArray().size() ==
  //     secret_key_array_.size()); size_t n = secret_key_array_.size(); for
  //     (size_t i = 0; i < n; i++) {
