...
[[maybe_unused]] void printDeviceArray(const DeviceArray<complex<double>> &r,
                                       bool dont_compress = false) {
...
[[maybe_unused]] void printDeviceArray(const complex<double> *r, size_t count,
                                       bool dont_compress = false) {
...
CKKSEncoderCuda::CKKSEncoderCuda(const SEALContextCuda &context)
  ...
  for (size_t i = 0; i <= m; i++) {
    roots[i] = std::polar<double>(
        1.0, 2 * pi * static_cast<double>(i) / static_cast<double>(m));
  }

  complex_roots_ = roots;
}

__device__ inline void dSetComplex(double *array, size_t index, double real,
                                   double imag) {
  array[index * 2] = real;
  array[index * 2 + 1] = imag;
}

__global__ void gEncodeInternalSetConjValues(const double *copied_values,
                                             size_t values_size, size_t slots,
                                             double *conj_values,
...
__global__ void gFftTransferFromRevLayered(size_t layer, double *operand,
  ...
  size_t gap = 1 << layer;
  size_t rid =
      (1 << poly_modulus_degree_power) - (m << 1) + 1 + (gindex >> layer);
  size_t coeff_index =
      ((gindex >> layer) << (layer + 1)) + (gindex & (gap - 1));
...
__global__ void gMultiplyScalar(double *operand, size_t n, double scalar) {
  GET_INDEX_COND_RETURN(n);
  operand[gindex * 2] *= scalar;
  operand[gindex * 2 + 1] *= scalar;
}

void kFftTransferFromRev(complex<double> *operand,
                         size_t poly_modulus_degree_power,
                         const complex<double> *roots, double fix = 1) {
  std::size_t n = size_t(1) << poly_modulus_degree_power;
  std::size_t m = n >> 1;
  std::size_t layer = 0;
  for (; m >= 1; m >>= 1) {
    KERNEL_CALL(gFftTransferFromRevLayered, n >> 1)
    (layer, reinterpret_cast<double *>(operand), poly_modulus_degree_power,
...
__global__ void gMaxReal(double *complexes, size_t n_sqrt, size_t n,
                         double *out) {
...
__global__ void gMax(double *values, size_t n_sqrt, size_t n, double *out) {
  GET_INDEX_COND_RETURN(n_sqrt);
  double m = 0;
  FOR_N(i, n_sqrt) {
    size_t id = gindex * n_sqrt + i;
...
__global__ void gEncodeInternalComplexArrayUtilA(double *conj_values, size_t n,
  ...
  FOR_N(j, coeff_modulus_size) {
    if (is_negative) {
      ...
      destination[gindex + j * n] =
          kernel_util::dBarrettReduce64(coeffu, coeff_modulus[j]);
    }
  }
}

__global__ void gEncodeInternalComplexArrayUtilB(double *conj_values, size_t n,
                                                 size_t coeff_modulus_size,
                                                 const Modulus *coeff_modulus,
                                                 uint64_t *destination,
                                                 bool is_complex = true) {
...
__global__ void gEncodeInternalComplexArrayUtilC(double *conj_values, size_t n,
                                                 size_t coeff_modulus_size,
                                                 const Modulus *coeff_modulus,
...
__global__ void gEncodeInternalComplexArrayUtilD(
    const double *conj_values, const uint64_t *coeffu_array, size_t n,
    size_t coeff_modulus_size, const Modulus *coeff_modulus,
    uint64_t *destination, bool is_complex = true) {
  GET_INDEX_COND_RETURN(n);
  double coeffd = round(conj_values[gindex * (is_complex ? 2 : 1)]);
  bool is_negative = coeffd < 0;
...
__global__ void gCopyMulScalar(const double *from, double *to, size_t n,
                               double scalar) {
  GET_INDEX_COND_RETURN(n);
  to[gindex] = from[gindex] * scalar;
}

void CKKSEncoderCuda::encodeInternal(const std::complex<double> *values,
                                     std::size_t values_size, ParmsID parms_id,
                                     double scale, PlaintextCuda &destination) {
  // Verify parameters.
  ...
  if (scale <= 0 || (static_cast<int>(log2(scale)) + 1 >=
                     context_data.totalCoeffModulusBitCount())) {
...

  {
    ...
    (reinterpret_cast<const double *>(copied_values.get()), values_size, slots_,
  ...
  kFftTransferFromRev(conj_values.get(), util::getPowerOfTwo(n),
  ...
  auto max_coeff_array = util::DeviceArray<double>(n_sqrt);
  {
    ...
    (reinterpret_cast<double *>(conj_values.get()), n_sqrt, n,
     max_coeff_array.get());
  }
  ...
  if (max_coeff_bit_count <= 64) {
    ...
    (reinterpret_cast<double *>(conj_values.get()), n, coeff_modulus_size,
     coeff_modulus.get(), destination.data());
    ...
    (reinterpret_cast<double *>(conj_values.get()), n, coeff_modulus_size,
     coeff_modulus.get(), coeffu_array.get());
    context_data.rnsTool()->baseq()->decomposeArrayKeepOrder(coeffu_array.get(),
                                                             n);
    gEncodeInternalComplexArrayUtilD<<<block_count, 256>>>(
        reinterpret_cast<double *>(conj_values.get()), coeffu_array.get(), n,
        coeff_modulus_size, coeff_modulus.get(), destination.data());
  }

  kernel_util::kNttNegacyclicHarvey(destination.data(), 1, coeff_modulus_size,
                                    getPowerOfTwo(n), ntt_tables);

  destination.parmsID() = parms_id;
  destination.scale() = scale;
}

void CKKSEncoderCuda::encodePolynomialInternal(const double *values,
                                               std::size_t values_size,
                                               ParmsID parms_id, double scale,
                                               PlaintextCuda &destination) {
  // Verify parameters.
  auto context_data_ptr = context_.getContextData(parms_id);
  if (!context_data_ptr) {
    throw std::invalid_argument(
  ...
  KernelProvider::memsetZero<double>(conj_values.get(), n);
  {
    ...
    (values_cuda.get(), conj_values.get(), n, scale);
  }
  ...
  if (max_coeff_bit_count <= 64) {
    KERNEL_CALL(gEncodeInternalComplexArrayUtilA, n)
    (reinterpret_cast<double *>(conj_values.get()), n, coeff_modulus_size,
     coeff_modulus.get(), destination.data(), false);
  } else if (max_coeff_bit_count <= 128) {
    KERNEL_CALL(gEncodeInternalComplexArrayUtilB, n)
    (reinterpret_cast<double *>(conj_values.get()), n, coeff_modulus_size,
     coeff_modulus.get(), destination.data(), false);
    ...
    KERNEL_CALL(gEncodeInternalComplexArrayUtilC, n)
    (reinterpret_cast<double *>(conj_values.get()), n, coeff_modulus_size,
     coeff_modulus.get(), coeffu_array.get(), false);
    context_data.rnsTool()->baseq()->decomposeArrayKeepOrder(coeffu_array.get(),
                                                             n);
    gEncodeInternalComplexArrayUtilD<<<block_count, 256>>>(
        reinterpret_cast<double *>(conj_values.get()), coeffu_array.get(), n,
        coeff_modulus_size, coeff_modulus.get(), destination.data(), false);
  }
...
__global__ void gEncodeInternalDoubleUtilA(double coeffd, size_t n,
                                           size_t coeff_modulus_size,
                                           const Modulus *coeff_modulus,
                                           uint64_t *destination) {
  GET_INDEX_COND_RETURN(n);
  ...
  FOR_N(j, coeff_modulus_size) {
    uint64_t fill =
        is_negative ? kernel_util::dNegateUintMod(kernel_util::dBarrettReduce64(
                                                      coeffu, coeff_modulus[j]),
                                                  coeff_modulus[j])
                    : kernel_util::dBarrettReduce64(coeffu, coeff_modulus[j]);
...
__global__ void gEncodeInternalDoubleUtilB(double coeffd, size_t n,
                                           size_t coeff_modulus_size,
                                           const Modulus *coeff_modulus,
                                           uint64_t *destination) {
  GET_INDEX_COND_RETURN(n);
...
__global__ void gEncodeInternalDoubleUtilC(const uint64_t *coeffu,
                                           bool is_negative, size_t n,
                                           size_t coeff_modulus_size,
                                           const Modulus *coeff_modulus,
                                           uint64_t *destination) {
  GET_INDEX_COND_RETURN(n);
  FOR_N(j, coeff_modulus_size) {
    uint64_t fill =
        is_negative ? kernel_util::dNegateUintMod(coeffu[j], coeff_modulus[j])
                    : coeffu[j];
    destination[gindex + j * n] = fill;
  }
}

void CKKSEncoderCuda::encodeInternal(double value, ParmsID parms_id,
                                     double scale, PlaintextCuda &destination) {
  // Verify parameters.
  auto context_data_ptr = context_.getContextData(parms_id);
  if (!context_data_ptr) {
  ...
  if (coeff_bit_count <= 64) {
    ...
    (coeffd, coeff_count, coeff_modulus_size, coeff_modulus.get(),
     destination.data());
    ...
    (coeffu_dev.get(), is_negative, coeff_count, coeff_modulus_size,
     coeff_modulus.get(), destination.data());
  }
...
__global__ void gEncodeInternalInt64(int64_t value, size_t n,
                                     size_t coeff_modulus_size,
                                     const Modulus *coeff_modulus,
                                     uint64_t *destination) {
  GET_INDEX_COND_RETURN(n);
  FOR_N(j, coeff_modulus_size) {
    uint64_t fill = (value < 0)
                        ? (static_cast<uint64_t>(value) +
                           DeviceHelper::getModulusValue(coeff_modulus[j]))
                        : static_cast<uint64_t>(value);
    destination[gindex + j * n] =
        kernel_util::dBarrettReduce64(fill, coeff_modulus[j]);
  }
}

void CKKSEncoderCuda::encodeInternal(int64_t value, ParmsID parms_id,
                                     PlaintextCuda &destination) {
  // Verify parameters.
  auto context_data_ptr = context_.getContextData(parms_id);
  if (!context_data_ptr) {
  ...
  int coeff_bit_count =
      getSignificantBitCount(static_cast<uint64_t>(llabs(value))) + 2;
  if (coeff_bit_count >= context_data.totalCoeffModulusBitCount()) {
    throw invalid_argument("encoded value is too large");
  }
  ...
  (value, coeff_count, coeff_modulus_size, coeff_modulus.get(),
   destination.data());
...
__global__ void gDecodeInternal(const uint64_t *plain_copy, size_t coeff_count,
                                size_t coeff_modulus_size,
                                const uint64_t *decryption_modulus,
                                const uint64_t *upper_half_threshold,
                                double inv_scale, double *res) {
  GET_INDEX_COND_RETURN(coeff_count);
  size_t i = gindex;
  bool greater =
      kernel_util::dCompareUint(plain_copy + i * coeff_modulus_size,
                                upper_half_threshold, coeff_modulus_size) >= 0;
  double two_pow_64 = pow(2.0, 64);
  ...
  if (greater) {
    ...
    for (std::size_t j = 0; j < coeff_modulus_size;
      ...
      if (plain_copy[i * coeff_modulus_size + j] > decryption_modulus[j]) {
        auto diff =
            plain_copy[i * coeff_modulus_size + j] - decryption_modulus[j];
        res[i * 2] +=
            diff ? static_cast<double>(diff) * scaled_two_pow_64 : 0.0;
      } else {
        auto diff =
            decryption_modulus[j] - plain_copy[i * coeff_modulus_size + j];
        res[i * 2] -=
            diff ? static_cast<double>(diff) * scaled_two_pow_64 : 0.0;
      }
    }
...
__global__ void gFftTransferToRevLayered(size_t layer, double *operand,
                                         size_t poly_modulus_degree_power,
                                         const double *roots) {
  GET_INDEX_COND_RETURN(1 << (poly_modulus_degree_power - 1));
  size_t m = 1 << layer;
...
void kFftTransferToRevLayered(size_t layer, complex<double> *operand,
                              size_t poly_modulus_degree_power,
...
void kFftTransferToRev(complex<double> *operand,
                       size_t poly_modulus_degree_power,
                       const complex<double> *roots) {
  std::size_t m = 1;
  std::size_t layer = 0;
  std::size_t n = size_t(1) << poly_modulus_degree_power;
  for (; m <= (n >> 1); m <<= 1) {
    kFftTransferToRevLayered(layer, operand, poly_modulus_degree_power, roots);
    layer++;
  }
}

__global__ void gDecodeInternalSetConjValues(const double *values, size_t slots,
                                             double *destination,
                                             uint64_t *matrix_reps_index_map) {
  GET_INDEX_COND_RETURN(slots);
...
void CKKSEncoderCuda::decodeInternal(const PlaintextCuda &plain,
  ...
  kernel_util::kSetPolyArray(plain.data(), 1, coeff_modulus_size, coeff_count,
                             plain_copy.get());

  // Transform each polynomial from NTT domain
  kernel_util::kInverseNttNegacyclicHarvey(
      plain_copy.get(), 1, coeff_modulus_size, logn, ntt_tables);
...

  {
    ...
    (plain_copy.get(), coeff_count, coeff_modulus_size,
     decryption_modulus.get(), upper_half_threshold.get(), inv_scale,
...
__global__ void gRetrieveReals(const double *from, double *to, size_t n) {
  GET_INDEX_COND_RETURN(n);
  to[gindex] = from[gindex * 2];
}

void CKKSEncoderCuda::decodePolynomialInternal(const PlaintextCuda &plain,
                                               double *destination) {
  if (!plain.isNttForm()) {
    throw std::invalid_argument("plain is not in NTT form");
  }
...

  {
    ...
    (reinterpret_cast<double *>(res.get()), destination_device.get(),
     coeff_count);
  }
