...
through the SEALContext::ContextData class once the SEALContext has been
created.
...
class EncryptionParameters {
  ...
  inline void setPolyModulusDegree(std::size_t poly_modulus_degree) {
    if (scheme_ == SchemeType::none && poly_modulus_degree) {
      throw std::logic_error(
          "poly_modulus_degree is not supported for this scheme");
    }

    // Set the degree
    // std::cout << "reset polydeg " <<  poly_modulus_degree_ << " -> " <<
    // poly_modulus_degree << std::endl;
  ...
  inline void setCoeffModulus(const std::vector<Modulus> &coeff_modulus) {
    ...
    if (scheme_ == SchemeType::none) {
      if (!coeff_modulus.empty()) {
        throw std::logic_error(
            "coeff_modulus is not supported for this scheme");
      }
  ...
  */
  inline void setPlainModulus(const Modulus &plain_modulus) {
    ...
    if (scheme_ != SchemeType::bfv && scheme_ != SchemeType::bgv &&
        !plain_modulus.isZero()) {
  ...
  */
  ...
  the random generator is set to
  UniformRandomGeneratorFactory::default_factory(). Setting this value allows a
  user to specify a custom random number generator source.

  @param[in] random_generator Pointer to the random generator factory
  */
  inline void setRandomGenerator(std::shared_ptr<UniformRandomGeneratorFactory>
  ...
  */
  inline SchemeType scheme() const noexcept { return scheme_; }
  ...
  */
  inline const Modulus &plainModulus() const noexcept { return plain_modulus_; }

  /**
  Returns a pointer to the random number generator factory to use for
  encryption.
  */
  inline std::shared_ptr<UniformRandomGeneratorFactory> randomGenerator()
      const noexcept {
  ...
  */
  ...
  //     coeff_modulus_total_size = util::mul_safe(coeff_modulus_total_size,
  //     coeff_modulus_.size());
  ...
  //     auto in_size =
  //         Serialization::Load(std::bind(&EncryptionParameters::load_members,
  //         &new_parms, _1, _2), stream, false);
  ...
  //         std::bind(&EncryptionParameters::load_members, &new_parms, _1, _2),
  //         in, size, false);
  //     std::swap(*this, new_parms);
  //     return in_size;
  // }
...
struct TroyHashParmsID {
  std::size_t operator()(const seal_gpu::ParmsID &parms_id) const {
...
template <>
struct hash<seal_gpu::EncryptionParameters> {
  std::size_t operator()(const seal_gpu::EncryptionParameters &parms) const {
