...
class SEALContextCuda {
 public:
  class ContextDataCuda {
   ...
   public:
    ...
    inline util::ConstDevicePointer<util::NTTTablesCuda> smallNTTTables()
        const {
      return small_ntt_tables_.asPointer();
    }

    inline util::ConstDevicePointer<util::NTTTablesCuda> plainNTTTables()
        const {
      return plain_ntt_tables_.asPointer();
    }

    inline const util::RNSToolCuda *rnsTool() const { return rns_tool_.get(); }

    inline size_t chainIndex() const { return chain_index_; }

    inline std::shared_ptr<const ContextDataCuda> prevContextData()
        const noexcept {
      return prev_context_data_.lock();
    }

    inline std::shared_ptr<const ContextDataCuda> nextContextData()
        const noexcept {
      return next_context_data_;
    }

    inline util::ConstDevicePointer<util::MultiplyUIntModOperand>
    coeffDivPlainModulus() const noexcept {
    ...
    inline util::ConstDevicePointer<uint64_t> plainUpperHalfIncrement()
        const noexcept {
      return plain_upper_half_increment_;
    }

    inline util::ConstDevicePointer<uint64_t> upperHalfThreshold()
        const noexcept {
      return upper_half_threshold_;
    }

    inline util::ConstDevicePointer<uint64_t> upperHalfIncrement()
        const noexcept {
    ...
    inline const util::GaloisToolCuda *galoisTool() const {
  ...
  inline bool using_keyswitching() const { return using_keyswitching_; }
 ...
 private:
  ...
  std::unordered_map<ParmsID, std::shared_ptr<ContextDataCuda>,
                     std::TroyHashParmsID>
      context_data_map_{};
