...
class EncryptionParameterQualifiers {
  ...
  */
  enum class ErrorType : int {
    ...
    */
    ...
    /**
    coeff_modulus's primes' count is not bounded by
    SEAL_COEFF_MOD_COUNT_MIN(MAX)
    */
    invalid_coeff_modulus_size = 2,

    /**
    coeff_modulus's primes' bit counts are not bounded by
    SEAL_USER_MOD_BIT_COUNT_MIN(MAX)
    */
    invalid_coeff_modulus_bit_count = 3,

    /**
    coeff_modulus's primes are not congruent to 1 modulo (2 *
    poly_modulus_degree)
    */
    ...
    */
    ...
    /**
    parameters are not compliant with HomomorphicEncryption.org security
    standard
    */
    ...
    */
    ...
    /**
    plain_modulus's bit count is not bounded by
    SEAL_PLAIN_MOD_BIT_COUNT_MIN(MAX)
    */
 ...
 private:
  EncryptionParameterQualifiers()
      : parameter_error(ErrorType::none),
        using_fft(false),
        using_ntt(false),
...
/**
Performs sanity checks (validation) and pre-computations for a given set of
encryption parameters. While the EncryptionParameters class is intended to be a
light-weight class to store the encryption parameters, the SEALContext class is
a heavy-weight class that is constructed from a given set of encryption
parameters. It validates the parameters for correctness, evaluates their
properties, and performs and stores the results of several costly
pre-computations.

After the user has set at least the poly_modulus, coeff_modulus, and
plain_modulus parameters in a given EncryptionParameters instance, the
parameters can be validated for correctness and functionality by constructing an
instance of SEALContext. The constructor of SEALContext does all of its work
automatically, and concludes by constructing and storing an instance of the
EncryptionParameterQualifiers class, with its flags set according to the
properties of the given parameters. If the created instance of
EncryptionParameterQualifiers has the parameters_set flag set to true, the given
parameter set has been deemed valid and is ready to be used. If the parameters
were for some reason not appropriately set, the parameters_set flag will be
false, and a new SEALContext will have to be created after the parameters are
corrected.

By default, SEALContext creates a chain of SEALContext::ContextData instances.
The first one in the chain corresponds to special encryption parameters that are
reserved to be used by the various key classes (SecretKey, PublicKey, etc.).
These are the exact same encryption parameters that are created by the user and
passed to th constructor of SEALContext. The functions key_context_data() and
keyParmsID() return the ContextData and the parms_id corresponding to these
special parameters. The rest of the ContextData instances in the chain
correspond to encryption parameters that are derived from the first encryption
parameters by always removing the last one of the moduli in the coeff_modulus,
until the resulting parameters are no longer valid, e.g., there are no more
primes left. These derived encryption parameters are used by ciphertexts and
...
last_context_data() return the ContextData corresponding to the first and the
last set of parameters in the "data" part of the chain, i.e., the second and the
last element in the full chain. The chain itself is a doubly linked list, and is
referred to as the modulus switching chain.
...
class SEALContext {
  ...
  */
  class ContextData {
    ...
    */
    inline const EncryptionParameters &parms() const noexcept { return parms_; }

    /**
    Returns the parms_id of the current parameters.
    */
    inline const ParmsID &parmsID() const noexcept { return parms_.parmsID(); }
    ...
    */
    inline const util::NTTTables *smallNTTTables() const noexcept {
      // std::cout << "get small ntt tables: size = " <<
      // small_ntt_tables_.size() << std::endl;
    ...
    */
    ...
    plaintext modulus.
    */
    inline const util::MultiplyUIntModOperand *coeffDivPlainModulus()
        const noexcept {
    ...
    */
    ...
    /**
    Returns a shared_ptr to the context data corresponding to the previous
    parameters in the modulus switching chain. If the current data is the first
    one in the chain, then the result is nullptr.
    */
    ...
    /**
    Returns a shared_ptr to the context data corresponding to the next
    parameters in the modulus switching chain. If the current data is the last
    one in the chain, then the result is nullptr.
    */
    ...
    */
    inline std::size_t chainIndex() const noexcept { return chain_index_; }
  ...
  */
  SEALContext(EncryptionParameters parms, bool expand_mod_chain = true,
  ...
  */
  inline std::shared_ptr<const ContextData> getContextData(
  ...
  */
  inline bool parametersSet() const {
    return firstContextData() ? firstContextData()->qualifiers_.parametersSet()
                              : false;
  }
  ...
  */
  inline const char *parameterErrorName() const {
    return firstContextData()
               ? firstContextData()->qualifiers_.parameterErrorName()
  ...
  */
  inline const char *parameterErrorMessage() const {
    return firstContextData()
               ? firstContextData()->qualifiers_.parameterErrorMessage()
               : "SEALContext is empty";
  }
  ...
  */
  inline const ParmsID &keyParmsID() const noexcept { return key_parms_id_; }
  ...
  */
  ...
  that are used for data.
  */
  inline const ParmsID &lastParmsID() const noexcept { return last_parms_id_; }
  ...
  */
  ...
  std::unordered_map<ParmsID, std::shared_ptr<const ContextData>,
                     std::TroyHashParmsID>
      context_data_map_{};
