...
class Ciphertext {
  ...
  */
  ...
  @throws std::invalid_argument if pool is uninitialized
  */
  explicit Ciphertext(const SEALContext &context) : data_() {
  ...
  @throws std::invalid_argument if pool is uninitialized
  */
  explicit Ciphertext(const SEALContext &context, ParmsID parms_id) : data_() {
    // Allocate memory but don't resize
  ...
  @throws std::invalid_argument if pool is uninitialized
  */
  explicit Ciphertext(const SEALContext &context, ParmsID parms_id,
  ...
  */
  ...
  @throws std::invalid_argument if size_capacity is less than 2 or too large
  */
  void reserve(const SEALContext &context, ParmsID parms_id,
               std::size_t size_capacity);
  ...
  */
  inline const auto &dynArray() const noexcept { return data_; }

  /**
  Returns a pointer to the beginning of the ciphertext data.
  */
  inline ct_coeff_type *data() noexcept { return data_.begin(); }

  /**
  Returns a const pointer to the beginning of the ciphertext data.
  */
  inline const ct_coeff_type *data() const noexcept { return data_.cbegin(); }
  ...
  */
  inline ct_coeff_type *data(std::size_t poly_index) {
    auto poly_uint64_count =
        util::mul_safe(poly_modulus_degree_, coeff_modulus_size_);
  ...
  */
  inline bool isTransparent() const {
    return (!data_.size() || (size_ < SEAL_CIPHERTEXT_SIZE_MIN) ||
  ...
  // */
  // std::streamoff save_size(compr_mode_type compr_mode =
  ...
  // {
  //     using namespace std::placeholders;
  //     return Serialization::Load(std::bind(&Ciphertext::load_members, this,
  ...
  */
  ...
  encryption scheme. The user should have little or no reason to ever change the
  scale by hand.
  */
  inline double &scale() noexcept { return scale_; }

  /**
  Returns a constant reference to the scale. This is only needed when using the
  ...
  */
  ...
  /**
  Returns a constant reference to the correction factor. This is only needed
  when using the BGV encryption scheme.
  */
 ...
 private:
  ...
  void resizeInternal(std::size_t size, std::size_t poly_modulus_degree,
                      std::size_t coeff_modulus_size);

  void expandSeed(const SEALContext &context,
  ...
  inline bool hasSeedMarker() const noexcept {
    return (data_.size() && (size_ == 2))
               ? (data(1)[0] == 0xFFFFFFFFFFFFFFFFULL)
               : false;
  }
