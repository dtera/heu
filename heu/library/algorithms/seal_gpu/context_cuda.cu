...
void SEALContextCuda::initialize(const SEALContext &context) {
  for (auto &pair : context.context_data_map_) {
    context_data_map_.emplace(std::make_pair(
        pair.first, std::make_shared<ContextDataCuda>(
                        std::move(ContextDataCuda(*pair.second)))));
  }
...
SEALContextCuda::ContextDataCuda::ContextDataCuda(
    const SEALContext::ContextData &contextData)
    : qualifiers_(contextData.qualifiers()),
      ...
      coeff_modulus_mod_plain_modulus_(
          contextData.coeffModulusModPlainModulus()),
      prev_context_data_(),
  ...
  for (size_t i = 0; i < n; i++) {
    small_ntt_tables_support_[i] =
        util::NTTTablesCuda(contextData.small_ntt_tables_[i]);
  }
  ...
  for (size_t i = 0; i < n; i++) {
    plain_ntt_tables_support_[i] =
        util::NTTTablesCuda(contextData.plain_ntt_tables_[i]);
  }
  plain_ntt_tables_ = util::DeviceArray(plain_ntt_tables_support_);
  galois_tool_ = new util::GaloisToolCuda(
      util::getPowerOfTwo(contextData.parms_.polyModulusDegree()));
}
