...
class KeyGenerator {
  ...
  */
  ...
  impact.
  */
  inline PublicKey createPublicKey() const { return generatePk(); }
  ...
  */
  ...
  keyswitching
  */
  inline RelinKeys createRelinKeys() { return createRelinKeys(1); }
  ...
  */
  inline void createGaloisKeys(const std::vector<std::uint32_t> &galois_elts,
  ...
  */
  inline GaloisKeys createGaloisKeys(const std::vector<int> &steps) {
    ...
    return createGaloisKeysInternal(
        context_.keyContextData()->galoisTool()->getEltsFromSteps(steps));
  }
  ...
  */
  inline void createGaloisKeys(GaloisKeys &destination) {
    createGaloisKeys(context_.keyContextData()->galoisTool()->getEltsAll(),
                     destination);
  }
  ...
  */
  inline GaloisKeys createGaloisKeys() {
    return createGaloisKeysInternal(
        context_.keyContextData()->galoisTool()->getEltsAll());
  }
 ...
 private:
  ...
  void computeSecretKeyArray(const SEALContext::ContextData &context_data,
  ...
  */
  void generateKswitchKeys(util::ConstHostPointer<uint64_t> new_keys,
                           std::size_t num_keys, KSwitchKeys &destination);

  /**
  Generates one key switching key for a new key.
  */
  void generateOneKswitchKey(util::ConstHostPointer<uint64_t> new_key,
  ...
  */
  ...
  @throws std::invalid_argument if the Galois elements are not valid
  */
  GaloisKeys createGaloisKeysInternal(
      const std::vector<std::uint32_t> &galois_elts);
