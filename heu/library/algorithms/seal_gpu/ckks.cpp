...
CKKSEncoder::CKKSEncoder(const SEALContext &context) : context_(context) {
  ...
  for (size_t i = 0; i < slots_; i++) {
    ...
    matrix_reps_index_map_[slots_ | i] =
        safe_cast<size_t>(reverseBits(index2, logn));
  ...
  if (m >= 8) {
    complex_roots_ = make_shared<util::ComplexRoots>(
        util::ComplexRoots(static_cast<size_t>(m)));
    for (size_t i = 1; i < coeff_count; i++) {
      root_powers_[i] = complex_roots_->getRoot(reverseBits(i, logn));
      inv_root_powers_[i] =
          conj(complex_roots_->getRoot(reverseBits(i - 1, logn) + 1));
    }
  } else if (m == 4) {
    root_powers_[1] = {0, 1};
...
void CKKSEncoder::encodeInternal(const std::complex<double> *values,
  ...
  for (std::size_t i = 0; i < values_size; i++) {
    ...
    // TODO: if values are real, the following values should be set to zero, and
    // multiply results by 2.
    conj_values[matrix_reps_index_map_[i + slots_]] = std::conj(values[i]);
  }
  double fix = scale / static_cast<double>(n);
  fft_handler_.transformFromRev(conj_values.get(), util::getPowerOfTwo(n),
  ...
  if (max_coeff_bit_count <= 64) {
      ...
      if (is_negative) {
        for (std::size_t j = 0; j < coeff_modulus_size; j++) {
          destination[i + (j * coeff_count)] = util::negateUintMod(
              util::barrettReduce64(coeffu, coeff_modulus[j]),
              coeff_modulus[j]);
        }
      } else {
        for (std::size_t j = 0; j < coeff_modulus_size; j++) {
    ...
    for (std::size_t i = 0; i < n; i++) {
      ...
      if (is_negative) {
        for (std::size_t j = 0; j < coeff_modulus_size; j++) {
          destination[i + (j * coeff_count)] = util::negateUintMod(
              util::barrettReduce128(coeffu, coeff_modulus[j]),
              coeff_modulus[j]);
        }
      } else {
        for (std::size_t j = 0; j < coeff_modulus_size; j++) {
    ...
    for (std::size_t i = 0; i < n; i++) {
      ...
      if (is_negative) {
        for (std::size_t j = 0; j < coeff_modulus_size; j++) {
          destination[i + (j * coeff_count)] =
              util::negateUintMod(coeffu[j], coeff_modulus[j]);
        }
      } else {
        for (std::size_t j = 0; j < coeff_modulus_size; j++) {
...
void CKKSEncoder::encodeInternal(double value, ParmsID parms_id, double scale,
  ...
  if (coeff_bit_count <= 64) {
    ...
    if (is_negative) {
      for (size_t j = 0; j < coeff_modulus_size; j++) {
        fill_n(destination.data() + (j * coeff_count), coeff_count,
               negateUintMod(barrettReduce64(coeffu, coeff_modulus[j]),
                             coeff_modulus[j]));
      }
    } else {
      for (size_t j = 0; j < coeff_modulus_size; j++) {
    ...
    if (is_negative) {
      for (size_t j = 0; j < coeff_modulus_size; j++) {
        fill_n(destination.data() + (j * coeff_count), coeff_count,
               negateUintMod(barrettReduce128(coeffu, coeff_modulus[j]),
                             coeff_modulus[j]));
      }
    } else {
      for (size_t j = 0; j < coeff_modulus_size; j++) {
...
void CKKSEncoder::encodeInternal(int64_t value, ParmsID parms_id,
                                 Plaintext &destination) {
  // Verify parameters.
  auto context_data_ptr = context_.getContextData(parms_id);
  if (!context_data_ptr) {
...
void CKKSEncoder::decodeInternal(const Plaintext &plain,
  ...
  for (std::size_t i = 0; i < coeff_modulus_size; i++) {
    util::inverseNttNegacyclicHarvey(plain_copy.get() + (i * coeff_count),
                                     ntt_tables[i]);
  }
  ...
  for (std::size_t i = 0; i < coeff_count; i++) {
    ...
    if (util::isGreaterThanOrEqualUint(
            plain_copy.get() + (i * coeff_modulus_size), upper_half_threshold,
            coeff_modulus_size)) {
