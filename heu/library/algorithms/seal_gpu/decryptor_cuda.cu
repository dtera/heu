...
//         ConstHostPointer<uint64_t> poly, size_t coeff_uint64_count, size_t
//         coeff_count, const uint64_t *modulus, uint64_t *result)
//     {

//         // Construct negative threshold: (modulus + 1) / 2
//         auto modulus_neg_threshold = allocateUint(coeff_uint64_count);
//         halfRoundUpUint(modulus, coeff_uint64_count,
//         modulus_neg_threshold.get());

//         // Mod out the poly coefficients and choose a symmetric
//         representative from [-modulus,modulus)
...
//             if (isGreaterThanOrEqualUint(polyi, modulus_neg_threshold.get(),
//             coeff_uint64_count))
//             {
//                 subUint(modulus, polyi, coeff_uint64_count,
//                 coeff_abs_value.get());
//             }
//             else
//             {
...
//             if (isGreaterThanUint(coeff_abs_value.get(), result,
//             coeff_uint64_count))
//             {
//                 // Store the new max
...
DecryptorCuda::DecryptorCuda(const SEALContextCuda &context,
                             const SecretKeyCuda &secret_key)
    : context_(context) {
  ...
  kernel_util::kSetPolyArray(secret_key.data().data(), 1, coeff_modulus_size,
                             coeff_count, secret_key_array_);
  secret_key_array_size_ = 1;
}

void DecryptorCuda::decrypt(const CiphertextCuda &encrypted,
...
void DecryptorCuda::bfvDecrypt(const CiphertextCuda &encrypted,
                               PlaintextCuda &destination) {
  ...
  // Add Delta / 2 and now we have something which is Delta * (m + epsilon)
  // where epsilon < 1 Therefore, we can (integer) divide by Delta and the
  // answer will round down to m.

  // Make a temp destination for all the arithmetic mod qi before calling
  // FastBConverse
  auto tmp_dest_modq =
      kernel_util::kAllocateZero(coeff_count, coeff_modulus_size);
  // SEAL_ALLOCATE_ZERO_GET_RNS_ITER(tmp_dest_modq, coeff_count,
  // coeff_modulus_size, pool);

  // put < (c_1 , c_2, ... , c_{count-1}) , (s,s^2,...,s^{count-1}) > mod q in
  // destination Now do the dot product of encrypted_copy and the secret key
  // array using NTT. The secret key powers are already NTT transformed.
  ...
  context_data.rnsTool()->decryptScaleAndRound(tmp_dest_modq.asPointer(),
                                               destination.data());

  // How many non-zero coefficients do we really have in the result?
  // size_t plain_coeff_count =
  // getSignificantUint64CountUint(destination.data(), coeff_count);

  // Resize destination to appropriate size
  // destination.resize(max(plain_coeff_count, size_t(1)));
}

void DecryptorCuda::ckksDecrypt(const CiphertextCuda &encrypted,
                                PlaintextCuda &destination) {
  if (!encrypted.isNttForm()) {
...
void DecryptorCuda::bgvDecrypt(const CiphertextCuda &encrypted,
                               PlaintextCuda &destination) {
  if (encrypted.isNttForm()) {
    throw invalid_argument("encrypted cannot be in NTT form");
  }
  ...
  context_data.rnsTool()->decryptModt(tmp_dest_modq.asPointer(),
                                      destination.data());
  ...
  if (encrypted.correctionFactor() != 1) {
    ...
    kernel_util::kMultiplyPolyScalarCoeffmod(
        destination.data(), 1, 1, coeff_count, fix, parms.plainModulusCuda(),
        destination.data());
  }
...
void DecryptorCuda::computeSecretKeyArray(size_t max_power) {
  ...
  auto secret_key_array =
      kernel_util::kAllocate(new_size, coeff_count, coeff_modulus_size);
  kernel_util::kSetPolyArray(secret_key_array_.get(), old_size, coeff_count,
                             coeff_modulus_size, secret_key_array.get());

  size_t poly_coeff_count = coeff_count * coeff_modulus_size;

  // Since all of the key powers in secret_key_array_ are already NTT
  // transformed, to get the next one we simply need to compute a dyadic product
  // of the last one with the first one [which is equal to NTT(secret_key_)].
  for (size_t i = 0; i < new_size - old_size; i++) {
    kernel_util::kDyadicProductCoeffmod(
        ...
        secret_key_array.asPointer(), 1, coeff_modulus_size, coeff_count,
        coeff_modulus,
...
void DecryptorCuda::dotProductCtSkArray(const CiphertextCuda &encrypted,
  ...
  size_t key_coeff_modulus_size =
      context_.keyContextData()->parms().coeffModulus().size();
  ...
  if (encrypted_size == 2) {
    ...
    if (isNttForm) {
      kernel_util::kDyadicProductCoeffmod(c1, secret_key_array, 1,
                                          coeff_modulus_size, coeff_count,
                                          coeff_modulus, destination);
      kernel_util::kAddPolyCoeffmod(destination, c0, 1, coeff_modulus_size,
                                    coeff_count, coeff_modulus, destination);
    } else {
      kernel_util::kSetPolyArray(c1, 1, coeff_modulus_size, coeff_count,
                                 destination);
      kernel_util::kNttNegacyclicHarveyLazy(destination, 1, coeff_modulus_size,
                                            coeff_power, ntt_tables);
      kernel_util::kDyadicProductCoeffmod(destination, secret_key_array, 1,
                                          coeff_modulus_size, coeff_count,
                                          coeff_modulus, destination);
      kernel_util::kInverseNttNegacyclicHarvey(
          destination, 1, coeff_modulus_size, coeff_power, ntt_tables);
      kernel_util::kAddPolyCoeffmod(destination, c0, 1, coeff_modulus_size,
                                    coeff_count, coeff_modulus, destination);
    }
  } else {
    ...
    // destination Now do the dot product of encrypted_copy and the secret key
    // array using NTT. The secret key powers are already NTT transformed.
    auto encrypted_copy = DeviceArray<uint64_t>(
        (encrypted_size - 1) * coeff_count * coeff_modulus_size);
    size_t poly_coeff_count = coeff_count * coeff_modulus_size;
    size_t key_poly_coeff_count = coeff_count * key_coeff_modulus_size;
    // assert(key_poly_coeff_count == poly_coeff_count);
    kernel_util::kSetPolyArray(encrypted.data(1), encrypted_size - 1,
                               coeff_modulus_size, coeff_count,
                               encrypted_copy.get());
    if (!isNttForm) {
      kernel_util::kNttNegacyclicHarveyLazy(encrypted_copy, encrypted_size - 1,
                                            coeff_modulus_size, coeff_power,
                                            ntt_tables);
    }
    for (size_t i = 0; i < encrypted_size - 1; i++) {
      kernel_util::kDyadicProductCoeffmod(
          encrypted_copy + i * poly_coeff_count,
          secret_key_array_ + i * key_poly_coeff_count, 1, coeff_modulus_size,
          coeff_count, coeff_modulus, encrypted_copy + i * poly_coeff_count);
    }

    kernel_util::kSetZeroPolyArray(1, coeff_count, coeff_modulus_size,
                                   destination.get());
    for (size_t i = 0; i < encrypted_size - 1; i++) {
      kernel_util::kAddPolyCoeffmod(
          destination, encrypted_copy + i * poly_coeff_count, 1,
          coeff_modulus_size, coeff_count, coeff_modulus, destination);
    }
    ...
    kernel_util::kAddPolyCoeffmod(destination, encrypted.data(), 1,
                                  coeff_modulus_size, coeff_count,
                                  coeff_modulus, destination);
  }
}
...
//     // SEAL_ALLOCATE_ZERO_GET_RNS_ITER(noise_poly, coeff_count,
...
//     // in destination_poly.
//     // Now do the dot product of encrypted_copy and the secret key array
//     using NTT.
...
//             noise_poly.asPointer(), coeff_modulus_size, coeff_count,
//             plain_modulus.value(), &coeff_modulus[0],
//             noise_poly.asPointer());
//     }

//     // CRT-compose the noise
//     context_data.rnsTool()->baseq()->composeArray(noise_poly.get(),
//     coeff_count);

//     // Next we compute the infinity norm mod parms.coeffModulus()
//     // StrideIter<const uint64_t *> wide_noise_poly((*noise_poly).ptr(),
//     coeff_modulus_size); polyInftyNormCoeffmod(noise_poly.get(),
//     coeff_modulus_size, coeff_count, context_data.totalCoeffModulus(),
//     norm.get());
...
//                          getSignificantBitCountUint(norm.get(),
