...
Encryptor::Encryptor(const SEALContext &context, const SecretKey &secret_key)
    : context_(context) {
  // Verify parameters
  if (!context_.parametersSet()) {
    throw invalid_argument("encryption parameters are not set correctly");
...
void Encryptor::encryptZeroInternal(ParmsID parms_id, bool is_asymmetric,
  ...
  if (is_asymmetric) {
    ...
    if (prev_context_data_ptr) {
      ...
      for (size_t i = 0; i < temp.size(); i++) {
        ...
        setPoly(temp.data() + i * (poly_element_count + coeff_count),
...
void Encryptor::encryptInternal(const Plaintext &plain, bool is_asymmetric,
                                Ciphertext &destination) const {
  ...
  if (scheme == SchemeType::bfv) {
    ...
    addPolyCoeffmod(destination_iter, plain_iter, coeff_modulus_size,
                    coeff_count, &coeff_modulus[0], destination_iter);
    ...
    addPlainWithoutScalingVariant(plain, *context_data_ptr,
                                  HostPointer(destination.data(0)));
  } else {
    throw invalid_argument("unsupported scheme");
  }
}
