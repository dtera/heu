...
class Plaintext {
  ...
  */
  ...
  @throws std::invalid_argument if pool is uninitialized
  */
  Plaintext(const std::string &hex_poly) : data_() { operator=(hex_poly); }
  ...
  */
  inline void shrinkToFit() { data_.shrinkToFit(); }
  ...
  */
  ...
  @throws std::out_of_range if start_coeff + length - 1 is not within [0,
  coeff_count)
  */
  inline void setZero(std::size_t start_coeff, std::size_t length) {
    ...
    if (start_coeff + length - 1 >= coeff_count_) {
      throw std::out_of_range(
          "length must be non-negative and start_coeff + length - 1 must be "
          "within [0, coeff_count)");
    }
  ...
  */
  inline pt_coeff_type *data() { return data_.begin(); }

  /**
  Returns a const pointer to the beginning of the plaintext polynomial.
  */
  inline const pt_coeff_type *data() const { return data_.cbegin(); }
  ...
  */
  inline bool isZero() const {
    return !coeff_count_ || std::all_of(data_.cbegin(), data_.cend(),
                                        util::isZero<pt_coeff_type>);
  }
  ...
  */
  inline std::size_t capacity() const noexcept { return data_.capacity(); }

  /**
  Returns the coefficient count of the current plaintext polynomial.
  */
  inline std::size_t coeffCount() const noexcept { return coeff_count_; }
  ...
  */
  inline std::string to_string() const {
    if (isNttForm()) {
      throw std::invalid_argument(
          "cannot convert NTT transformed plaintext to string");
    }
  ...
  // {
  //     using namespace std::placeholders;
  //     return Serialization::Load(std::bind(&Plaintext::load_members, this,
  ...
  */
  inline bool isNttForm() const noexcept { return (parms_id_ != parmsIDZero); }
