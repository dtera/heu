...
using ErrorType = seal_gpu::EncryptionParameterQualifiers::ErrorType;
...
const char *EncryptionParameterQualifiers::parameterErrorMessage()
    const noexcept {
  switch (parameter_error) {
    case ErrorType::none:
    ...
    case ErrorType::invalid_coeff_modulus_size:
      return "coeff_modulus's primes' count is not bounded by "
             "SEAL_COEFF_MOD_COUNT_MIN(MAX)";

    case ErrorType::invalid_coeff_modulus_bit_count:
      return "coeff_modulus's primes' bit counts are not bounded by "
             "SEAL_USER_MOD_BIT_COUNT_MIN(MAX)";

    case ErrorType::invalid_coeff_modulus_no_ntt:
      return "coeff_modulus's primes are not congruent to 1 modulo (2 * "
             "poly_modulus_degree)";

    case ErrorType::invalid_poly_modulus_degree:
      return "poly_modulus_degree is not bounded by "
             "SEAL_POLY_MOD_DEGREE_MIN(MAX)";
    ...
    case ErrorType::invalid_parameters_insecure:
      return "parameters are not compliant with HomomorphicEncryption.org "
             "security standard";
    ...
    case ErrorType::invalid_plain_modulus_bit_count:
      return "plain_modulus's bit count is not bounded by "
...
SEALContext::ContextData SEALContext::validate(EncryptionParameters parms) {
  ...
  // The number of coeff moduli is restricted to 64 to prevent unexpected
  // behaviors
  if (coeff_modulus.size() > SEAL_COEFF_MOD_COUNT_MAX ||
      coeff_modulus.size() < SEAL_COEFF_MOD_COUNT_MIN) {
    context_data.qualifiers_.parameter_error =
        ErrorType::invalid_coeff_modulus_size;
    return context_data;
  }
  ...
  for (size_t i = 0; i < coeff_modulus_size; i++) {
    ...
    if (coeff_modulus[i].value() >> SEAL_USER_MOD_BIT_COUNT_MAX ||
      ...
      context_data.qualifiers_.parameter_error =
          ErrorType::invalid_coeff_modulus_bit_count;
      return context_data;
    }
  }
  ...
  context_data.total_coeff_modulus_ =
      util::HostArray<uint64_t>(coeff_modulus_size);
  auto coeff_modulus_values = util::HostArray<uint64_t>(coeff_modulus_size);
  for (size_t i = 0; i < coeff_modulus_size; i++) {
    coeff_modulus_values[i] = coeff_modulus[i].value();
  }
  multiplyManyUint64(coeff_modulus_values.get(), coeff_modulus_size,
                     context_data.total_coeff_modulus_.get());
  context_data.total_coeff_modulus_bit_count_ = getSignificantBitCountUint(
  ...
  if (poly_modulus_degree < SEAL_POLY_MOD_DEGREE_MIN ||
    ...
    context_data.qualifiers_.parameter_error =
        ErrorType::invalid_poly_modulus_degree;
    return context_data;
  }
  ...
  if (coeff_count_power < 0) {
    // Parameters are not valid
    context_data.qualifiers_.parameter_error =
        ErrorType::invalid_poly_modulus_degree_non_power_of_two;
    return context_data;
  }
  ...
  if (!productFitsIn(coeff_modulus_size, poly_modulus_degree)) {
    context_data.qualifiers_.parameter_error =
        ErrorType::invalid_parameters_too_large;
    return context_data;
  }
  ...
  if (context_data.total_coeff_modulus_bit_count_ >
    ...
    if (sec_level_ != SecurityLevel::none) {
      // Parameters are not valid
      context_data.qualifiers_.parameter_error =
          ErrorType::invalid_parameters_insecure;
      return context_data;
    }
  }
  ...
  try {
    ...
    // Parameters are not valid
    context_data.qualifiers_.parameter_error =
        ErrorType::failed_creating_rns_base;
    return context_data;
  }
  ...
  try {
    context_data.small_ntt_tables_ =
        CreateNTTTables(coeff_count_power, coeff_modulus);
  } catch (const invalid_argument &) {
    context_data.qualifiers_.using_ntt = false;
    // Parameters are not valid
    context_data.qualifiers_.parameter_error =
        ErrorType::invalid_coeff_modulus_no_ntt;
    return context_data;
  }
  ...
  if (parms.scheme() == SchemeType::bfv || parms.scheme() == SchemeType::bgv) {
    ...
    if (plain_modulus.value() >> SEAL_PLAIN_MOD_BIT_COUNT_MAX ||
      ...
      context_data.qualifiers_.parameter_error =
          ErrorType::invalid_plain_modulus_bit_count;
      return context_data;
    }
    ...
    for (size_t i = 0; i < coeff_modulus_size; i++) {
      if (!areCoprime(coeff_modulus[i].value(), plain_modulus.value())) {
        context_data.qualifiers_.parameter_error =
            ErrorType::invalid_plain_modulus_coprimality;
        return context_data;
      }
    }

    // Check that plain_modulus is smaller than total coeff modulus
    if (!isLessThanUint(plain_modulus.data(), plain_modulus.uint64Count(),
                        context_data.total_coeff_modulus_.get(),
                        coeff_modulus_size)) {
      // Parameters are not valid
      context_data.qualifiers_.parameter_error =
          ErrorType::invalid_plain_modulus_too_large;
      return context_data;
    }
    ...
    try {
      context_data.plain_ntt_tables_ =
          CreateNTTTables(coeff_count_power, {plain_modulus});
    } catch (const invalid_argument &) {
      context_data.qualifiers_.using_batching = false;
    }

    // Check for plain_lift
    // If all the small coefficient moduli are larger than plain modulus, we can
    // quickly lift plain coefficients to RNS form
    ...
    context_data.coeff_div_plain_modulus_ =
        HostArray<MultiplyUIntModOperand>(coeff_modulus_size);
    context_data.upper_half_increment_ =
        HostArray<uint64_t>(coeff_modulus_size);
    auto wide_plain_modulus(duplicateUintIfNeeded(plain_modulus.data(),
                                                  plain_modulus.uint64Count(),
                                                  coeff_modulus_size, false));
    divideUint(context_data.total_coeff_modulus_.get(),
               wide_plain_modulus.get(), coeff_modulus_size,
               temp_coeff_div_plain_modulus.get(),
               context_data.upper_half_increment_.get());

    // Store the non-RNS form of upper_half_increment for BFV encryption
    context_data.coeff_modulus_mod_plain_modulus_ =
        context_data.upper_half_increment_[0];
    ...
    if (context_data.qualifiers_.using_fast_plain_lift) {
      ...
      subUint(context_data.totalCoeffModulus(), wide_plain_modulus.get(),
              coeff_modulus_size,
    ...
    if (!plain_modulus.isZero()) {
      // Parameters are not valid
      context_data.qualifiers_.parameter_error =
          ErrorType::invalid_plain_modulus_nonzero;
      return context_data;
    }
    ...
    // Calculate plain_upper_half_increment = 2^64 mod coeff_modulus for CKKS
    // plaintexts
    context_data.plain_upper_half_increment_ =
        HostArray<uint64_t>(coeff_modulus_size);
    for (size_t i = 0; i < coeff_modulus_size; i++) {
      ...
      context_data.plain_upper_half_increment_[i] =
          multiplyUintMod(tmp, sub_safe(coeff_modulus[i].value(), uint64_t(2)),
                          coeff_modulus[i]);
    }

    // Compute the upper_half_threshold for this modulus.
    context_data.upper_half_threshold_ =
        HostArray<uint64_t>(coeff_modulus_size);
    incrementUint(context_data.totalCoeffModulus(), coeff_modulus_size,
                  context_data.upper_half_threshold_.get());
    rightShiftUint(context_data.upper_half_threshold_.get(), 1,
                   coeff_modulus_size,
                   context_data.upper_half_threshold_.get());
  } else {
    context_data.qualifiers_.parameter_error = ErrorType::invalid_scheme;
  ...
  try {
    context_data.rns_tool_ = HostObject(
        new RNSTool(poly_modulus_degree, *coeff_modulus_base, plain_modulus));
  } catch (const std::exception &) {
    // Parameters are not valid
    context_data.qualifiers_.parameter_error =
        ErrorType::failed_creating_rns_tool;
    return context_data;
  }

  // Check whether the coefficient modulus consists of a set of primes that are
  // in decreasing order
...
ParmsID SEALContext::createNextContextData(const ParmsID &prev_parms_id) {
  ...
  if (next_context_data.qualifiers_.parameter_error != ErrorType::success) {
    std::cout << "next_context_data.qualifiers_.parameter_error: "
              << next_context_data.qualifiers_.parameterErrorName()
              << std::endl;
  }
  ...
  context_data_map_.emplace(
      make_pair(next_parms_id,
                make_shared<const ContextData>(std::move(next_context_data))));
...
SEALContext::SEALContext(EncryptionParameters parms, bool expand_mod_chain,
  ...
  if (first_context_data.qualifiers_.parameter_error != ErrorType::success) {
    std::cout << "first_context_data.qualifiers_.parameter_error: "
              << first_context_data.qualifiers_.parameterErrorName()
              << std::endl;
  }
  context_data_map_.emplace(
      make_pair(parms.parmsID(),
                make_shared<const ContextData>(std::move(first_context_data))));
  ...
  // key_parms_id_.
  if (!context_data_map_.at(key_parms_id_)->qualifiers_.parametersSet() ||
      parms.coeffModulus().size() == 1) {
    first_parms_id_ = key_parms_id_;
  } else {
    auto next_parms_id = createNextContextData(key_parms_id_);
    first_parms_id_ =
        (next_parms_id == parmsIDZero) ? key_parms_id_ : next_parms_id;
  }
  ...
  // parameter sets as long as they are valid to use (i.e., parameters_set() ==
  // true).
  if (expand_mod_chain &&
      context_data_map_.at(first_parms_id_)->qualifiers_.parametersSet()) {
    auto prev_parms_id = first_parms_id_;
    while (context_data_map_.at(prev_parms_id)->parms().coeffModulus().size() >
           1) {
  ...
  while (context_data_ptr) {
    // We need to remove constness first to modify this
    std::const_pointer_cast<ContextData>(context_data_ptr)->chain_index_ =
        --parms_count;
