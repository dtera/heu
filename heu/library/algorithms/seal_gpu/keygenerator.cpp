...
void KeyGenerator::generateSk(bool is_initialized) {
  ...
  if (!is_initialized) {
    ...
    samplePolyTernary(parms.randomGenerator()->create(), parms,
                      secret_key.get());
  ...
  setPoly(secret_key_.data().data(), coeff_count, coeff_modulus_size,
...
PublicKey KeyGenerator::generatePk() const {
  ...
  encryptZeroSymmetric(secret_key_, context_, context_data.parmsID(), true,
                       public_key.data());
...
RelinKeys KeyGenerator::createRelinKeys(size_t count) {
  ...
  if (!sk_generated_) {
    throw logic_error(
        "cannot generate relinearization keys for unspecified secret key");
  }
  ...
  generateKswitchKeys(secret_key + coeff_count * coeff_modulus_size, count,
...
GaloisKeys KeyGenerator::createGaloisKeysInternal(
  ...
  for (auto galois_elt : galois_elts) {
    ...
    auto rotated_secret_key =
        HostArray<uint64_t>(coeff_count * coeff_modulus_size);
    // SEAL_ALLOCATE_GET_RNS_ITER(rotated_secret_key, coeff_count,
    // coeff_modulus_size, pool_);
    ConstHostPointer secret_key(secret_key_.data().data());
    galois_tool->applyGaloisNtt(secret_key, coeff_modulus_size, galois_elt,
                                rotated_secret_key.asPointer());
    ...
    // Create Galois keys.
    generateOneKswitchKey(rotated_secret_key.asPointer(),
...
void KeyGenerator::computeSecretKeyArray(
  ...
  for (size_t i = 0; i < new_size - old_size; i++) {
    dyadicProductCoeffmod(secret_key_power + i * poly_diff, secret_key,
                          coeff_modulus_size, coeff_count, &coeff_modulus[0],
                          next_secret_key_power + i * poly_diff);
  }
...
void KeyGenerator::generateOneKswitchKey(ConstHostPointer<uint64_t> new_key,
                                         vector<PublicKey> &destination) {
  ...
  size_t decomp_mod_count =
      context_.firstContextData()->parms().coeffModulus().size();
  ...
  for (size_t i = 0; i < decomp_mod_count; i++) {
    // SEAL_ITERATE(iter(new_key, key_modulus, destination, size_t(0)),
    // decomp_mod_count, [&](auto I) {
    // FIXME: allocate related action
    auto temp = HostArray<uint64_t>(coeff_count);
    encryptZeroSymmetric(secret_key_, context_, keyContextData.parmsID(), true,
                         destination[i].data());
    uint64_t factor =
        barrettReduce64(key_modulus.back().value(), key_modulus[i]);
    multiplyPolyScalarCoeffmod(new_key + i * coeff_count, coeff_count, factor,
                               key_modulus[i], temp.asPointer());

    // We use the SeqIter at get<3>(I) to find the i-th RNS factor of the first
    // destination polynomial.
    auto destination_iter =
        HostPointer(destination[i].data().data() + i * coeff_count);
    addPolyCoeffmod(destination_iter, temp.asPointer(), coeff_count,
                    key_modulus[i], destination_iter);
  }
}

void KeyGenerator::generateKswitchKeys(
    util::ConstHostPointer<uint64_t> new_keys, size_t num_keys,
  ...
  for (size_t i = 0; i < num_keys; i++) {
    this->generateOneKswitchKey(new_keys + i * coeff_count * coeff_modulus_size,
                                destination.data()[i]);
  }
}
...
GaloisKeys KeyGenerator::createAutomorphismKeys() {
  size_t poly_degree =
      this->context_.keyContextData()->parms().polyModulusDegree();
