...
constexpr std::size_t prng_seed_byte_count =
    prng_seed_uint64_count * util::bytesPerUint64;
...
class UniformRandomGeneratorInfo {
  ...
  */
  UniformRandomGeneratorInfo(PRNGType type, PRNGSeed seed)
      : type_(type), seed_(std::move(seed)) {}
  ...
  */
  UniformRandomGeneratorInfo &operator=(
      const UniformRandomGeneratorInfo &assign) = default;
  ...
  */
  inline bool operator==(
      const UniformRandomGeneratorInfo &compare) const noexcept {
  ...
  */
  ~UniformRandomGeneratorInfo() { clear(); }
  ...
  */
  inline PRNGType type() const noexcept { return type_; }

  /**
  Returns a reference to the PRNG type.
  */
  inline PRNGType &type() noexcept { return type_; }

  /**
  Returns a reference to the PRNG seed.
  */
  inline const PRNGSeed &seed() const noexcept { return seed_; }

  /**
  Returns a reference to the PRNG seed.
  */
  inline PRNGSeed &seed() noexcept { return seed_; }
  ...
  //         Serialization::ComprSizeEstimate(sizeof(PRNGType) +
  //         prng_seed_byte_count, compr_mode);
  //     return static_cast<std::streamoff>(sizeof(Serialization::SEALHeader) +
  //     members_size);
  // }
  ...
  //         std::bind(&UniformRandomGeneratorInfo::save_members, this, _1),
  //         save_size(compr_mode_type::none), stream, compr_mode, true);
  // }
  ...
  //         std::bind(&UniformRandomGeneratorInfo::load_members, &new_info, _1,
  //         _2), stream, true);
  ...
  // @param[out] out The memory location to write the UniformRandomGeneratorInfo
  // to
  ...
  // */
  // inline std::streamoff save(
  //     std::byte *out, std::size_t size, compr_mode_type compr_mode =
  //     Serialization::compr_mode_default) const
  // {
  //     using namespace std::placeholders;
  //     return Serialization::Save(
  //         std::bind(&UniformRandomGeneratorInfo::save_members, this, _1),
  //         save_size(compr_mode_type::none), out, size, compr_mode, true);
  // }
  ...
  // @param[in] in The memory location to load the UniformRandomGeneratorInfo
  // from
  // @param[in] size The number of bytes available in the given memory location
...
class UniformRandomGenerator {
  ...
  */
  UniformRandomGenerator(PRNGSeed seed)
      : seed_(util::HostArray<uint64_t>(seed.cbegin(), prng_seed_uint64_count)),
        buffer_(util::HostArray<std::byte>(4096)),
        buffer_current_(4096) {}
  ...
  */
  inline void refresh() { refill_buffer(); }
...
class UniformRandomGeneratorFactory {
  ...
  */
  auto create() -> std::shared_ptr<UniformRandomGenerator> {
    return use_random_seed_ ? create_impl([]() {
      PRNGSeed seed;
      random_bytes(reinterpret_cast<std::byte *>(seed.data()),
  ...
  */
  inline bool use_random_seed() noexcept { return use_random_seed_; }
  ...
  */
  inline PRNGSeed default_seed() noexcept { return default_seed_; }

 protected:
  virtual auto create_impl(PRNGSeed seed)
...
class Blake2xbPRNG : public UniformRandomGenerator {
 ...
 protected:
  PRNGType type() const noexcept override { return PRNGType::blake2xb; }
...
class Blake2xbPRNGFactory : public UniformRandomGeneratorFactory {
  ...
  */
  Blake2xbPRNGFactory(PRNGSeed default_seed)
 ...
 protected:
  auto create_impl(PRNGSeed seed)
...
class Shake256PRNG : public UniformRandomGenerator {
 ...
 protected:
  PRNGType type() const noexcept override { return PRNGType::shake256; }
...
class Shake256PRNGFactory : public UniformRandomGeneratorFactory {
  ...
  */
  Shake256PRNGFactory(PRNGSeed default_seed)
      : UniformRandomGeneratorFactory(default_seed) {}
